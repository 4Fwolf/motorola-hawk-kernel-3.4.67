...
getFeatureEnumToStr(int featureID, int featureVal)
{
    ...
    const char *psceneMode[] = {
        ...
        CameraParameters::SCENE_MODE_AUTO, MtkCameraParameters::SCENE_MODE_NORMAL,
...
//<2012/11/19,daisy zang,[8317][New feature][Camera]Add picture size HD(Full Screen) for hawk4.0 front camera
  ...
  /*  int picSizes[] = {
    ...
    const char *ppicSizes[] = {
        ...
        "640x480", "320x240"};*/
//<2012/11/21-17904-alberthsiao,temp solution for 8M capture issue on Hawk 40 
//<2012/12/5-18225-alberthsiao,[Hawk40]remove temp solution for 8M capture issue
#if 1 //defined( ARIMA_PCBA_RELEASE )
//>2012/12/5-18225-alberthsiao
        int picSizes[] = {
        CAPTURE_SIZE_3264_2448, CAPTURE_SIZE_3264_1836,CAPTURE_SIZE_2592_1944,CAPTURE_SIZE_2592_1728,CAPTURE_SIZE_2560_1920, CAPTURE_SIZE_2048_1536, CAPTURE_SIZE_1600_1200, CAPTURE_SIZE_1280_960,CAPTURE_SIZE_1280_720,
        CAPTURE_SIZE_1024_768, CAPTURE_SIZE_640_480, CAPTURE_SIZE_320_240};
    const char *ppicSizes[] = {
        "3264x2448", "3264x1836","2592x1944","2592x1728","2560x1920", "2048x1536", "1600x1200", "1280x960", "1280x720","1024x768",
        "640x480", "320x240"};
#else
        int picSizes[] = {
        CAPTURE_SIZE_3200_2400, CAPTURE_SIZE_3264_1836,CAPTURE_SIZE_2592_1944,CAPTURE_SIZE_2592_1728,CAPTURE_SIZE_2560_1920, CAPTURE_SIZE_2048_1536, CAPTURE_SIZE_1600_1200, CAPTURE_SIZE_1280_960,CAPTURE_SIZE_1280_720,
        CAPTURE_SIZE_1024_768, CAPTURE_SIZE_640_480, CAPTURE_SIZE_320_240};
    const char *ppicSizes[] = {
        "3200x2400", "3264x1836","2592x1944","2592x1728","2560x1920", "2048x1536", "1600x1200", "1280x960", "1280x720","1024x768",
        "640x480", "320x240"};
#endif
//>2012/11/21-17904-alberthsiao
...
//>2012/11/19,daisy zang,[8315][New feature][Camera]Add picture size 16:9 (6M) for hawk 4.0 and 3:2 (4.5M) for hawk3.5
    ...
    int prvSizes[] = {
        ...
        //<2012/11/05-josephhsu, ALPS00383678-Hawk35-preview-not-clear, PREVIEW_SIZE_384_288 added
        PREVIEW_SIZE_640_480, PREVIEW_SIZE_480_368, PREVIEW_SIZE_480_320, PREVIEW_SIZE_384_288, PREVIEW_SIZE_352_288, PREVIEW_SIZE_320_240, PREVIEW_SIZE_176_144,
        //>2012/11/05-josephhsu
        PREVIEW_SIZE_160_120};
    const char *pprvSizes[] = {
        "1920x1088", "1280x720", "800x600", "864x480", "800x480", "720x480",
        //<2012/11/05-josephhsu, ALPS00383678-Hawk35-preview-not-clear, PREVIEW_SIZE_384_288 added
        "640x480", "480x368", "480x320", "384x288", "352x288", "320x240", "176x144",
        //>2012/11/05-josephhsu
        "160x120"};
    //
    //    
...
//        FRAME_RATE_075FPS};
    ...
    int pcapModes[] = {
        ...
        CAPTURE_MODE_BEST_SHOT, CAPTURE_MODE_EV_BRACKET, CAPTURE_MODE_MAV, CAPTURE_MODE_ASD, CAPTURE_MODE_PANO_3D, CAPTURE_MODE_SINGLE_3D, CAPTURE_MODE_FACE_BEAUTY, CAPTURE_MODE_CONTINUOUS_SHOT};
...
getCamFeatureEnums()
{
    ...
    if (status != NO_ERROR) {
        ...
        mParameters.set(getSupportedValuesSuffix((char *) MtkCameraParameters::KEY_STEREO_3D_PICTURE_SIZE), "2560x720");        
        // Stereo 3D-preview-size
        mParameters.set(MtkCameraParameters::KEY_STEREO_3D_PREVIEW_SIZE, "1280x720");
        mParameters.set(getSupportedValuesSuffix((char *) MtkCameraParameters::KEY_STEREO_3D_PREVIEW_SIZE), "1280x720");
        // Stereo 3D-type 
    ...
    if  ( NULL == mParameters.get(getSupportedValuesSuffix((char*)CameraParameters::KEY_VIDEO_STABILIZATION)) )
    {
        mParameters.set(CameraParameters::KEY_VIDEO_STABILIZATION_SUPPORTED, CameraParameters::FALSE);
        MY_LOGD("NULL video-stabilization-values");
    }
    else
...
updateShotMode(String8 const& rs8ShotMode)
{
    ...
    uint32_t u4NewShotMode = Map::inst().valueFor(rs8ShotMode);
    //
    String8 sSceneMode(mParameters.get(MtkCameraParameters::KEY_SCENE_MODE));
    if  ( sSceneMode == CameraParameters::SCENE_MODE_HDR )
    {
        u4NewShotMode = MHAL_CAM_CAP_MODE_HDR;
        MY_LOGI("HDR Scene");
    }
    //
    ...
    if(u4NewShotMode==MHAL_CAM_CAP_MODE_BURST_SHOT || u4NewShotMode==MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT)
    {
    	MY_LOGI("line=%d shot mode=%d burst=%d, conti=%d", __LINE__, u4NewShotMode, (int)MHAL_CAM_CAP_MODE_BURST_SHOT, (int)MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT);
        mmHalCamParam.cam3AParam.isBurstShotMode=1;
    }
    else
    {		
    	MY_LOGI("linee=%d shot mode=%d burst=%d, conti=%d", __LINE__, u4NewShotMode, (int)MHAL_CAM_CAP_MODE_BURST_SHOT, (int)MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT);
...
setParameters(const CameraParameters& params)
{
    ...
    int changed = 0;
    int picWidth = 0, picHeight = 0, prvWidth = 0, prvHeight = 0;
...
#warning "[FIXME] when scene mode changes, only scene-dependent parameters must be updated; otherwise no change"
        ...
        CAM_LOGD("[setParameters]getPictureSize:picWidth(%d),picHeight(%d)",picWidth,picHeight); 
        mParameters.setPictureSize(picWidth, picHeight);
        //sync PreviewSize
        params.getPreviewSize(&prvWidth, &prvHeight);
        CAM_LOGD("[setParameters]getPreviewSize:prvWidth(%d),prvHeight(%d)",prvWidth,prvHeight); 
    ...
    MBOOL Enable3D;
    //
    Property::get(MHAL_CAM_STEREO_3D_ENABLE,value,MHAL_CAM_STEREO_3D_ENABLE_N);
    Enable3D = (MBOOL)(atoi(value));
    CAM_LOGD("[setParameters]MHAL_CAM_STEREO_3D_ENABLE(%d)",Enable3D);
    //
    if(Enable3D)
    {
        pstrPreSize = (char *) MtkCameraParameters::KEY_STEREO_3D_PREVIEW_SIZE;
        if((params.get(pstrPreSize)) != NULL)
        {
            const char *pSizeStr  = params.get(pstrPreSize); 
            status_t ret = NO_ERROR;
            ret = getImgSize(
                    ...
                    pSizeStr,
                    strlen(pSizeStr) + 1);
            CAM_LOGD("[setParameters]getImgSize:prvWidth(%d),prvHeight(%d)",prvWidth,prvHeight); 
            if(ret != NO_ERROR)
            {
                CAM_LOGE("[%s]ERROR: Invalid Preview Size = %s \n", __FUNCTION__, pSizeStr);
                return ret;
        ...
        //
        pstrPicSize = (char *) MtkCameraParameters::KEY_STEREO_3D_PICTURE_SIZE;
        if((params.get(pstrPicSize)) != NULL)
        {
            const char *pSizeStr  = params.get(pstrPicSize); 
            status_t ret = NO_ERROR;
            ret = getImgSize(
                    picWidth,
                    picHeight,
                    pSizeStr,
                    strlen(pSizeStr) + 1);
            CAM_LOGD("[setParameters]getImgSize:picWidth(%d),picHeight(%d)",picWidth,picHeight); 
            if(ret != NO_ERROR)
            {
                CAM_LOGE("[%s]ERROR: Invalid Preview Size = %s \n", __FUNCTION__, pSizeStr);
                return ret;
...
initDefaultParameters()
{
    ...
    mhalCamSensorInfo_s mhalCamSensorInfo;
    if  ( 0 == ::mHalIoctl(mmHalFd, MHAL_IOCTL_QUERY_SENSOR_INFO, (void*)getOpenId(), 0, &mhalCamSensorInfo, sizeof(mhalCamSensorInfo), 0) )
    {
        mParameters.set(CameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, mhalCamSensorInfo.viewAngles[0]);
        mParameters.set(CameraParameters::KEY_VERTICAL_VIEW_ANGLE, mhalCamSensorInfo.viewAngles[1]);
        MY_LOGD("OpenId:%d view-angles:%d %d", getOpenId(), mhalCamSensorInfo.viewAngles[0], mhalCamSensorInfo.viewAngles[1]);
    }
    else
    {
...
//<2013/02/06-ClarkLin-Focal Length for CTS
    ...
    mParameters.set(CameraParameters::KEY_FOCAL_LENGTH, "4.27"); // Should query from driver
#elif defined(ARIMA_PROJECT_HAWK40)
    mParameters.set(CameraParameters::KEY_FOCAL_LENGTH, "4.36"); // Should query from driver
#else
    ...
    int         sizeLength)
{
    ...
    if ( NULL == pStart ) {
        ...
        rWidth = static_cast<MINT32>(strtol(pStart, &pEnd, 10));
    }
    // height 
    if ( sep != *pEnd ) {
        CAM_LOGE("[%s] Parsing of the height failed!", __FUNCTION__);
        ...
        rHeight = static_cast<MINT32>(strtol(pEnd+1, &pEnd, 10));
    }

    if ((rWidth < 0) || (rHeight < 0)) {
        CAM_LOGE("img size error!");
        ret = -EINVAL;
    }
...
updatePreviewSize()
{
    ...
    // workaround for 1080p 
    if (oriPrvSize.height == 1080) 
    {
        mParameters.setPreviewSize(oriPrvSize.width, 1088); 
        oriPrvSize.height = 1088; 
    }
    //
    ...
    **************************************************************************/
    if ( prvSizeDiff > 0 )
    {
        MY_LOGD("new prvSize(%dx%d)", candPrvSize.width, candPrvSize.height);
        mParameters.setPreviewSize(candPrvSize.width, candPrvSize.height);
    }
    else
    {
        MY_LOGD("new prvSize(%dx%d)", candPrvSize.height, candPrvSize.width);
