...
MTKCameraHardware::MTKCameraHardware(String8 const& rName, int32_t const i4OpenId, CamDevInfo const& rDevInfo)
	...
	mbCanCShotQvCB(true),
    mSemCShotQvCB(),
    mbCShotQvCBThreadAlive(0),
    mCShotQvCBThreadHandle(0),
    //
    mpMainThread(NULL),
    //   
    mEventCallback(NULL),    
...
dispatchCallback(void*const param)
{
    ...
    case MHAL_CAM_CB_POSTVIEW:
		...
		if (MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT == mu4ShotMode)
		{
			cshotQvCallback();
		}
		else
...
//            case MHAL_CAM_CAP_MODE_NORMAL:
    ...
    case MHAL_CAM_CB_AUTORAMA:
        {
            ...
            if  ( mpBufProvider->allocBuffer(mem, u4DataSize, 1) )
            {
                if  ( mem.data && u4DataSize <= mem.size )
                {
                    ...
                    //if (pCBData[2])
                    //    mNotifyCb(CAMERA_MSG_SHUTTER, 0, 0, mCallbackCookie);
                    CAM_LOGD("[dispatchCallback] MHAL_CAM_CB_AUTORAMA pCBData[0] %d,pCBData[1] %d,pCBData[2] %d,pCBData[3] %d\n",pCBData[0],pCBData[1],pCBData[2],pCBData[3]);
                    mDataCb(MTK_CAMERA_MSG_EXT_DATA, &mem, 0, NULL, mCallbackCookie);
                }
                mpBufProvider->freeBuffer(mem);
            }
    ...
    case MHAL_CAM_CB_AUTORAMAMV:
        ...
        break;
    case MHAL_CAM_CB_PANO_ORI_DATA:
        {
            CAM_LOGD("[dispatchCallback] MHAL_CAM_CB_PANO_ORI_DATA u4Size %d",u4Size);
            uint32_t const jpegSize = u4Size;
            uint32_t const u4DataSize = jpegSize + sizeof(uint32_t) * (1+2); 
            camera_memory_t mem;
            if  ( mpBufProvider->allocBuffer(mem, u4DataSize, 1) )
            {
                if  ( mem.data && u4DataSize == mem.size )
                {
                    ...
                    pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_AUTORAMA;
                    pCBData[1] = 2;
                    ::memcpy((void*)(&pCBData[2]), (void*)pvData, jpegSize);
                    //
                    mDataCb(MTK_CAMERA_MSG_EXT_DATA, &mem, 0, NULL, mCallbackCookie);
                }
                mpBufProvider->freeBuffer(mem);  
        ...
        break;
    case MHAL_CAM_CB_MAV_ORI_DATA:
        {
            CAM_LOGD("[dispatchCallback] MHAL_CAM_CB_MAV_ORI_DATA u4Size %d",u4Size);
            uint32_t const jpegSize = u4Size;
            uint32_t const u4DataSize = jpegSize + sizeof(uint32_t) * (1+4); 
            camera_memory_t mem;
            if  ( mpBufProvider->allocBuffer(mem, u4DataSize, 1) )
            {
                if  ( mem.data && u4DataSize == mem.size )
                {
                    uint32_t*const pCBData = reinterpret_cast<uint32_t*>(mem.data);
                    pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_MAV;
                    pCBData[1] = 2;
                    ::memcpy((void*)(&pCBData[2]), (void*)pvData, jpegSize);
                    //
                    mDataCb(MTK_CAMERA_MSG_EXT_DATA, &mem, 0, NULL, mCallbackCookie);
        ...
        break; 
    case MHAL_CAM_CB_FB_ORI_DATA:
        {
            CAM_LOGD("[dispatchCallback] MHAL_CAM_CB_FB_ORI_DATA");
            uint32_t const jpegSize = u4Size;
            uint32_t const u4DataSize = jpegSize + sizeof(uint32_t) * (1+1); 
            camera_memory_t mem;
            if  ( mpBufProvider->allocBuffer(mem, u4DataSize, 1) )
            {
                ...
                {
                    uint32_t*const pCBData = reinterpret_cast<uint32_t*>(mem.data);
                    pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_FACEBEAUTY;
                    pCBData[1] = 0;
                    ::memcpy((void*)(&pCBData[2]), (void*)mJpegMemPool->getVirAddr(), jpegSize);
                    //
                    mDataCb(MTK_CAMERA_MSG_EXT_DATA, &mem, 0, NULL, mCallbackCookie);
        ...
        break; 
    case MHAL_CAM_CB_HDR_ORI_DATA:
        {
            CAM_LOGD("[dispatchCallback] MHAL_CAM_CB_HDR_ORI_DATA");
            uint32_t const jpegSize = u4Size;
            uint32_t const u4DataSize = jpegSize + sizeof(uint32_t) * (1+1); 
            camera_memory_t mem;
            if  ( mpBufProvider->allocBuffer(mem, u4DataSize, 1) )
            {
                ...
                {
                    uint32_t*const pCBData = reinterpret_cast<uint32_t*>(mem.data);
                    pCBData[0] = MTK_CAMERA_MSG_EXT_DATA_HDR;
                    pCBData[1] = 0;
                    ::memcpy((void*)(&pCBData[2]), (void*)mJpegMemPool->getVirAddr(), jpegSize);
                    //
                    mDataCb(MTK_CAMERA_MSG_EXT_DATA, &mem, 0, NULL, mCallbackCookie);
...
*
********************************************************************************/
void* MTKCameraHardware::_cshotQvCBThread(void* arg)
{
    ::prctl(PR_SET_NAME,"Camera@CShotQvCB",0,0,0);
    //
    //  thread policy & priority
    int policy = 0, priority = 0;
    getPriority(policy, priority);
    CAM_LOGD("[mHalCamAdapter::_cshotQvCBThread] (tid, policy, priority)=(%d, %d, %d)", ::gettid(), policy, priority);
    ...
    return  pHardware->cshotQvCBThread();
}
/******************************************************************************
*
*******************************************************************************/
...
cshotQvCBThread()
{
    MY_LOGD("+ tid(%d), mbCShotQvCBThreadAlive(%d)", ::gettid(), mbCShotQvCBThreadAlive);
    //
    while (mbCShotQvCBThreadAlive == 0x55555555)
    {
        ::sem_wait(&mSemCShotQvCB);  
        if ( mbCShotQvCBThreadAlive != 0x55555555)
        {
            break;
        }
        
        CamProfile profile(__FUNCTION__, "MTKCameraHardware");
        sp<IImageBufferPool>   pMemPool;
        if (mCShotQvMemPool.get() ) {
            pMemPool = mCShotQvMemPool; 
        }
		
        if(pMemPool == NULL)
            continue;
		
		postBuffer(pMemPool, 0, 0, CAMERA_MSG_POSTVIEW_FRAME);
		mbCanCShotQvCB = true;    
    }
    //
    MY_LOGD("- tid(%d)", ::gettid());
...
*
*******************************************************************************/
void MTKCameraHardware::cshotQvCallback()
{
    CAM_LOGD("cshotQvCallback + ");

    if ( !mbCanCShotQvCB ) {
		MY_LOGD("CShot: show qv too long , skip one ");
        return; 
    }
	
	//memcpy here
	::memcpy((void*)mCShotQvMemPool->getVirAddr(), (void*)mQvMemPool->getVirAddr(), mQvMemPool->getBufSize());
    mbCanCShotQvCB = false;    
    sem_post(&mSemCShotQvCB);
...
status_t MTKCameraHardware::startPreviewInternal()
{
	...
	CAM_LOGD("Check Recording hint\n");
	char const* valstr = NULL;
	valstr = mParameters.get(CameraParameters::KEY_RECORDING_HINT); 
	if ( valstr ) {
		if (::strcmp(valstr, "true") == 0) {
			CAM_LOGD("Recording hint is true \n");
			mmHalCamParam.u4CamMode = 2;
		} else {
			CAM_LOGD("Recording hint is false \n");
		}
	} else {
	    CAM_LOGD("Recording hint is NULL \n");
	}
...
#else 
    // using the preview size in support list. 
    updatePreviewSize(); 
...
status_t MTKCameraHardware::allocCamCaptureMem()
{
    ...
    if (mCShotQvMemPool == NULL && mu4ShotMode == MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT) {
        int qvW = 0, qvH = 0;
        //qv size is the same as preview size
        mParameters.getPreviewSize(&qvW, &qvH);
        if(1920 == qvW && 1088 == qvH)
        ...
        mCShotQvMemPool = allocImagePool(MHAL_CAM_BUF_POSTVIEW, qvW, qvH, "cshot cb qv");
        //
        if (mCShotQvMemPool == NULL) {
            CAM_LOGD("[allocCamCaptureMem] Err Not enough MEMORY for mCShotCBQvPmemPool \n\n");
            return NO_MEMORY;
        }
    }    
    //
...
status_t MTKCameraHardware::takePictureInternal()
{
    ...
    MINT32 mode = 0;
    mode = mParameters.getInt(MtkCameraParameters::KEY_RAW_SAVE_MODE);
    if (mode > 0)
    {
     	LOGD("[EM] cap mode: %d", mode);
        mhalCamRawImageInfo_t rawInfo;
        memset(&rawInfo, 0, sizeof(mhalCamRawImageInfo_t));              
        ::mHalIoctl(mmHalFd, MHAL_IOCTL_GET_RAW_IMAGE_INFO, &mode, sizeof(int), &rawInfo, sizeof(mhalCamRawImageInfo_t), NULL);
        capW = rawInfo.u4Width;
        capH = rawInfo.u4Height;
    }    
    // Get Zoom dimension
    mZoomValue = mParameters.getInt(MtkCameraParameters::KEY_ZOOM);
    ...
    case MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT:
        ...
        if (strcmp(mParameters.get(MtkCameraParameters::KEY_FAST_CONTINUOUS_SHOT), MtkCameraParameters::ON) == 0)
            mmHalCamParam.u4ContinuousShotSpeed = 8;
        else
            mmHalCamParam.u4ContinuousShotSpeed = 5;
...
//<2012/12/26,daisy zang,[8317][Camera]Mtk patch for kpi time not the same issue of land and port save image 
    /*
    else if (mu4ShotMode == MHAL_CAM_CAP_MODE_CONTINUOUS_SHOT ||
                                         ...
                                         int32_t arg2)
{
    ...
    case CAMERA_CMD_STOP_MAV:
        status = stopMAV(arg1);
        break;
#warning [FIX_ME]KK_Migration[SeanC]
#if 0 
    case CAMERA_CMD_START_3DSHOT:
	    status = start3Dshot(arg1);
	    break;
    case CAMERA_CMD_STOP_3DSHOT:
        status = stop3Dshot(arg1);
        break;
#endif
    ...
    case CAMERA_CMD_SET_CSHOT_SPEED:
        mmHalCamParam.u4ContinuousShotSpeed = arg1;
        break;
...
init()
{
    ...
    //  cshot qv cb thread
    {
        mbCShotQvCBThreadAlive = 0x55555555;
        pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR, RTPM_PRIO_CAMERA_PREVIEW};
        ::pthread_create(&mCShotQvCBThreadHandle, &attr, _cshotQvCBThread, this);
    }
    //
    //  main thread
...
{
    MY_LOGI(
        "+ caller tid(%d), OpenId(%d), getStrongCount(%d), mPreviewThreadHandle(%lx), mVideoThreadHandle(%lx),mCShotQvCBThreadHandle(%lx), mmHalFd(%p)", 
        ::gettid(), getOpenId(), getStrongCount(), mPreviewThreadHandle, mVideoThreadHandle, mCShotQvCBThreadHandle, mmHalFd
    );
