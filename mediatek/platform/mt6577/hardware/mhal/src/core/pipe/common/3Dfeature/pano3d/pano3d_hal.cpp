...
getInstance()
{
    MHAL_LOG("[halPANO3D] getInstance \n");
    if (pHalPANO3D == NULL) {
...
)
{
    ...
    MHAL_LOG("[mHal3dfInit] \n");
    MHAL_LOG("[mHal3dfInit] MavInitInData 0x%x MotionInitInData 0x%x WarpInitInData 0x%x Pano3DInitInData 0x%x\n",(MUINT32)MavInitInData,(MUINT32)MotionInitInData,(MUINT32)WarpInitInData,(MUINT32)Pano3DInitInData);
    if (m_pMTKMavObj) 
        MHAL_LOG("[mHal3dfInit] m_pMTKMavObj Init has been called \n");    
    else    	
    ...
    if (m_pMTKMotionObj) 
        MHAL_LOG("[mHal3dfInit] m_pMTKMotionObj Init has been called \n");    
    else
    ...
    MHAL_ASSERT(m_pMTKPano3dObj != NULL, "Err");
...
{
	  MINT32 err = S_MAV_OK;
    MHAL_LOG("[mHalPANO3DMain] \n");	
    if (!m_pMTKPano3dObj) {
        err = E_MAV_ERR;
        MHAL_LOG("[mHalPANO3DMain] Err, Init has been called \n");
    }
...
{
	  MINT32 err = S_MAV_OK;
    MHAL_LOG("[mHalPANO3DAddImg] \n");	
    if (!m_pMTKMavObj) {
        err = E_MAV_ERR;
        MHAL_LOG("[mHalPANO3DAddImg] Err, Init has been called \n");
    }
...
)
{
    ...
    MHAL_LOG("[mHalPANO3DMerge] \n");	
    if (!m_pMTKMavObj) {
        err = E_MAV_ERR;
        MHAL_LOG("[mHalPANO3DMerge] Err, Init has been called \n");
    }
...
)
{
    ...
    MHAL_LOG("[mHalPANO3DWarp] ImgNum %d\n",ImgNum);	
    if (!m_pMTKWarpObj) {
        err = E_MAV_ERR;
        MHAL_LOG("[mHalPANO3DWarp] Err, Init has been called \n");
    }
...
)
{
	 ...
	 {	
	 	  MHAL_LOG("[mHalPANO3DGetResult] Stitch fail\n");	
	 	  err = E_MAV_ERR;
	 }
	 else
	 {
	 	  memcpy(Pano3dResult,(void *)&MyPano3DResultInfo,sizeof(Pano3DResultInfo));
	 	  MHAL_LOG("[mHalPANO3DGetResult] Stitch success\n");		 	   
	 }	
	 return err;	
}
...
********************************************************************************/
MINT32
halPANO3D::mHal3dfGetResult(MUINT32& MavResult
)
{
	 MINT32 err = S_MAV_OK;
	 ...
	 {	
	 	  MHAL_LOG("[mHalPANO3DGetResult] Warp fail\n");	
	 	  MavResult=0;
	 	  err = E_MAV_ERR;
	 }
	 else
	 {
	 	  MHAL_LOG("[mHalPANO3DGetResult] Warp success\n");	
	 	  MavResult=1; 	 	   
	 }	
	 return err;	
}
