...
#ifdef DEBUG_MCU_DRV
#define MCU_DRV_DBG(fmt, arg...) XLOGD(LOG_TAG fmt, ##arg)
#define MCU_DRV_ERR(fmt, arg...) XLOGE(LOG_TAG "Err: %5d:, "fmt, __LINE__, ##arg)
#else
...
)
{
    ...
    MCU_DRV_DBG("lensSearch() - E \n"); 
    MCU_DRV_DBG("[CurrSensorDev]0x%04x\n", a_u4CurrSensorDev);
    ...
    if (a_u4CurrSensorDev == 0)   // only search for main sensor, dummy for sub sensor always
    {
        ...
        for (i=0; i<MAX_NUM_OF_SUPPORT_LENS; i++) {
            if (MCUDrv::m_LensInitFunc[i].SensorId == a_u4CurrSensorId)
            {
                MCUDrv::m_u4CurrLensIdx = i;
                break;
            }
    ...
    MCU_DRV_DBG("[CurrLensIdx]%d", m_u4CurrLensIdx);
