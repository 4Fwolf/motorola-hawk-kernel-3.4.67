...
********************************************************************************/
#undef LOG_TAG
#define LOG_TAG "EepromDrv"

#define EEPROM_DRV_LOG(fmt, arg...)    XLOGD(LOG_TAG " "fmt, ##arg)
#define EEPROM_DRV_ERR(fmt, arg...)    XLOGE(LOG_TAG "Err: %5d: "fmt, __LINE__, ##arg)
...
//    int err = EEPROM_NO_ERROR;
    ...
    if ((a_eSensorType > DUAL_CAMERA_SUB_SENSOR) ||(a_eSensorType < DUAL_CAMERA_MAIN_SENSOR) ||
    ...
    if((a_eEepromDataType!=CAMERA_EEPROM_DATA_PREGAIN)&&
    	( la_pEepromData->pCameraPara!= NULL))
    {
    ...
    pEerom_data->Shading.ISPComm.CommReg[0] = u4SensorID;   
    EEPROM_DRV_LOG("[2GetEepromCalData] pEerom_data->Shading.ISPComm.CommReg[0](0x%x) %x\n",pEerom_data->Shading.ISPComm.CommReg[0],u4SensorID);
    if(!bfFirstCalData)
    {
    ...
    case CAMERA_EEPROM_DATA_SHADING_TABLE:
        ...
        else if (m32EepromDataValidation&EEPROM_ERR_MTKOTP_SHADING)
        {
            EEPROM_DRV_LOG("(0x%x)MTK OTP Shading \n",  m32EepromDataValidation);	
        }
        else
        {
            if((la_pEepromData->pCameraPara!=NULL)&&(la_pEepromData->pCameraShading!=NULL))
            {
                if(eEepromCaldata.Shading.ISPComm.CommReg[EEPROM_INFO_IN_COMM_LOAD] != CAL_DATA_LOAD)
                {
                    ...
                    {
                        //la_pEepromData->pCameraPara->ISPRegs.Idx.Shading = 2; //for diable ISP Shading.
                    ...
                    else
                    {
                        ...
                        la_pEepromData->pCameraPara->ISPRegs.Shading[0].shading_ctrl2.val=eEepromCaldata.Shading.PreRegister.shading_ctrl2;
                        la_pEepromData->pCameraPara->ISPRegs.Shading[0].shading_last_blk.val=eEepromCaldata.Shading.PreRegister.shading_last_blk;
                        la_pEepromData->pCameraPara->ISPRegs.Shading[1].shading_ctrl2.val=eEepromCaldata.Shading.CapRegister.shading_ctrl2;
                        la_pEepromData->pCameraPara->ISPRegs.Shading[1].shading_last_blk.val=eEepromCaldata.Shading.CapRegister.shading_last_blk;
