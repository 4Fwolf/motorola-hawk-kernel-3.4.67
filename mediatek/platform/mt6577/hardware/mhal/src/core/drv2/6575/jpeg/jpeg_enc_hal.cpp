...
JPEG_ENC_STATUS_ENUM jpegEncConfigEncoder(int drvID, JPEG_ENC_HAL_IN inParams)
{
    ...
    drvIn.dstBufferAddr = (unsigned char*)dst_mva;
#else
    drvIn.dstBufferAddr = inParams.dstBufferAddr;
#endif
    
    drvIn.dstBufferSize = inParams.dstBufferSize;
    drvIn.dstWidth = inParams.dstWidth;
    drvIn.dstHeight = inParams.dstHeight;
    drvIn.dstFormat = (unsigned int)inParams.dstFormat;
    drvIn.dstQuality = (unsigned int)inParams.dstQuality;
...
JPEG_ENC_STATUS_ENUM jpegEncWaitIRQ(int drvID, long timeout_msec, unsigned int *encodeSize, JPEG_ENC_RESULT_ENUM *status)
{
    ...
    unsigned int fileSize, encodeResult;
