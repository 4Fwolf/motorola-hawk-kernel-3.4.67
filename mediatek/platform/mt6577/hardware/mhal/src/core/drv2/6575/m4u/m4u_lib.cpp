...
static void m4u_dump_backtrace(void)
{
    ...
    M4UMSG("m4u dump back trace when error============> \n");
        M4UMSG("m4u fd=%d, module=%d\n", mFileDescriptor, eModuleID);
...
{    
    ...
    {
        M4UMSG("warning: fd=-1 when ~MTKM4UDrv!\n");
    }
    //pthread_mutex_unlock(&fdMutex);
}
...
{
    return m4u_alloc_mva(eModuleID, NULL, BufAddr, BufSize, pRetMVAAddr);
}
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_query_mva(M4U_MODULE_ID_ENUM eModuleID, 
								  ...
								  unsigned int *pRetMVAAddr)
{
    return m4u_query_mva(eModuleID, NULL, BufAddr, BufSize, pRetMVAAddr);
}
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_dealloc_mva(M4U_MODULE_ID_ENUM eModuleID, 
                  ...
                  const unsigned int MVAStart)
{
    return m4u_dealloc_mva(eModuleID, NULL, BufAddr, BufSize, MVAStart);
}
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_config_port(M4U_PORT_STRUCT* pM4uPort)
{
    ...
    {
        M4UMSG("m4u fd=%d, port=%d\n", mFileDescriptor, pM4uPort->ePortID);
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_get_task_struct(void** pTask, unsigned int *pOomAdj)
{
    ...
    if(mFileDescriptor == -1 )
    {
        M4UMSG("m4u_get_task_struct failed \n");
        m4u_dump_backtrace();
        //pthread_mutex_unlock(&fdMutex);
        return M4U_STATUS_INVALID_HANDLE;
    }
	        
    if(ioctl(mFileDescriptor, MTK_M4U_T_GET_TSK_STRUCT, &args))
    {
        M4UMSG("m4u fd = %d\n", mFileDescriptor);
        M4UMSG(" ioctl MTK_M4U_T_GET_TSK_STRUCT fail! %d, %s \n", errno, strerror(errno));
        m4u_dump_backtrace();
        //pthread_mutex_unlock(&fdMutex);
        return M4U_STATUS_KERNEL_FAULT;
    }
...
M4U_STATUS_ENUM MTKM4UDrv::m4u_put_task_struct(unsigned int pOomAdj)
{
    struct GET_TSK_STRUCT_ARGS args;
    args.oom_adj = pOomAdj;

    //pthread_mutex_lock(&fdMutex);
    if(mFileDescriptor == -1 )
    {
        M4UMSG("m4u_put_task_struct failed \n");
        m4u_dump_backtrace();
        //pthread_mutex_unlock(&fdMutex);
        return M4U_STATUS_INVALID_HANDLE;
    }
	        
    if(ioctl(mFileDescriptor, MTK_M4U_T_PUT_TSK_STRUCT, &args))
    {
        M4UMSG("m4u fd = %d\n", mFileDescriptor);
        M4UMSG(" ioctl MTK_M4U_T_PUT_TSK_STRUCT fail! %d, %s \n", errno, strerror(errno));
        m4u_dump_backtrace();
        //pthread_mutex_unlock(&fdMutex);
        return M4U_STATUS_KERNEL_FAULT;
    }
