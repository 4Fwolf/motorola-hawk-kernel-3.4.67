...
int main()
{
    ...
    h = 480;//64;
    // allocate PMEM
    src_va = (unsigned char *)pmem_alloc_sync(BUFFER_SIZE + 16, &src_fd);
    if(src_va == NULL)
    {
...
//
    ...
    xlog("src va:0x%x, pa:0x%x dst va:0x%x, pa:0x%x", (unsigned long)src_va , src_pa, (unsigned long)dst_va , dst_pa);
#else
    dst_size = BUFFER_SIZE;
    ...
    jpgEncoder->setDstWidth(720);
    jpgEncoder->setDstHeight(480);
    jpgEncoder->setEncFormat(JpgEncHal::kYUV_420_Format);
    jpgEncoder->setSrcFormat(JpgEncHal::kNV21_Format);
    jpgEncoder->setQuality(95);

    jpgEncoder->setSrcAddr((void *)src_va);
    jpgEncoder->setDstAddr((void *)dst_va);
    jpgEncoder->setDstSize(dst_size);

    if(jpgEncoder->start(&enc_size))
    {
        FILE *fp;
        fp = fopen("/data/ttt.jpg", "w");
        int index;
    ...
    pmem_free(src_va , BUFFER_SIZE + 16 , src_fd);
