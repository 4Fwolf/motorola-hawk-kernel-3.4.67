...
*
*******************************************************************************/
class IMhal_facebeauty : public IShot
{
private:
    IMhal_facebeauty(ShotBase*const pShot);
    virtual ~IMhal_facebeauty() {}

public:     ////    Interfaces.
    //
    static IMhal_facebeauty*    createInstance(ESensorType_t const eSensorType, EDeviceId_t const eDeviceId);

    virtual MVOID   destroyInstance();

    virtual MBOOL   init(ShotParam const& rShotParam, Hal3ABase*const pHal3A, MUINT32 const rMetadata, MUINT32 const rFaceInfo);
