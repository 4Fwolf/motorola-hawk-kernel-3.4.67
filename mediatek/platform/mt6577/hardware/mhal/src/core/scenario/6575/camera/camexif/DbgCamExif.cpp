...
init(CamExifParam const& rCamExifParam, Hal3ABase*const pHal3A, CamDbgParam const& rCamDbgParam)
{
    ...
    //  (0)
    if  ( ! CamExif::init(rCamExifParam, pHal3A) )
    {
        goto lbExit;
    }
    ...
    {
        MY_LOGE("[DbgCamExif::init] (mpIspHal, ec)=(%p, %X)", mpIspHal, err);
        if  ( mpIspHal )
        {
...
appendDebugExif(MUINT8* const puAppnBuf, MUINT32*const pu4AppnSize)
{
    ...
    // Copy ISP debug info
    {
        ...
        {
            MY_LOGE("[DbgCamExif::appendDebugExif] ispDbgSize(%d) > 3072", ispDbgSize);
            goto lbExit;
        }
    }
    ...
    // Cam debug info
    {
        ...
        {
            MY_LOGE("[DbgCamExif::appendDebugExif] camDebugSize(%d) < 0", camDebugSize);
            goto lbExit;
        }
    }
