...
********************************************************************************/
ShotBase::
ShotBase(char const*const szShotName, ESensorType_t const eSensorType, EDeviceId_t const eDeviceId)
    : IShot()
    //
...
invokeQvCB(MUINT32 const u4Type, MVOID*const pData, MUINT32 const u4Size /*= 0*/)
{
    ...
    return  invokeCallback(u4Type, pData, u4Size);
}
...
shutterCBThread(MVOID* arg)
{
    ...
    //
    ShotProfile profile("shutterCBThread");
    //
...
handleCaptureDone(MINT32 const i4ErrCode /*= 0*/)
{
    ...
    invokeQvCB(MHAL_CAM_CB_POSTVIEW, NULL);
...
{
    ShotProfile profile("makeQuickViewThumbnail", getShotName());
    MY_LOGD("[makeQuickViewThumbnail]");

    MBOOL   ret = MTRUE;
    BufInfo const thumbnailBufInfo = getBufInfo_ThumbnailEnc();
    BufInfo const qvBufInfo = getBufInfo_quickViewEnc(); 
...
{
    ShotProfile profile("flattenJpgPicBuf", getShotName());
    ...
    //  (0) Init Exif Object.
    DbgCamExif dbgCamExif(meSensorType, meDeviceId);
    ret = dbgCamExif.init(
    ...
    MUINT32 w = frmJpg.w;
    MUINT32 h = frmJpg.h;
    if (mShotParam.u4CapPreFlag)
    {
        mpIspHal->sendCommand(mShotParam.u4CapPreFlag ? ISP_CMD_GET_SENSOR_PRV_RANGE : ISP_CMD_GET_SENSOR_FULL_RANGE, (int)&w, (int)&h);
    }
    //--
    ret = dbgCamExif.makeExifApp1(w, h, u4ThumbSize, pdest, &u4App1HeaderSize);
    if  ( ! ret )
    {
        goto lbExit;
    }
    ...
    {
        MY_LOGD("[flattenJpgPicBuf] u4App1Size(%d) is bigger than 64KB", u4App1Size);
    }


    //  (3) Append Debug Exif Info.
    ret = dbgCamExif.appendDebugExif(pdest, &u4AppnDebugSize);
    ...
    //  (5) 
    ru4JpgPictureSize = u4App1Size + u4AppnDebugSize + u4JpgImgSize;
...
BufInfo
ShotBase::
getBufInfo_quickViewEnc() const 
{
    if (0 == mShotParam.u4JPEGOrientation ) {
        return BufInfo (
        ...
        return BufInfo (
           mrRotatedQVBuf.u4BufVA, 
           mrRotatedQVBuf.u4BufPA, 
           mrRotatedQVBuf.u4BufSize, 
           1
        ); 
    }
}
...
MBOOL
ShotBase::
allocateRotatedQuickViewBuf(BufInfo &rBuf)
{
    //
    rBuf.u4BufVA = (MUINT32)(UINT8*)memalign(L1_CACHE_BYTES, rBuf.u4BufSize); 
    MY_LOGI("[allocateRotatedQuickViewBuf] addr:0x%x, size:%d \n", rBuf.u4BufVA, rBuf.u4BufSize); 
    if (0 == rBuf.u4BufVA) {
        MY_LOGE("[allocateRotatedQuickViewBuf] Not enought for rotated YUV memory, allocated size = %d\n", rBuf.u4BufSize); 
        return false; 
    } 
...
MBOOL
ShotBase::
freeRotatedQuickViewBuf(BufInfo &rBuf)
{
    if (0 != rBuf.u4BufVA) {
        MY_LOGI("[freeRotatedQuickViewBuf] addr:0x%x\n", rBuf.u4BufVA); 
        free((void*)rBuf.u4BufVA); 
        rBuf.u4BufVA = 0; 
    }
    return true;     
}
