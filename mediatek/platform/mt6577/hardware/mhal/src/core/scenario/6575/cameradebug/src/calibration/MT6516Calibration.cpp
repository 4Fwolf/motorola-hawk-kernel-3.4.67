...
#define PREVIEW_TEST            (UINT32)0x1
#define CAPTURE_TEST            (UINT32)0x2
...
void svd_tool(unsigned int *src_addr, int nrow, int ncol, int num_term, int num_len, TBL_INFO_T rTableINFO, UINT8 a_u1Mode)
{
	...
	ACDK_LOGE("AcdkCalibration:: nrow= %d, ncol= %d\n", nrow, ncol);
	ncol = ncol*4;
...
/* this block outputs the singular values and their cummulative engergy */
	...
	runsum = 0.0;
	ACDK_LOGE("AcdkCalibration ::\n");
	ACDK_LOGE("eigen value, power \n");
	for(i = 1; i <= ncol; i++)
	{
		runsum += w[i];
		ACDK_LOGE("%7.4f %7.4f\n",w[i], runsum/sum);
...
/* this block check average difference */
	...
	ACDK_LOGE("AcdkCalibration::average difference (normalize to maxmun input element) = %f\n",avg_diff);
	...
	m = (int)((m+7)/8)+2 + 8;
	ACDK_LOGE("AcdkCalibration:: Total bytecnt = %d\n", m);
...
void lscReConstruct(int nrow, int ncol, unsigned char *input_buffer, unsigned short *output_buffer)
{
    ...
    readsize = 8; //lst 8 bytes is for hw table
    //num_term
...
void vlscHWTBLCAL(TBL_INFO_T info)
{
    ...
    unsigned int blk_info_0,blk_info_1;
    unsigned int *p_pgn;
    ...
    dim_x.grid_num=((blk_info_0&0xF0000000)>>28)+2;
    dim_x.di=(blk_info_0&0x0FFF0000)>>16;
    dim_x.dn=(blk_info_1&0x0FFF0000)>>16;
    ...
    dim_y.grid_num=((blk_info_0&0x0000F000)>>12)+2;
    dim_y.di=(blk_info_0&0x00000FFF);
    dim_y.dn=(blk_info_1&0x00000FFF);
...
//
/////////////////////////////////////////////////////////////////////////
UINT32 u4LSC_Integer_SQRT(unsigned int x)
{
...
//
/////////////////////////////////////////////////////////////////////////
MRESULT mrLSC_POLY_Ratio_Get(int d, int dmax, RATIO_POLY_T coef_t)
{
	float di,dni,ri;
	float coefpoly[6];
	int k;
...
//
/////////////////////////////////////////////////////////////////////////
MRESULT mrLSC_Calibrate(LSC_CALI_INFO_T cali_info, UINT8 a_u1Mode, UINT16 a_u2SVDTermNum)
{
	int i,j,k,bi,bj;
	...
	int wd=g_lsc_param.plane_wd; // raw domain width
	int ht=g_lsc_param.plane_ht; // raw domain height
	...
	dis_max=u4LSC_Integer_SQRT((((wd*wd)>>2)+((ht*ht)>>2))<<SQRT_NORMALIZE_BIT); // max distance to raw center: from (0,0) to (wd/2,ht/2), for sqrt calculation, right shift SQRT_NORMALIZE_BIT bit for accuracy
	...
	for(j = 0; j < n; j++)
	{
		for(i = 0; i < m; i++)
		{
			...
			if(ratio_poly_enable)
			{
				...
				x0=(wd>>1);y0=(ht>>1);
				dis_cur=u4LSC_Integer_SQRT(((kx-x0)*(kx-x0)+(ky-y0)*(ky-y0))<<SQRT_NORMALIZE_BIT);
...
//
/////////////////////////////////////////////////////////////////////////
MRESULT mrBPC_Calibrate(BPC_CAL_INI_PARAM_T cali_info)
{
