 ...
 ************************************************************************************************/
//
#define LOG_TAG "scenario/mHalVdo"
...
********************************************************************************/
MINT32
mHalCam::mHalCamVideoRecordStart(
    VOID *a_pInBuffer,
...
********************************************************************************/
MINT32
mHalCam::mHalCamVideoRecordStop(
    VOID *a_pInBuffer,
    MUINT32 a_u4InBufSize
)
{
    ...
    for (int i = 0; i < mVdoBusyFrmCnt; i++) {
        ...
        mpCameraIOObj->releasePreviewFrame(ICameraIO::ePREVIEW_VIDEO_PORT, &rBufInfo);                 
    }
#endif 
...
)
{
    ...
    mHalCam *const pmHalCam = reinterpret_cast<mHalCam*>(arg); 
    if (!pmHalCam ) {
...
********************************************************************************/
MVOID*
mHalCam::mHalCamVideoRecordThread(
)
{
...
*******************************************************************************/
MINT32 
mHalCam::postVideoFrame(VideoFrame const &rVdoFrame) 
{
    Mutex::Autolock _l(mVdoFrameLock); 
    MCAM_DBG1("[postVideoFrame] Index = %d\n", rVdoFrame.hwIndex); 
...
*******************************************************************************/
MINT32 
mHalCam::getVideoFrame(VideoFrame &rVdoFrame) 
{
    Mutex::Autolock _l(mVdoFrameLock);
    //
    if  ( ! mVdoFrameQueue.empty() ) {
        ...
        MCAM_DBG1("[getVideoFrame] Index = %d\n", rVdoFrame.hwIndex);     
    }
    else {
        MCAM_DBG1("[getVideoFrame] No video frame in \n"); 
        return -1; 
    }
    return 0;     
}
