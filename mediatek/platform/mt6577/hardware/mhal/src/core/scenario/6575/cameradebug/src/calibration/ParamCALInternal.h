...
#define CAL_ASSERT(x, str); \
    if (x) {printf("[Assert %s, %d]: %s", __FILE__, __LINE__, str);  XLOGE("[Assert %s, %d]: %s", __FILE__, __LINE__, str); while(1);} \
    else {} 
#else
#define CAL_ASSERT(x, str);
#endif
...
#define GRID_MAX 17 // if > 17 must modify vLSC_Calibration_INIT() lsc_hw_tbl_cal()...
#define GN_BIT 13
