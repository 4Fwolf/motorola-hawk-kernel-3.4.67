...
VOID AcdkCalibration:: vCaptureCallBack(VOID *a_pParam)
{
    ...
    if (prImgBufInfo->eImgType == RAW_TYPE)
    {
        if (m_pucRawBuf != NULL)
        {
            free(m_pucRawBuf);
        }
        m_u2RawImgWidth = prImgBufInfo->rRawBufInfo.rRawImgInfo.u2Width;
        m_u2RawImgHeight = prImgBufInfo->rRawBufInfo.rRawImgInfo.u2Height;
        MUINT8 uBytePerPixel = prImgBufInfo->rRawBufInfo.rRawImgInfo.uBytePerPixel;
        UINT32 u2ImgSize = prImgBufInfo->rRawBufInfo.u4ImgSize;
        MUINT8 *puImgBuf = prImgBufInfo->rRawBufInfo.pucRawBuf;
        m_eColorOrder = prImgBufInfo->rRawBufInfo.rRawImgInfo.eColorOrder;
        m_pucRawBuf = (MUINT8 *) malloc (m_u2RawImgWidth * m_u2RawImgHeight * 2 * sizeof(MUINT8));

        if ( prImgBufInfo->rRawBufInfo.bPacked == TRUE)
        {
            m_pAcdkImgToolObj->vUnPackRawImg(puImgBuf,m_pucRawBuf, m_u2RawImgWidth, m_u2RawImgHeight, 0, 2);
        }
        else
        {
            memcpy(m_pucRawBuf, prImgBufInfo->rRawBufInfo.pucRawBuf, u2ImgSize);
        }
        ...
        if (m_eRawFormat == ONE_BYTE_RAW)
        {
            ...
            {
                *puImgBuf++ = (*pu2ImgBuf++) >> (prImgBufInfo->rRawBufInfo.rRawImgInfo.uBitDepth - 8);
            }
        }
    }
...
/////////////////////////////////////////////////////////////////////////
MRESULT AcdkCalibration::init(AcdkCCTCtrl *a_pAcdkCCTCtrlObj)
{
    ACDK_LOGD(" Init AcdkCalibration Tool\n");

    m_pAcdkCCTCtrlObj = a_pAcdkCCTCtrlObj;

    if (0 == m_pAcdkCCTCtrlObj) {
...
//!
/////////////////////////////////////////////////////////////////////////
MRESULT AcdkCalibration::mrAnalyzeRAWImage(MINT32 a_i4PreCap, MINT32 a_i4SequenceNo)
{
    ...
    if (m_pAcdkImgToolObj == NULL || m_pAcdkCCTCtrlObj == NULL)
    {
    ...
    {
        // after capture image done, the raw image info will be got
        m_u4CapMode = 0; //preview mode
        mrRet = m_pAcdkCCTCtrlObj->takePicture(PREVIEW_MODE,
                                                                             RAW_TYPE, 
                                                                             vCapCb); 
    ...
    else if (a_i4PreCap == 1)    //Capture Test
    {
        ...
        mrRet = m_pAcdkCCTCtrlObj->takePicture(CAPTURE_MODE, 
                                                                       RAW_TYPE, 
                                                                       vCapCb); 
    ...
    mrRet = m_pAcdkCCTCtrlObj->startPreview(NULL);
                                        ...
                                        ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    MUINT32 u4AFInfo = 0;
    MUINT32 u4AWBInfo = 0;
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ENABLE_INFO, NULL, 0, (MUINT8 *)&u4AEInfo, sizeof(MUINT32), &u4RetLen);
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AF_GET_ENABLE_INFO, NULL, 0, (MUINT8 *)&u4AFInfo, sizeof(MUINT32), &u4RetLen);
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AWB_GET_AUTO_RUN_INFO, NULL, 0, (MUINT8 *)&u4AWBInfo, sizeof(MUINT32), &u4RetLen);
    ACDK_LOGD("[Backup 3A Enable Info] AE = %d, AF = %d, AWB = %d\n", u4AEInfo, u4AFInfo, u4AWBInfo);

    // Backup OB level
    MUINT32 u4OBBackup = 0;
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ISP_OB, NULL, 0, (MUINT8 *)&u4OBBackup, sizeof(MUINT32), &u4RetLen);
    ACDK_LOGD("[Backup ISP OB Level] = %d\n", u4OBBackup);

    // Backup ISP RAW gain
    MUINT32 u4RawGainBackup = 0;
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ISP_RAW_GAIN, NULL, 0, (MUINT8 *)&u4RawGainBackup, sizeof(MUINT32), &u4RetLen);
    ACDK_LOGD(" [Backup ISP RAW Gain] = %d\n", u4RawGainBackup);

    // Disable 3A
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_DISABLE, NULL, 0, NULL, 0, &u4RetLen);
    MINT32 MFPos = 0;
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_MF_OPERATION, (MUINT8 *)&MFPos, sizeof(MFPos), NULL, 0, &u4RetLen); // MF
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AF_DISABLE, NULL, 0, NULL, 0, &u4RetLen);
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AWB_DISABLE_AUTO_RUN, NULL, 0, NULL, 0, &u4RetLen);
    ACDK_LOGD("[Disable 3A]\n");

    // Lock exposure setting
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_LOCK_EXPOSURE_SETTING, NULL, 0, NULL, 0, &u4RetLen);
    ACDK_LOGD("[Lock Exposure Setting]\n");

    // Set operation mode to Pure RAW
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_FEATURE_SET_OPERATION_MODE, (MUINT8 *)ISP_OPER_MODE_PURE_RAW, 0, NULL, 0, &u4RetLen);
    ...
    for (MINT32 i4Exp = a_i4ExpStart; i4Exp <= a_i4ExpEnd; i4Exp += a_i4ExpInterval)
    {
        ...
        {
            m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_LINE, (MUINT8 *)&i4Exp, sizeof(MINT32), NULL, 0, &u4RetLen);
            ACDK_LOGD("[Set Exposure Line] = %d\n", i4Exp);
        }
        else // exposure time
        {
            m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&i4Exp, sizeof(MINT32), NULL, 0, &u4RetLen);
            ACDK_LOGD("[Set Exposure Time] = %d\n", i4Exp);
        }

        // Set sensor gain
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_GAIN, (MUINT8 *)&a_i4Gain, sizeof(MINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD("[Set Sensor Gain] = %d\n", a_i4Gain);

        // Set ISP OB level to 0
        MUINT32 u4ISPOB = 0;
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_OB, (MUINT8 *)&u4ISPOB, sizeof(MUINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD("[Set ISP OB Level] = %d\n", u4ISPOB);

        // Set ISP RAW gain = 128
        MUINT32 u4ISPRawGain = 128;
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_RAW_GAIN, (MUINT8 *)&u4ISPRawGain, sizeof(MUINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD(" [Set ISP RAW Gain] = %d\n", u4ISPRawGain);

        // RAW image analysis
        mrRet = mrAnalyzeRAWImage(a_i4PreCap, i4SequenceNo);

        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ISP_RAW_GAIN, NULL, 0, (MUINT8 *)&u4ISPRawGain, sizeof(MUINT32), &u4RetLen);
        ACDK_LOGD(" [Get ISP RAW Gain] = %d\n", u4ISPRawGain);

        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ISP_OB, NULL, 0, (MUINT8 *)&u4ISPOB, sizeof(MUINT32), &u4RetLen);
...
Exit:
    ...
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_FEATURE_SET_OPERATION_MODE, (MUINT8 *)ISP_OPER_MODE_META, 0, NULL, 0, &u4RetLen);
    ACDK_LOGD("[Meta Mode]\n");

    // Unlock exposure setting
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_UNLOCK_EXPOSURE_SETTING, NULL, 0, NULL, 0, &u4RetLen);
    ACDK_LOGD("[Unlock Exposure Setting]\n");

    // Restore OB level
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_OB, (MUINT8 *)&u4OBBackup, sizeof(MUINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD("[Resotre ISP OB Level] = %d\n", u4OBBackup);

    // Restore ISP RAW gain
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_RAW_GAIN, (MUINT8 *)&u4RawGainBackup, sizeof(MUINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD(" [Resotre ISP RAW Gain] = %d\n", u4RawGainBackup);

   // Restore 3A
   if (u4AEInfo)
   {
       m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_ENABLE, NULL, 0, NULL, 0, &u4RetLen);
   }

   if (u4AFInfo)
   {
       m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AF_ENABLE, NULL, 0, NULL, 0, &u4RetLen);
   }

   if (u4AWBInfo)
   {
       m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AWB_ENABLE_AUTO_RUN, NULL, 0, NULL, 0, &u4RetLen);
   }
                                         ...
                                         ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    {
        // Set exposure time
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&a_i4ExpTime, sizeof(MINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD("[Set Exposure Time] = %d\n", a_i4ExpTime);

        // Set sensor gain
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_GAIN, (MUINT8 *)&i4Gain, sizeof(MINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD("[Set Sensor Gain] = %d\n", i4Gain);
                                              ...
                                              ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    for (MINT32 i4GainTableIndex = 0; i4GainTableIndex < a_i4GainTableSize; i4GainTableIndex++)
    {
        ...
        // Set sensor gain
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_GAIN, (MUINT8 *)&a_pi4GainTable[i4GainTableIndex], sizeof(MINT32), NULL, 0, &u4RetLen);
                                     ...
                                     ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    MUINT32 u4OBISPRAWGain = (MUINT32)((((DOUBLE)1023)/(1023-a_i4OBLevel))*128+0.5);
    ...
    // Set exposure time
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&i4DefaultExpTInUS, sizeof(MINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD("[Set Exposure Time] = %d\n", i4DefaultExpTInUS);
    ...
    // Set ISP OB level
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_OB, (MUINT8 *)&a_i4OBLevel, sizeof(MINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD("[Set ISP OB Level] = %d\n", a_i4OBLevel);

    // Set ISP RAW gain
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_ISP_RAW_GAIN, (MUINT8 *)&u4OBISPRAWGain, sizeof(MUINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD(" [Set ISP RAW Gain] = %d\n", u4OBISPRAWGain);

    // RAW image analysis
    mrRet = mrAnalyzeRAWImage(a_i4PreCap, 0);
    ...
    // Set exposure time
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&i4TargetExpTInUs_LA, sizeof(MINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD("[Set Exposure Time] = %d\n", i4TargetExpTInUs_LA);
    ...
    // RAW image analysis
    mrRet = mrAnalyzeRAWImage(a_i4PreCap, 1);
    ...
    // Set exposure time
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&i4TargetExpTInUs_UA, sizeof(MINT32), NULL, 0, &u4RetLen);
    ACDK_LOGD("[Set Exposure Time] = %d\n", i4TargetExpTInUs_UA);
    ...
    // RAW image analysis
    mrRet = mrAnalyzeRAWImage(a_i4PreCap, 2);
                                                    ...
                                                    ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    {
        // Set exposure time
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME, (MUINT8 *)&ExpT, sizeof(MINT32), NULL, 0, &u4RetLen);
        ACDK_LOGD("[Set Exposure Time] = %d\n", ExpT);
...
MRESULT AcdkCalibration::mrCalLinearityC(MINT32 a_i4Expline, MINT32 a_i4ExpTimes,MINT32 a_i4Iris, MINT32 a_i4PreCap, RAWAnalyzeResult *a_pResult)
{
   ...
   m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AWB_DISABLE_AUTO_RUN, NULL, 0, NULL , 0, &u4RetLen);
    ...
    rAEExpPara.uFlareValue= 0;
    rAEExpPara.uFlareGain = 128;
    rAEExpPara.uFrameRate = 30;
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AE_APPLY_EXPO_INFO,  (MUINT8 *)&rAEExpPara,  sizeof(ACDK_AE_MODE_CFG_T), NULL, 0, &u4RetLen);
    ...
    for (INT16 i2Times = 0; i2Times <= a_i4ExpTimes; i2Times ++)
    {
        ...
        rAEExpPara.uFrameRate = 1000000/ rAEExpPara.u4Eposuretime;
        if (rAEExpPara.uFrameRate > 30)
        {
             rAEExpPara.uFrameRate = 30;
        }
...
extern MRESULT mrBPC_Calibrate(BPC_CAL_INI_PARAM_T cali_info);
...
MRESULT AcdkCalibration::mrCalLenShading(INT32 a_i4XINIBorder, INT32 a_i4XENDBorder, INT32 a_i4YINIBorder, INT32 a_i4YENDBorder,  UINT16 a_u2AttnRatio, UINT32 a_u4Index, INT32 a_i4PreCap, UINT8 a_u1FixShadingIndex)
{
    ...
    ACDK_LOGD(" a_i4XINIBorder %d  a_i4XENDBorder %d  a_i4YINIBorder %d  a_i4YENDBorder %d   a_u2AttnRatio %d  a_u4Index %d  a_i4PreCap %d  a_u1FixShadingIndex %d \n",
...
//   MINT32 u4CaliMode = 0;
   ...
   //store AE status
   mrRet = m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_GET_ENABLE_INFO,
                                                        NULL,
                                                        0,
                                                        (UINT8 *)&u4AEEnable,
                                                        sizeof(u4AEEnable),
    ...
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AE_GET_AUTO_EXPO_PARA,
                                                        (UINT8 *)&rAEExpPara,
    ...
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AE_GET_BAND,
                                                        NULL,
                                                        0,
    ...
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AE_SET_METERING_MODE, (UINT8 *)&i4AEMeteringMode, sizeof(INT32), NULL, 0, &u4RetLen);
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_AE_ENABLE, NULL, 0, NULL, 0, &u4RetLen);
    ...
    ACDK_LOGE("[AcdkCalibration::mrCalLenShading]"
     ...
     m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AE_APPLY_EXPO_INFO,  (UINT8 *)&rAEExpPara,  sizeof(ACDK_AE_MODE_CFG_T), NULL, 0, &u4RetLen);

    //disable AWB
    //store current AWB status
    mrRet = m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_AWB_GET_AUTO_RUN_INFO,
                                                        NULL,
                                                        0,
                                                        (UINT8 *)&i4AWBEnable,
                                                        sizeof(INT32),
    ...
    if ((UINT32)a_i4PreCap & PREVIEW_TEST)   //preview Test
    {
        ...
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_GET_SHADING_ON_OFF,
        ...
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_SET_SHADING_ON_OFF,
        ...
        m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_GET_SHADING_PARA,
                                                       (UINT8*) &uCompMode,
                                                       sizeof(UINT8),
                                                       (UINT8*)&ACDK_Output,
        ...
        mrLSC_Calibrate(g_lsc_cali_info, 0, u2SVDTermNum);//preview
        ...
        UINT16 uShadingTableGain[MAX_SHADING_Preview_SIZE]={0}; // buffer size > rLSCCaliINIParam.i4GridXNUM*rLSCCaliINIParam.i4GridYNUM*4
        UINT32 uShadingTableCoef[MAX_SHADING_Preview_SIZE]={0};
        ...
        //calculate hw table
        memcpy(&(rTableINTO.reg_info0), uShadingTableSVD,4);
        memcpy(&(rTableINTO.reg_info1), uShadingTableSVD+4,4);
        ...
        shadingTablePolyCoef.Length = MAX_SHADING_Preview_SIZE;
        ...
        if (a_u1FixShadingIndex == 0)
        {
            ...
            m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_SET_SHADING_TABLE_V3,
                                                  (UINT8 *)&shadingTable,
            ...
            m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_SET_SHADING_TABLE_POLYCOEF,
    ...
    if ((UINT32)a_i4PreCap & CAPTURE_TEST)    //Capture Test
    {
        ...
        vLSC_PARAM_INIT(rLSCCaliINIParam);
        vLSC_Calibration_INIT(m_pucRawBuf);
        mrLSC_Calibrate(g_lsc_cali_info, 1, u2SVDTermNum);//capture
        ...
        //!Apply parameter ISP
        UINT8 uShadingTableSVD[MAX_SVD_SHADING_SIZE]={0};
        UINT16 uShadingTableGain[MAX_SHADING_Capture_SIZE]={0};
        UINT32 uShadingTableCoef[MAX_SHADING_Capture_SIZE]={0};
        ...
        ACDK_CCT_TABLE_SET_STRUCT  shadingTablePolyCoef;
        memset (&shadingTablePolyCoef, 0, sizeof(ACDK_CCT_TABLE_SET_STRUCT));
        shadingTablePolyCoef.Length = MAX_SHADING_Capture_SIZE;
        shadingTablePolyCoef.Offset = 0;
        shadingTablePolyCoef.pBuffer = (UINT32 *)uShadingTableCoef;//g_lsc_cali_info.tbl_info.dst_tbl_addr;
...
Exit:
    ...
    //disable Calibration
    //u4CaliMode = 0;
    //m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_OP_SET_CALI_MODE, (UINT8*)&u4CaliMode, sizeof(UINT32), NULL, 0,  &u4RetLen);
    ...
    m_pAcdkCCTCtrlObj->sendcommand(ACDK_CCT_V2_OP_ISP_SET_SHADING_INDEX,
                                                        (UINT8*)&index,
                                                        sizeof(UINT8),
                                                        NULL,
                                                        0,
    ...
    ACDK_LOGD(" Lens Shading Calibration Finish !!\n");
    return mrRet;
}
