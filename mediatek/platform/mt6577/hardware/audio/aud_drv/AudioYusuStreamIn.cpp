...
static const char * streamin_process = "/sdcard/mtklog/audio_dump/StreamIn_ProcessOut_Dump.pcm";
static const char * streamin_process_propty = "streamin.pcm.process.dump";
...
bool AudioYusuStreamIn::CanRecordFM(int mode)
{
   ...
   if( (mode == android_audio_legacy::AudioSystem::MODE_NORMAL) && (mHw->GetAnalogLineinEnable() && mHw->GetRecordFM ()) && (mInputSource == AUDIO_SOURCE_FM))
   {
      return true;
   }
   else
...
void AudioYusuStreamIn::SetHdrecordingMode(int mode , bool *bStereoRecOn)
{
    if( mode == android_audio_legacy::AudioSystem::MODE_IN_CALL ||
       ...
       (mode == android_audio_legacy::AudioSystem::MODE_NORMAL && 
        mHw->getInputDevice(mDevice) == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) ){
...
{
   bool ret = true, BT_In_VoiceRecog = false;
   int mode;
   m_no_data_count = 0;
   ...
   mHw->getMode(&mode);
   if((mSetSphEnh == false)&&(mode != android_audio_legacy::AudioSystem::MODE_IN_CALL)){//ALPS00358204, Can't set speech mode without speech off
      // VOICE_RECOGNITION, must disable sph enhancement
      // Remember to turn on again if we turn off it.
      int Device= mHw->getInputDevice(mDevice);
      ALOGD("+RecOpen, mSetSphEnh=false mDevice = %x",Device);
      if(Device == AUDIO_DEVICE_IN_WIRED_HEADSET)
      {
         ALOGD("+RecOpen, LAD_SetInputSource LADIN_HEADSET, LAD_SetOutputDevice LADOUT_HEADSET");
         pLad->LAD_SetSpeechMode((ENUM_Speech_Mode)SPH_MODE_BT_CORDLESS);
         pLad->LAD_SetInputSource(LADIN_Microphone2);
	  ...
	  {
	  	ALOGD("+RecOpen, LAD_SetInputSource LADIN_BTIn, LAD_SetOutputDevice LADOUT_BTOut");
        pLad->LAD_SetSpeechMode((ENUM_Speech_Mode)SPH_MODE_BT_CARKIT);
      	pLad->LAD_SetInputSource (LADIN_BTIn);
        pLad->LAD_SetMicrophoneVolume(0xffffffe0);
		pLad->LAD_SetOutputDevice (LADOUT_BTOut);
		BT_In_VoiceRecog = true;
	  }
	  else
	  {
        pLad->LAD_SetSpeechMode((ENUM_Speech_Mode)SPH_MODE_BT_CORDLESS);
   ...
   if ( (mSampleRateModem != mSampleRate) || mChNum != 1 && mFormat != android_audio_legacy::AudioSystem::VM_FMT ) {
      ...
      //Release src handle when it was configured before.
      if(pSrcHdl!= NULL){
          ALOGD("Reset pSrcHdl = %p",pSrcHdl);
      ...
      if(pSrcHdl == NULL){
          ...
          pSrcHdl = BLI_Open( mSampleRateModem, mRealChNum, mSampleRate, mChNum, pSrcBuf,NULL);
      }
      else{
...
ssize_t AudioYusuStreamIn::read(void* buffer, ssize_t bytes)
{
   ...
   property_get("streamin.pcm.process.dump", value2, "0");
   int bflag2=atoi(value2);
...
#endif
        process_frames(buf,bytes);

        dumpPCMData(streamin_process,buf,bytes,streamin_process_propty);
...
bool AudioYusuStreamIn::SetInputMute(bool bMicMute)
{
...
bool AudioYusuStreamIn::vGetHdSpecifictModeInfo(uint8 *modeIndex, bool *bStereoRecOn, int AssignedModeIndex)
{
    ...
    *bStereoRecOn = false;
    ...
    if (hdRecordSceneTable.scene_table[u4SceneIdx][HD_REC_DEVICE_SOURCE_BT_EARPHONE] != 0xFF
        && u4CurrentDevice == AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET)
    {
...
status_t AudioYusuStreamIn::addAudioEffect(effect_handle_t effect)
{
    int status;
    ...
    if (memcmp(&desc.type, FX_IID_AEC, sizeof(effect_uuid_t)) == 0) {
        ...
        mAPPS->in_configure_reverse(mChNum,mSampleRate);
    }
exit:
...
status_t AudioYusuStreamIn::removeAudioEffect(effect_handle_t effect)
{
    int i;
    int status = -EINVAL;
    effect_descriptor_t desc;
...
ssize_t	AudioYusuStreamIn::process_frames(void * buffer,ssize_t bytes)
{

    if((mAPPS==NULL)||(mAPPS->num_preprocessors==0)){
        return 0;
    }
    else{
        ssize_t frames_wr = 0;
        ...
        //ALOGD("process_frames(): %d bytes, %d frames, proc_buf_frames=%d", bytes,frames,mAPPS->proc_buf_frames);
        ...
        if ((mAPPS->proc_buf_size < (size_t)needframes) || (mAPPS->proc_buf_in==NULL)) {
            ...
            mAPPS->proc_buf_in = (int16_t *)realloc(mAPPS->proc_buf_in, mAPPS->proc_buf_size*mChNum*sizeof(int16_t));
            ...
            ALOGD("process_frames(): proc_buf_in %p extended to %d bytes",mAPPS->proc_buf_in, mAPPS->proc_buf_size*sizeof(int16_t)*mChNum);
        }

        memcpy(mAPPS->proc_buf_in + mAPPS->proc_buf_frames * mChNum, buffer, bytes);
        ...
        while (frames_wr < frames) {
          ...
          /* in_buf.frameCount and out_buf.frameCount indicate respectively
            ...
            out_buf.s16 = (int16_t *)mAPPS->proc_buf_out + frames_wr * mChNum;
        ...
        /* if not enough frames were passed to process(), read more and retry. */
            if (out_buf.frameCount == 0) {
                ALOGW("No frames produced by preproc, something wrong here!!!");
                break;
            }
