...
void AudioAfe::Afe_UL_Start(AFE_MODE OpenMode)
{
   ...
   if(OpenMode == AFE_MODE_ADC)
   {
      ...
      SetAfeReg(AFE_UL_SRC_0,0x0000,0x0400);  //disable UL IIR DC REMOVAL
...
void AudioAfe::Afe_DL_Stop(AFE_MODE OpenMode)
{
   ...
   int rec_on = false;
   int vt_on = false;
   int A2M_speech_on = false;
   int mStreamMode = android_audio_legacy::AudioSystem::MODE_NORMAL;
   ...
   if(OpenMode == AFE_MODE_DAC)
   {
      ...
      if(mAudioHardware != NULL){
         ...
         A2M_speech_on  = mAudioHardware->Get_A2M_SpeechOn_Status();       
         mAudioHardware->getMode(&mStreamMode);
      }

      ALOGD("Afe_DL_Stop, sph_on:%d, bgs_on:%d, rec_on:%d, vt_on:%d, A2M_speech_on:%d \n", sph_on, bgs_on, rec_on, vt_on, A2M_speech_on);
      //ALPS00437462: add to avoid disabling audio hw while MD speech on ACK not back yet(bSpeechFlag not on yet)
      if( (A2M_speech_on==false) && (sph_on==false) && (bgs_on==false) && (vt_on==false) )
      {
         // Close device
   ...
   else if(OpenMode == AFE_MODE_DAI)
   {
       ...
       if(mAudioHardware != NULL)
       {
           if(!rec_on || mAudioHardware->pLad->m_InputSrc!=LADIN_BTIn)
           {
...
void AudioAfe::Afe_Set_Stereo(void)
{
   ...
   if(mAudioHardware != NULL)
   {
     ...
     if((mAudioHardware->mEnableStereoOutput==false)//For CMCC speaker mono/stereo
        ...
        ||(i4CurrentDevice == AUDIO_DEVICE_OUT_SPEAKER)
        ||(i4CurrentDevice == AUDIO_DEVICE_OUT_EARPIECE)
#endif
     )
     {
