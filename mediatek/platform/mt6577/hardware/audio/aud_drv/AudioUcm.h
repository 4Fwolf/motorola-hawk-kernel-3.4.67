...
public:
	 ...
	 ~AudioYusuUserCaseManager();
		 ...
		 virtual uint64_t volIndexToAnalogVol(audio_devices_t device,int index=0);

private:
	     static audio_devices_t getDeviceForVolume(audio_devices_t device);
		 uint32_t computeBtVol(audio_devices_t device);
		 ...
		 void exchangeChannelGain(uint32_t * value);
