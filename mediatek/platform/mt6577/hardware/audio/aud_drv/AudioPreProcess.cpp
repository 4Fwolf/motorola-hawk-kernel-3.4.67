...
status_t AudioPreProcess::set(AudioYusuHardware *hw)
{
    Mutex::Autolock lock(mLock);
    mHw = hw;
    return NO_ERROR;
...
void AudioPreProcess::remove_echo_reference(struct echo_reference_itfe *reference)
{
    ...
    mHw->mStreamHandler->mOutput[0]->remove_echo_reference(reference);
}
                                                        ...
                                                        uint32_t channel_count,uint32_t sampling_rate)
{
    ...
    /* echo reference is taken from the low latency output stream used
         * for voice use cases */
    if (!mHw->mStreamHandler->mOutput[0]->IsStandby()) {
        uint32_t wr_channel_count = mHw->mStreamHandler->mOutput[0]->GetChannelInfo();
        uint32_t wr_sampling_rate = mHw->mStreamHandler->mOutput[0]->GetSampleRateInfo();
...
void AudioPreProcess::add_echo_reference(struct echo_reference_itfe *reference)
{
    ...
    mHw->mStreamHandler->mOutput[0]->add_echo_reference(reference);
}
...
int AudioPreProcess::in_configure_reverse(uint32_t channel_count,uint32_t sampling_rate)
{
    ...
    if (num_preprocessors > 0) {
        ...
        config.outputCfg.samplingRate = mHw->mStreamHandler->mOutput[0]->GetSampleRateInfo();
