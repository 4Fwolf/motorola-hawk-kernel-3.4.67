...
#ifndef ONLY_USING_SPK_AUDIOBUFFER_INCALL
...
uint64_t AudioYusuUserCaseManager::volIndexToAnalogVol(audio_devices_t device,int index)
{
    ALOGV("volIndexToAnalogVol device 0x%x",device);
	Mutex::Autolock _l(mLock);

    uint64_t volume = 0xFFFFFFFFFFFFFFFF;
    if(mPhoneState == AudioSystem::MODE_IN_CALL){
        if(AudioSystem::isBluetoothScoDevice((AudioSystem::audio_devices)device))
        {
            uint32_t vol = computeBtVol(device);
            volume = vol;
            volume = volume<<32 | vol;
            return volume;
        }
        ...
        STREAM_SPEECH_GAIN_CONTROL_STRUCT *pSpeechWB = &pGainTable->speechGainWB;
            ...
            device &(AudioSystem::DEVICE_OUT_WIRED_HEADPHONE | AudioSystem::DEVICE_OUT_WIRED_HEADSET))
        {
...
#ifdef ALL_USING_VOICEBUFFER_INCALL
#ifdef ONLY_USING_SPK_AUDIOBUFFER_INCALL  // open to use SPK audio buffer
           exchangeChannelGain(&pgaSpeaker);// speak uses audio buffer, but metal indicates voice buffer
#endif
#endif
           uint32_t pgaHP = pSpeech->speechGain[GAIN_OUTPUT_HEADSET][voiceIndex].u32Gain_PGA_Amp;
           uint32_t pgaGain = ((pgaSpeaker >> 15)<<15) | (pgaHP & 0x7FFF); // combile hp gain 
           ...
           uint32_t volNB =( (pgaGain >> 10) << 12) | (digital<<4) | mask; //allocate space for digital and mask 
           ALOGV("volIndexToAnalogVol IncallNB level %d PGASpeaker 0x%x PAGHp 0x%x, dig=0x%x, mask =0x%x,volNB =0x%x",
               voiceIndex,pgaSpeaker,pgaHP,digital,mask,volNB);
           //2. compute WB gain
           uint32_t pgaSpeakerWB = pSpeechWB->speechGain[GAIN_OUTPUT_SPEAKER][voiceIndex].u32Gain_PGA_Amp;
           uint8_t digitalWB = pSpeechWB->speechGain[GAIN_OUTPUT_SPEAKER][voiceIndex].u8Gain_digital;
           //WB do not has amp gain, so use NB amp gain
           uint32_t pgaGainWB = ((pgaSpeakerWB >>24)<<24)|((pgaGain<<8)>>8);
           uint32_t volWB =( (pgaGainWB >> 10) << 12) | (digitalWB<<4) | mask; //allocate space for digital and mask 
           ALOGV("volIndexToAnalogVol IncallWB pgaSpeakerWB 0x%x  digWB=0x%x, volWB =0x%x",
           pgaSpeaker,digitalWB,volWB);
           //3. combine NB & WB
           volume = volNB;
           volume = volume<<32 | volWB;
           ALOGD("volIndexToAnalogVol Both Devices vol 0x%x",volume);
        }
        else
        {
            int voiceIndex = streamDesc.getVolumeIndex(getDeviceForVolume(device));
            ...
            {
                exchangeChannelGain(&pga);// speak uses audio buffer, but metal indicates voice buffer
            }
#endif
#endif
            //  Analog, INPUT1,  INPUT2 , SPEAKER ,   HP  ,   Digital,      mask
            ...
            int mask = getAmpValidMask(mPhoneState, device);
            uint32_t volNB = ((pga >> 10) << 12) | (digital<<4) | mask; //allocate space for digital and mask 
            ALOGV("volIndexToAnalogVol Incall gainOutputDevice %d,index %d, pga 0x%x , dig=0x%x,mask =0x%x,  volNB =0x%x",
                   gainOutputDevice,voiceIndex,pga,digital,mask,volNB);
            //2. compute WB
             uint32_t pgaWB = pSpeechWB->speechGain[gainOutputDevice][voiceIndex].u32Gain_PGA_Amp;
             uint32_t pgaAMPWB = ((pgaWB>>24)<<24) | ((pga<<8)>>8);
             uint8_t digitalWB = pSpeechWB->speechGain[gainOutputDevice][voiceIndex].u8Gain_digital;
             uint32_t volWB = ((pgaAMPWB >> 10) << 12) | (digitalWB<<4) | mask; //allocate space for digital and mask 
             ALOGV("volIndexToAnalogVol IncallWB pgaWB 0x%x  digWB=0x%x, volWB =0x%x",
             pgaWB,digitalWB,volWB);
             //3. combine NB & WB;
             volume = volNB;
             volume = volume<<32 | volWB;
             ALOGD("volIndexToAnalogVol  singal Device vol 0x%x",volume);
        }
        return volume;
    }
    ...
    if(stream > AudioSystem::DEFAULT)
    {
            ...
            device &(AudioSystem::DEVICE_OUT_WIRED_HEADPHONE | AudioSystem::DEVICE_OUT_WIRED_HEADSET))
        {
           ...
           uint32_t pga = ((pgaSpeaker >> 15)<<15) | (pgaHP & 0x7FFF); // combile hp gain 
           int mask = getAmpValidMask(mPhoneState, device);
           uint32_t vol = ((pga >>4)<<4) | mask; //use resolved bits to store valid mask.
           ALOGV("volIndexToAnalogVol  index %d PGASpeaker 0x%x PAGHp 0x%x, mask =0x%x, vol =0x%x",
           streameIndex,pgaSpeaker,pgaHP,mask,vol);
           volume = vol;
           volume = volume<< 32 | 0xFFFFFFFF;
        }
        else
        {
            int streameIndex = streamDesc.getVolumeIndex(getDeviceForVolume(device));
...
void AudioYusuUserCaseManager::exchangeChannelGain(uint32_t * value)
{
    //  Analog,   INPUT1,  INPUT2 , SPEAKER ,     HP  ,   resolved,      mask
    //xxxxxxxx,     xx,         xx,      xxxxx,       xxxxx,  xxxxxxxxxx  (32 bits)
    uint32_t temp = *value;
    uint32_t input1= temp >>22 & 0x3;
    uint32_t input2 = temp >>20 & 0x3;
    *value  = ((temp >>24)<<24) | (input2<<22) |(input1<<20) | ((temp<<12)>>12);
    ALOGD("exchangeChannelGain old 0x%x, new 0x%x",temp,*value);
}

uint32_t AudioYusuUserCaseManager::computeBtVol(audio_devices_t device)
{
    UCMStreamDescriptor &streamDesc = mStreams[AudioSystem::BLUETOOTH_SCO];
    char value[PROPERTY_VALUE_MAX];
    property_get("af.bt.vol.support", value, "1");
    int support = atoi(value);
    int voiceIndex = BLUETOOTHSCO_GAIN_TABLE_LEVEL -1;
    if(!support)
    {
        voiceIndex = streamDesc.getVolumeIndex(getDeviceForVolume(device));
    }
    int gainOutputDevice = getGainOutputDevice(device); //use headset
    STREAM_GAIN_CONTROL_STRUCT *pStream = &pGainTable->voiceCall + AudioSystem::BLUETOOTH_SCO;
    int pga = 0;
    //  Analog, INPUT1,  INPUT2 , SPEAKER ,   HP  ,   Digital,      mask
    //xxxxxx,     xx,         xx,      xxxxx,       xxxxx,  xxxxxxxx, xxxx   (32 bits)
    //use BLUETOOTH_SCO headset
    uint8_t digital = pStream->streamGain[gainOutputDevice][voiceIndex].u8Gain_digital;
    int mask = 0;
    uint32_t vol = ((pga >> 10) << 12) | (digital<<4) | mask; //allocate space for digital and mask 
     ALOGV("computeBtVol gainOutputDevice %d,index %d, pga 0x%x , dig=0x%x,mask =0x%x,  vol =0x%x",
    ...
    {   STREAM_GAIN_CONTROL_STRUCT *pStreamGainTable = &(pGainTable->voiceCall)+k;
        for(int i=0; i < NUM_GAIN_OUTPUT_DEVICES; i++){
            for(int j=0 ; j < GAIN_TABLE_LEVEL; j++){
                ...
                ALOGV("level = %d gain = 0x%x gain = 0x%x",
    ...
    for(int i=0 ; i < NUM_OF_SPEECHSGAINS; i++){
         for(int j=0 ; j < GAIN_TABLE_LEVEL; j++){
             AUDIO_GAIN_CONTROL_STRUCT *ptemp = &(pSpeechGainTable->speechGain[i][j]);
               ALOGV("level = %d digiGain = 0x%x pgaGain = 0x%x",
                   j,ptemp->u8Gain_digital,ptemp->u32Gain_PGA_Amp);
         }
     }
}
