...
{
    if(device == AUDIO_DEVICE_OUT_BLUETOOTH_SCO || device == AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET ||
...
status_t AudioYusuStreamOut::getNextWriteTimestamp(int64_t *timestamp)
{
    return INVALID_OPERATION;
}

status_t AudioYusuStreamOut::setCallBack(stream_callback_t callback, void *cookie)
{
    //TODO : new for KK
    return INVALID_OPERATION;
}

status_t AudioYusuStreamOut::getPresentationPosition(uint64_t *frames, struct timespec *timestamp)
{

    ALOGV("%s()",__FUNCTION__);
    *frames = mPresentedBytes / (uint64_t) (channels_t() * (format() == AUDIO_FORMAT_PCM_8_BIT ? 1 : 2));
    ...
    uint32_t *pRate)
{
    ...
    ALOGD("HwBufferSize = %u, Format = %d, ChannelCount=%u, SampleRate=%u"
    ...
    if(mAudioCompFlt)
    {
        mAudioCompFlt->init(AUDIO_COMPENSATION_FLT_MODE,bufferSize());
    }
#endif
    ...
    {
        mHeadphoneCompFlt->init(HEADPHONE_COMPENSATION_FLT_MODE,bufferSize());
    }
#endif
...
size_t AudioYusuStreamOut::writePcmDataToAudioHw(void* buffer, size_t bytes)
{
    ...
    if(mAudioHardware->GetSpeakerStatus()&& modeApplyCompensation(mStreamMode))
    {
        if (mAudioCompFlt)
        {
        ...
        {
            outbuffer=inbuffer; //in case acf instance is not created
        }

        if(mHeadphoneCompFlt && mHeadphoneCompFlt->isStarted()) mHeadphoneCompFlt->stop();

        dumpPcmData(streamout_acf, outbuffer, bytes, streamout_acf_propty);
    }
    else if(!mAudioHardware->GetSpeakerStatus() && (mAudioHardware->GetHDMIAudioStatus()==false))
    {
        if (mHeadphoneCompFlt)
        {
        ...
        {
            outbuffer=inbuffer; //in case hcf instance is not created
        }

        if(mAudioCompFlt && mAudioCompFlt->isStarted()) mAudioCompFlt->stop();
...
//        ALOGV("writePcmDataToAudioHw echo_reference %p",echo_reference);
        struct echo_reference_buffer b;
        b.raw = (void *)buffer;
        b.frame_count = bytes/sizeof(int16_t)/mAttribute->mChannelCount;

       get_playback_delay(b.frame_count, &b);
       echo_reference->write(echo_reference, &b);
    }
    EffectMutexUnlock();
...
ssize_t AudioYusuStreamOut::write(const void* buffer, size_t bytes)
{
    ...
    if(mStandby)
    {
        ...
        mStandby = false;
        EffectMutexLock();
        if (mAudioHardware->mStreamHandler->echo_reference != NULL)
        {
            echo_reference = mAudioHardware->mStreamHandler->echo_reference;
            ALOGD("write echo_reference %p",echo_reference);
        }
        EffectMutexUnlock();
    ...
    if(mStreamMode == android_audio_legacy::AudioSystem::MODE_IN_CALL)
    {
        ...
        writePcmDataToModem(outbuffer,bytes);
            ...
            mAudioHardware->GetCurrentDevice () == AUDIO_DEVICE_OUT_BLUETOOTH_SCO)
	#else //ccc skype
...
status_t AudioYusuStreamOut::standby()
{
    ...
    if(mStandby == false)  // stop this stream afe
    {
        ...
        EffectMutexLock();
        if (echo_reference != NULL) {
            echo_reference->write(echo_reference, NULL);
            echo_reference = NULL;
        }
        EffectMutexUnlock();
        mAudioHardware->mStreamHandler->OutputStreamUnLock();
...
status_t AudioYusuStreamOut::setParameters(const String8& keyValuePairs)
{
    ...
    if (param.getInt(keyRoute, value) == NO_ERROR)
    {
        if(value!=0)
        {
            mAudioHardware->doKeyRouting(value);
        }
...
status_t AudioYusuStreamOut::ACFCompensationFilter::init(int filterMode,uint32_t bufferSize)
{
    ALOGV("ACFCompensationFilter::init");
    mCompFlt = new AudioCompensationFilter(AUDIO_COMP_FLT_AUDIO,bufferSize); // create filter
    if(mCompFlt)
    {
        mCompFlt->Init();
...
status_t AudioYusuStreamOut::ACFCompensationFilter::start(uint32 channel, uint32 samplerate)
{
    if(mCompFlt&&(bStarted==false))
    {
        ...
        mCompFlt->SetWorkMode(channel, samplerate,(AudioComFltMode_t)mFilterMode);
        mCompFlt->Start();
        bStarted = true;
        return NO_ERROR;
    }
...
status_t AudioYusuStreamOut::HCFCompensationFilter::init(int filterMode,uint32_t bufferSize)
{
    ALOGV("HCFCompensationFilter::init");
    mCompFlt = new AudioCompensationFilter(AUDIO_COMP_FLT_HEADPHONE,bufferSize); // create filter
    if(mCompFlt)
    {
        mCompFlt->Init();
        mCompFlt->LoadACFParameter();
...
bool AudioYusuStreamOut::EffectMutexLock(void)
{
    return mEffectLock.lock ();
}
bool AudioYusuStreamOut::EffectMutexUnlock(void)
{
    mEffectLock.unlock ();
    return true;
}

bool AudioYusuStreamOut::IsStandby(void)
{
    return mStandby;
}
void AudioYusuStreamOut::remove_echo_reference(struct echo_reference_itfe *reference)
{
    Mutex::Autolock _l(mEffectLock);
    ALOGD("remove_echo_reference %p",reference);
    if (echo_reference == reference) {
        /* stop writing to echo reference */
        reference->write(reference, NULL);
        echo_reference = NULL;
    }
}
void AudioYusuStreamOut::add_echo_reference(struct echo_reference_itfe *reference)
{
    Mutex::Autolock _l(mEffectLock);
...
unsigned int AudioYusuStreamOut::GetSampleRateInfo()
{
    return mAttribute->mSampleRate;
}
unsigned int AudioYusuStreamOut::GetChannelInfo()
{
    return mAttribute->mChannelCount;
}

int AudioYusuStreamOut::get_playback_delay(size_t frames, struct echo_reference_buffer *buffer)
{
