...
bool AudioStreamInHandler::ModemRecordCondition(uint32_t RecordSampleRate,uint32_t devices)
{
    ...
    else if((devices & AUDIO_DEVICE_IN_AMBIENT)==AUDIO_DEVICE_IN_AMBIENT || (devices & AUDIO_DEVICE_IN_FM)==AUDIO_DEVICE_IN_FM)
    {
        return false;
    }
    else
...
bool AudioStreamInHandler::SetInputMute(bool bMicMute)
{
    ALOGD("SetInputMute bMicMute = %d",bMicMute);
    if(mStreamInput != NULL){
         mStreamInput->SetInputMute(bMicMute);
    }
    return true;
}
...
status_t AudioStreamInHandler::addAudioEffect(effect_handle_t effect)
{
    ALOGD("addAudioEffect");
    status_t status = NO_ERROR;
    Mutex::Autolock lock(mRecordHandlerLock);
    if(mStreamInput != NULL){
         status = mStreamInput->addAudioEffect(effect);
    }
    return status;
}
status_t AudioStreamInHandler::removeAudioEffect(effect_handle_t effect)
{
    ALOGD("removeAudioEffect");
    status_t status = NO_ERROR;
    Mutex::Autolock lock(mRecordHandlerLock);
    if(mStreamInput != NULL){
         status = mStreamInput->removeAudioEffect(effect);
    }
    return status;
}
