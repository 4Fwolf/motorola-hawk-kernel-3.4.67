	...
	android_audio_legacy::AudioSystem::audio_in_acoustics acoustics)
{
    ...
    if(mApSampleRate != mSampleRate){
        ...
        pSrcHdl = BLI_Open( mApSampleRate, mChNum, mSampleRate, mChNum, pSrcBuf,NULL);
        if ( !pSrcHdl ) {
            return false;
        }
    }
...
void AudioYusuApStreamIn::StereoMonoConvert(short *buffer , int length)
{
#ifndef AUDIO_HQA_SUPPORT
#ifdef MTK_AP_SPEECH_ENHANCEMENT
    if(mChNum==1)   //need return mono data
    {
        if(mStereoMode) //speech enhancement output data is stereo, need to convert to mono
        {   
            short left; //use main MIC data
            int copysize = length>>2;
        ...
        //speech enhancement output data is mono
        else
        {
            short left; //use main MIC data
            int copysize = length>>2;
    ...
    else    //need return stereo data
    {
        ...
        //speech enhancement output data is stereo, no need to convert
    }
#else
    if(mChNum==1)   //need mono data
    {
        StereoToMono((short*)buffer ,length);
    }
    //need stereo data: is already stereo data, no need to convert
#endif
#endif
    ...
    if(pSrcHdl && (mApSampleRate != mSampleRate)){
        do{
            ...
            if(pInputSrcRead >= pInputSrcWrite){
                ...
                Read_Size = process_frames(pInputbuf+pInputSrcWrite, emptybufLen);

                 StereoMonoConvert((short*)(pInputbuf+pInputSrcWrite),Read_Size);
        ...
        Read_Size = process_frames(buffer,bytes);
        CheckNeedMute(buffer,bytes);
        if(CanRecordFM ()){
        ...
        CheckNeedDataSwitch((short*)(buffer),bytes);
        #endif	
        StereoMonoConvert((short*)(buffer),bytes);
...
void AudioYusuApStreamIn::SetMicSource(int mictype){
    ...
    switch (mictype){
        case LADIN_FM_Radio:{
            ...
            mHw->mAnaReg->SetAnaReg(AUDIO_CON5,0x0002,0xf);       // Select Level-shift buffer input as line-in R channels
            mHw->mAnaReg->SetAnaReg(AUDIO_CON6,0x1533,0xffff);       // Select Level-shift buffer input as line-in L ch
            usleep(200);
...
void AudioYusuApStreamIn::openRecordDevicePower()
{
    ...
    switch(Device){
        ...
        case LADIN_FM_AnalogIn_Stereo:
        {
            ...
            mHw->mAnaReg->SetAnaReg(AUDIO_CON6,0x1a33 ,0xffffffff);
            usleep(200);
            break;
        }
        case LADIN_Microphone1:
...
bool AudioYusuApStreamIn::SetInputMute(bool bMicMute)
{
    ALOGD("SetInputMute %d",bMicMute);
    if(mMicMute != bMicMute)
    {
        mMicMute =  bMicMute;
        mMuteTransition = false;
    }
...
status_t AudioYusuApStreamIn::ApplyInputMute(void* Buffer , uint32 BufferSize)
{
...
//                mHw->mpSPELayer->LoadSPEParameter();    //load voip parameter
                mHw->mpSPELayer->SetMICDigitalGain(SPE_MODE_VOIP, SetApMicGain_HDRecord(mApInputSource));
//                SetAPMicGain(mApInputSource);
                ...
                {
                    if(mHw->GetCurrentDevice () == AUDIO_DEVICE_OUT_SPEAKER)
...
#endif            
            {
                modeIndex = SetHdrecordingMode(mode,&mStereoMode);
                ...
                mHw->mpSPELayer->SetMICDigitalGain(SPE_MODE_REC, SetApMicGain_HDRecord(mApInputSource));
//                SetAPMicGain(mApInputSource);
                if(mStereoMode)
                    mHw->mpSPELayer->SetChannel(2); //set library do stereo process
                else
                    mHw->mpSPELayer->SetChannel(1); //set library do mono process
            }
...
//                SetAPMicGain(mApInputSource);
    ...
    if (mAPPS->need_echo_reference && echo_reference == NULL)
        echo_reference = mAPPS->start_echo_reference(
                                    AUDIO_FORMAT_PCM_16_BIT,
                                    mChNum,
                                    mApSampleRate);
    ALOGD("RecOpen echo_reference=%p",echo_reference);
    }
    return true;
}
...
bool AudioYusuApStreamIn::CanRecordFM()
{
   ...
   //if(mHw->GetAnalogLineinEnable() && mHw->GetRecordFM () && mDevice == AUDIO_DEVICE_IN_FM )
   if(mHw->GetAnalogLineinEnable() && mHw->GetRecordFM () && (mInputSource == AUDIO_SOURCE_FM))
   {
...
int AudioYusuApStreamIn::SetHdrecordingMode(int mode , bool *bStereoRecOn)
{
    ...
    //can not get match HD record mode, use the default one
        ...
        //also need to update stereo/mono condition
        /* only stereo flag is true, the stereo record is enabled */
        if(mHw->getInputDevice(mDevice)!= AUDIO_DEVICE_IN_WIRED_HEADSET){
    ...
    if(modeIndex==0xFF)
    {
        ALOGD("SetHdrecordingMode,if no modeIndex get from the scene table, use default parameters");
        if(mHw->GetApMicType() == LADIN_Microphone2)
            modeIndex = 3;
        else
            modeIndex = 2;
    }
    ALOGD("SetHdrecordingMode,modeIndex=%d,APMicType=%d,u4SceneIdx=%d",modeIndex,mHw->GetApMicType(),u4SceneIdx);
    return modeIndex;    
}
...
bool AudioYusuApStreamIn::vGetHdRecordModeInfo(uint8 *modeIndex, bool *bStereoRecOn)
{
    ...
    *bStereoRecOn = false;
    ...
    {        
        // get mode index
        uint32 u4CurrentDevice = mHw->getInputDevice(mDevice);

        if (mhdRecordSceneTable.scene_table[u4SceneIdx][HD_REC_DEVICE_SOURCE_HEADSET] != 0xFF
            && u4CurrentDevice == AUDIO_DEVICE_IN_WIRED_HEADSET)
        {
            *modeIndex = mhdRecordSceneTable.scene_table[u4SceneIdx][HD_REC_DEVICE_SOURCE_HEADSET];
...
bool AudioYusuApStreamIn::UpdateSPEParams()
{
    ...
    ALOGD("AudioYusuApStreamIn::UpdateSPEParams,mStereoMode=%d",mStereoMode);
    ...
    mHw->mpSPELayer->SetRecCompFilter(mhdRecordParam.hd_rec_map_to_fir_for_ch1[mHDRecordModeIndex],mhdRecordParam.hd_rec_map_to_fir_for_ch2[mHDRecordModeIndex],mStereoMode);
#endif    
...
#ifdef MTK_AP_SPEECH_ENHANCEMENT
    if((bflag1||mbLRChannelSwitch) && (miSpecificMicChoose==0 && bflag!=1&& bflag!=2) && mStereoMode)   //channel switch
#else
...
status_t AudioYusuApStreamIn::addAudioEffect(effect_handle_t effect)
{
    int status;
    ...
    if (memcmp(&desc.type, FX_IID_AEC, sizeof(effect_uuid_t)) == 0) {
        ...
        if(!mstandby){            
            if (echo_reference != NULL) {
                ...
                mAPPS->stop_echo_reference(echo_reference);
                echo_reference = NULL;
            }
            else{
        ...
        mAPPS->in_configure_reverse(mChNum,mApSampleRate);
    }
...
status_t AudioYusuApStreamIn::removeAudioEffect(effect_handle_t effect)
{
	int i;
    int status = -EINVAL;
    effect_descriptor_t desc;
    ...
    for (i = 0; i < mAPPS->num_preprocessors; i++){
        if (status == 0) { /* status == 0 means an effect was removed from a previous slot */
            ...
            ALOGD("removeAudioEffect moving fx from %d to %d", i, i - 1);
            continue;
        }
        if (mAPPS->preprocessors[i].effect_itfe == effect) {
            ALOGD("removeAudioEffect found fx at index %d", i);
//            free(preprocessors[i].channel_configs);
    ...
    ALOGD("removeAudioEffect(), effect type: %08x", desc.type.timeLow);
...
ssize_t AudioYusuApStreamIn::process_frames(void* buffer, ssize_t bytes)
{
    if(mAPPS->num_preprocessors == 0){
        ssize_t Read_Size;
        ...
        ssize_t frames = bytes / sizeof(int16_t)/ mChNum;
        int i;
        void *proc_buf_out;
        proc_buf_out = buffer;
        ...
        /* first reload enough frames at the end of process input buffer */
            if (mAPPS->proc_buf_frames < (size_t)frames) {
                ssize_t frames_rd;

                if (mAPPS->proc_buf_size < (size_t)frames) {
                    mAPPS->proc_buf_size = (size_t)frames;
                    mAPPS->proc_buf_in = (int16_t *)realloc(mAPPS->proc_buf_in,
                                                 mAPPS->proc_buf_size *mChNum* sizeof(int16_t));
                                                 
                ALOGD("process_frames(): proc_buf %p size extended to %d frames",
                     mAPPS->proc_buf_in, mAPPS->proc_buf_size);
            }
            
            frames_rd = ::read(mFd,mAPPS->proc_buf_in + mAPPS->proc_buf_frames * mChNum,
                               (frames - mAPPS->proc_buf_frames) * sizeof(int16_t) * mChNum);                        
                                    
            //dump data
            if(bflag)
            {
                char filename[]="/sdcard/mtklog/audio_dump/StreamIn_OriDump.pcm";                                  
                            
                if(checkAndCreateDirectory(filename)<0)
                    ALOGE("AudioYusuApStreamIn Dump_StreamIn_PCM checkAndCreateDirectory() fail!!!");
                else
                {
                    fp_in= fopen(filename, "ab+");
                    fwrite(mAPPS->proc_buf_in + mAPPS->proc_buf_frames * mChNum, frames_rd, 1, fp_in);
                    fclose(fp_in);
                }
            }
            
            frames_rd = frames_rd / sizeof(int16_t) / mChNum;
            if (frames_rd < 0) {
                frames_wr = frames_rd;
                break;
            }
            mAPPS->proc_buf_frames += frames_rd;
        }
        ...
        out_buf.s16 = (int16_t *)proc_buf_out + frames_wr * mChNum;        
        ...
        if (mAPPS->proc_buf_frames) {
            memcpy(mAPPS->proc_buf_in,
                   mAPPS->proc_buf_in + in_buf.frameCount * mChNum,
                   mAPPS->proc_buf_frames * mChNum * sizeof(int16_t));
        }

        /* if not enough frames were passed to process(), read more and retry. */
        if (out_buf.frameCount == 0)
        {
            ALOGD("Not enough frames");
            continue;
        }

        frames_wr += out_buf.frameCount;
    }
    return frames_wr * sizeof(int16_t) * mChNum;
  }
