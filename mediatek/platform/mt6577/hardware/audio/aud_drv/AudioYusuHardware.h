...
#define ASM_BUFFER_SIZE (0x2000) // 8k bytes
#endif
...
//headphone
    ...
    // speake  & headphone
    void doKeyRouting(uint32_t devices, int flag=0);
    status_t doOutputDeviceRouting(bool force=false);
    void doOutputDeviceRouting(uint32_t oldDevices, uint32_t newDevices,bool force=false);
    status_t doInputDeviceRouting(void);
    uint32_t selectDuptyDevice(uint32_t devices);
    void toggleDevices(uint32_t devices,bool on);
    void toggleCurrentDevices(bool on);
    void doModeRouting(uint32_t oldDevices, uint32_t newDevices);
    uint32_t getNewDevices();
...
//
    ...
    void SetInputSourceToModem();
    void SetInputSource(int LadSource, bool setToModem=false);
    void doInputKeyRouting(uint32_t inputDevices);
    uint32_t getInputDevice(uint32_t inputDevices);
	...
	bool setAnalogGain(uint64_t analogGain);
	bool setAnalogGain(uint32_t analogGain);
    bool setSpeechGaintableVolume(uint32_t gain);
	void  restoreAnalogGain();
	void  speechBandChange(bool NB);
	bool  isSpeechNB();
    ...
    virtual status_t SetDtvFmEnable(bool enable);
    ...
    int Get_A2M_SpeechOn_Status(void);
    void Set_A2M_SpeechOn_Status(bool Enable);
...
// for event handling
    ...
    //ALPS00264744
    pthread_mutex_t HardwareOnOffMutex; // use for audiostartrunning mutex
    //~ALPS00264744
...
private:
	...
	uint32_t			mAudioGainNormal;
	uint32_t			mAudioGainSpeech;
	uint64_t			mAudioAnalogGain;
	bool				mSpeechNB;
    bool                mA2M_SpeechOn_Status;
