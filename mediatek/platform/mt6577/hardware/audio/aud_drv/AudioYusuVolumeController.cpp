 ...
 int32 AudioYusuVolumeController::AudioGetRoute(uint32 routes)
{
    ...
    int i=0;
    routes = mAudioHardware->GetCurrentDevice();
/*
    int mode = -1;
...
bool AudioYusuVolumeController::InitVolumeController()
{
   ...
   //----------------------------------------------------------
    ...
    SetMicGain(VOIP_Handfree_Mic, -1 * degradegain<<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_VOICE_RECOGNITION_RECEIVER]);
    SetMicGain(Voice_Rec_Mic_Handset, -1 * degradegain<<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_VOICE_RECOGNITION_HEADSET]);
    SetMicGain(Voice_Rec_Mic_Headset, -1 * degradegain <<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_INCALL_WB_RECEIVER]);
    SetMicGain(Normal_Mic_WB,         -1 *  degradegain<<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_INCALL_WB_HEADSET]);
    SetMicGain(Headset_Mic_WB,        -1 * degradegain<<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_INCALL_WB_SPEAKER]);
    SetMicGain(Handfree_Mic_WB,       -1 * degradegain<<1);
    degradegain = (unsigned char)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_MIC_RESOLVED]);
    SetMicGain(TTY_CTM_Mic ,      -1 * degradegain<<1);
...
bool AudioYusuVolumeController::SetLadMicGain(int MicType)
{
    ...
    {
#ifdef MTK_AUDIO_GAIN_TABLE
        bool nb = mAudioHardware->isSpeechNB();
        if(!nb)
        {
            MicType +=(Normal_Mic_WB - Normal_Mic);
        }
#endif
...
switch (MicMode){
    ...
    case Fm_Record:
        ...
        break;
    case Voice_Rec_Mic_Handset:
        degradegain = (uint32)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_VOICE_RECOGNITION_RECEIVER]);
        break;
    case Voice_Rec_Mic_Headset:
        degradegain = (uint32)MampUplinkGain(mGainTable.microphoneGain.micGain[GAIN_VOICE_RECOGNITION_HEADSET]);
        break;
    case VOIP_Normal_Mic:
...
uint32 AudioYusuVolumeController::GetSideTone(int32_t device)
{
    ALOGD("GetSideTone device 0x%x",device);
    uint32 gain =0;
#ifdef MTK_AUDIO_GAIN_TABLE
    ...
    else if(device == android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADPHONE ||
             device == android_audio_legacy::AudioSystem::DEVICE_OUT_WIRED_HEADSET)
    {
        gain =  mSideTone[1];
    }
    else if(device == android_audio_legacy::AudioSystem::DEVICE_OUT_SPEAKER)
    {
        gain =  mSideTone[2];
    }
    else
...
bool AudioYusuVolumeController::setVoiceVolume(float volume, uint32 mode,uint32 routes)  // volume should range 0.0 ~ 1.0
{
    ...
    volume,mode,routes,GetDevice,logToLinear (volume)) ;
    //MapVolume = AudioMapVolume(volume); // map volume to 0xffff degree
    MapVolume = logToLinear (volume);
...
bool AudioYusuVolumeController::SetDtvFmVolume(int volume)
{
 //ALSA_A609N("%s",__FUNCTION__)//genesis
   int ret=0;
   ALOGD("!!SetFmVolume=%d, mfmFd=%d",volume,mfmFd);
...
bool AudioYusuVolumeController::SetMatvMute(bool b_mute)
{
 ...
 int AudioYusuVolumeController::logToLinear(float volume)
 {
