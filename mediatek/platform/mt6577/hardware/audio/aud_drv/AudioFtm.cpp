 ...
 * $Log:$
 *
 * 02 12 2014 weiguo.li
 * [ALPS01423978] [LGE][V5/S1][KK.BSP] Audio & Speech Path Change
 * .
 *
 * 02 09 2014 weiguo.li
 * [ALPS01424025] [LGE][V5/S1][KK.BSP] AMR WB/NB audio gain separation
 * fix build error
 *
 * 02 09 2014 weiguo.li
 * [ALPS01424025] [LGE][V5/S1][KK.BSP] AMR WB/NB audio gain separation
...
AudioFtm *AudioFtm::UniqueAudioFtmInstance = NULL;
...
void AudioFtm::AudFtm_SetHWPtr(AudioYusuHardware *hw, AudioAfe *pAfe, AudioAnalog *pAnaAfe)
{
    ALOGD("AudFtm_SetHWPtr \n");
    mAudioHardware = hw;
    mAfe = pAfe;
    mAnalog = pAnaAfe;
}
...
#ifdef MTK_AUDIO_GAIN_TABLE
void AudioFtm::initGainTable()
{
    int mask = 0;
    uint32_t gain = 0;

    mGainTable = (AUDIO_GAIN_TABLE_STRUCT *)malloc(sizeof(AUDIO_GAIN_TABLE_STRUCT));
    if(mGainTable){
        GetAudioGainTableParamFromNV(mGainTable); //read data from NVRAM
    }

    //loopbackSpeechGain[NUM_GAIN_OUTPUT_DEVICES][GAIN_TABLE_LEVEL];
    STREAM_SPEECH_GAIN_CONTROL_STRUCT *pSpeech = &mGainTable->speechGain;
    for(int i=0; i< NUM_GAIN_OUTPUT_DEVICES; i++)
    {
        if (i == GAIN_OUTPUT_HEADSET)
        {
            mask = 0x5;
        }
        else if (i == GAIN_OUTPUT_SPEAKER)
        {
#ifdef ALL_USING_VOICEBUFFER_INCALL
#ifdef ONLY_USING_SPK_AUDIOBUFFER_INCALL // if loudspeaker use audio buffer
            mask = 0xA;
#else
            mask = 0x6;
#endif
        ...
        {
            uint32_t pga = pSpeech->speechGain[i][j].u32Gain_PGA_Amp;
            uint8_t digital = pSpeech->speechGain[i][j].u8Gain_digital;

#ifdef ALL_USING_VOICEBUFFER_INCALL
#ifdef ONLY_USING_SPK_AUDIOBUFFER_INCALL // if loudspeaker use audio buffer
            if (i == GAIN_OUTPUT_SPEAKER)
            {
                uint32_t temp = pga;
                uint32_t input1 = pga>>22 & 0x3;
                uint32_t input2 = pga>>20 & 0x3;
                pga = ((temp>>24)<<24) | (input2<<22) | (input1<<20) | ((temp<<12)>>12);
            }
#endif
#endif
            gain = ((pga >> 10) << 12) | (digital<<4) | mask;
            loopbackSpeechGain[i][j]= gain;
        }
    }
    //update loopbackMicGain[GAIN_TABLE_LEVEL];
    STREAM_MICROPHONE_GAIN_CONTROL_STRUCT *pMic = &mGainTable->microphoneGain;
    for(int i=0; i< GAIN_TABLE_LEVEL; i++)
    {
        uint32_t pga = pMic->micGain[i];
        if(pga > MIC_GAIN_MAX) pga = MIC_GAIN_MAX;
        int degradedB = MIC_GAIN_MAX - pga;
        uint32_t gain = -1 * degradedB<<1;
        if(gain == 0) gain--;
        loopbackMicGain[i]= gain;
    }
    //loopbackSidetoneGain[GAIN_TABLE_LEVEL];
    STREAM_SIDETONE_GAIN_CONTROL_STRUCT *pSideTone = &mGainTable->sidetoneGain;
    for(int i=0; i< GAIN_TABLE_LEVEL; i++)
    {
        uint32_t pga = pSideTone->sidetoneGain[i];
        if(pga > SIDETONE_GAIN_MAX) pga = SIDETONE_GAIN_MAX;
        int degradedB = (SIDETONE_GAIN_MAX - pga)/SIDETONE_ONEDB_STEP;
        uint32_t gain = -1 * degradedB<<1;
        if(gain == 0) gain--;
        loopbackSidetoneGain[i]= gain;
    }
}
#endif
...
int AudioFtm::RecieverTest(char receiver_test)
{
    ...
    {
        FTM_AnaLpk_on();
        mAnalog->SetAnaReg(AUDIO_CON3, 0x01e0, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON5, 0x4400, 0xffff); //Enable HS + HPL buffer
        mAnalog->SetAnaReg(AUDIO_CON1, 0x0404, 0xffff); //adjust HS volume

        mAfe->Afe_DL_Unmute(AFE_MODE_DAC);
...
int AudioFtm::FMLoopbackTest(char bEnable)
{
    ...
    if (bEnable == true)
    {
        ...
        // ...
        // enable Analog AFE
        mAnalog->SetAnaReg(AUDIO_CON0, (0x4 << 12), 0xf000);
        mAnalog->SetAnaReg(AUDIO_CON1, 0x0C0C, MASK_ALL);
        mAnalog->SetAnaReg(AUDIO_CON2, 0x000C, MASK_ALL);
        mAnalog->SetAnaReg(AUDIO_CON3, 0x0070, MASK_ALL); // enable voice buffer, audio left/right buffer
        //      SetAnaReg(AUDIO_CON5,0x0220,MASK_ALL);  // FM mono playback (analog line in)
        mAnalog->SetAnaReg(AUDIO_CON5, 0x0110, MASK_ALL); // FM stereo playback (analog line in)
#if 0
        //mAnalog->AnalogAFE_Depop(FM_PATH_STEREO,true);
#else
        mAnalog->SetAnaReg(AUDIO_NCP0, 0x102B, 0xffff);
        mAnalog->SetAnaReg(AUDIO_NCP1, 0x0600, 0x0E00);
        mAnalog->SetAnaReg(AUDIO_LDO0, 0x1030, 0x1fff);
        mAnalog->SetAnaReg(AUDIO_LDO1, 0x3010, 0xffff);
        mAnalog->SetAnaReg(AUDIO_LDO2, 0x0013, 0x0013);
        mAnalog->SetAnaReg(AUDIO_GLB0, 0x2920, 0xffff);
        mAnalog->SetAnaReg(AUDIO_GLB1, 0x0000, 0xffff);
        mAnalog->SetAnaReg(AUDIO_REG1, 0x0001, 0x0001);

        mAnalog->SetAnaReg(AUDIO_CON0, 0x5000, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON1, 0x3f0C, 0x3F3F);
        mAnalog->SetAnaReg(AUDIO_CON2, 0x000C, 0x003F);
        mAnalog->SetAnaReg(AUDIO_CON3, 0x0030, 0x01FF);
        mAnalog->SetAnaReg(AUDIO_CON5, 0x0110, MASK_ALL);

        mAnalog->SetAnaReg(AUDIO_CON6, 0x0533, 0x0FFF);
        mAnalog->SetAnaReg(AUDIO_CON7, 0x003F, 0x003F);
        mAnalog->SetAnaReg(AUDIO_CON7, 0x0000, 0xC000);
        mAnalog->SetAnaReg(AUDIO_CON8, 0x0000, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON9, 0x0058, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON14, 0x0000, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON17, 0x0018, 0xffff);
#endif
        mAfe->Afe_DL_Unmute(AFE_MODE_DAC);
        Audio_Set_Speaker_On(Channel_Stereo);
    }
    ...
    {
        mAfe->Afe_DL_Mute(AFE_MODE_DAC);
        //mAfe->ForceDisableSpeaker();
        Audio_Set_Speaker_Off(Channel_Stereo);
        // disable Digital AFE
        // ...
        // disable Analog AFE
        mAnalog->SetAnaReg(AUDIO_CON0, 0, 0xf000);
        mAnalog->SetAnaReg(AUDIO_CON3, 0x0000, 0x00f0); // disable voice buffer, audio left/right buffer
        mAnalog->SetAnaReg(AUDIO_CON5, 0x0440, 0x0ff0); // disable FM mono playback (analog line in)

        mAnalog->AnalogAFE_Depop(FM_PATH_STEREO, false);
...
int AudioFtm::PhoneMic_Receiver_Acoustic_Loopback(int Acoustic_Type, int *Acoustic_Status_Flag, int bHeadset_Output)
{
    ...
    LadInPutDevice_Line inputSource = LADIN_Microphone1;
    ...
    switch (Acoustic_Type)
    {
        ...
        case DUAL_MIC_WITHOUT_DMNR_ACS_ON:
            ...
            SetLoopbackOn(android::MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITHOUT_DMNR, bHeadset_Output);
        ...
        case DUAL_MIC_WITH_DMNR_ACS_ON:
            ...
            SetLoopbackOn(android::MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITH_DMNR, bHeadset_Output);
...
static bool bCurrentUseEarphone = false;
...
bool AudioFtm::SetLoopbackOn(ENUM_LOOPBACK_TYPES eLoopbackType, bool bEarphoneInserted)
{
...
#endif
#endif
            eCurrentLoopbackMode = SPH_MODE_EARPHONE;
        }
...
#endif
#endif
            eCurrentLoopbackMode = SPH_MODE_LOUDSPK;
        }
        else {  // receiver
            eOutputDevice = LADOUT_SPEAKER1;
            eCurrentLoopbackMode = SPH_MODE_NORMAL;
        }
    ...
    if (bMdLoopback == true)
    {
        // turn on amp for loud speaker
        if (bCurrentUseLoudSpeaker == true)
        {
            Audio_Set_Speaker_On(Channel_Stereo);
        }
        // turn on amp for earphone
        else if (bCurrentUseEarphone == true)
        {
            Audio_Set_HeadPhone_On(Channel_Stereo);
        }
...
#else
        SetLoopbackVolume(eCurrentLoopbackVolumeLevel, true);
        mLad->LAD_SetMicrophoneVolume(loopbackMicGain[eCurrentLoopbackMode+2]);  // speech mic gain start from index 2
        mLad->LAD_SetSidetoneVolume(loopbackSidetoneGain[eCurrentLoopbackMode]);
#endif
        // unmute
        usleep(50000);
...
bool AudioFtm::SetLoopbackOff()
{
    ...
    if (bMdLoopback == true)
    {
        ...
        // turn off amp for earphone
        else if (bCurrentUseEarphone)
        {
            Audio_Set_HeadPhone_Off(Channel_Stereo);
            mLad->LAD_SetOutputDevice(LADOUT_SPEAKER1);
            bCurrentUseEarphone = false;
        }
    }
...
bool AudioFtm::SetLoopbackDelay(uint32 delay)
{
    ALOGD("+SetLoopbackDelay(), delay = %d", delay);

    pthread_mutex_lock_timeout_np(&mLoopbackMutex, 200); // 200 ms

    mLad->LAD_SetAcousticLoopbackDelay(delay);

    pthread_mutex_unlock(&mLoopbackMutex);

    ALOGD("-SetLoopbackDelay()");

    return true;
}

void AudioFtm::SetLoopbackVolume(uint loopbackVolumeLevel, bool setForce)
{
    ALOGD("+SetLoopbackVolume(), eCurrentLoopbackMode = %d, loopbackVolumeLevel = %d", eCurrentLoopbackMode, loopbackVolumeLevel);

#ifdef MTK_AUDIO_GAIN_TABLE
    if ((eCurrentLoopbackType != NO_LOOPBACK) || setForce) // set gain during loopback
    {
        uint32_t analogGain = loopbackSpeechGain[eCurrentLoopbackMode][loopbackVolumeLevel];
        ...
        ALOGD("digital 0x%x,analog 0x%x,amp 0x%x, mask 0x%x",digital,hpGain,amp,mask);

#ifndef ALL_USING_VOICEBUFFER_INCALL
        if((eCurrentLoopbackMode == SPH_MODE_EARPHONE) ||
           (eCurrentLoopbackMode == SPH_MODE_LOUDSPK)) // control audio buffer for speaker and headphone mode
        {
            mAnalog->AnalogAFE_Set_DL_AUDHPL_PGA_Gain(hpGain);
            mAnalog->AnalogAFE_Set_DL_AUDHPR_PGA_Gain(hpGain);
        }
#endif

        mLad->LAD_SetGainTableOutputVolume(gain);

        if (mask != 0)
        {
            mAudioHardware->setAmpAnalogGain((uint32_t)amp);
        }
    }

    eCurrentLoopbackVolumeLevel = loopbackVolumeLevel;
#endif

    ALOGD("-SetLoopbackVolume()");
}
...
{
    ALOGD("EarphoneTestLR is not supported!!! ");
    return false;
}
...
{
    ALOGD("Audio_FM_I2S_Play : %d", bEnable);
    //Request Analog clock before access analog hw
    mAnalog->AnalogAFE_Request_ANA_CLK();

    if (bEnable == true)
    {
        ALOGD("+Audio_FM_I2S_Play true");
        //mAnalog->Meta_Open_Analog(AUDIO_PATH);
        //Temp solution
        mAnalog->SetAnaReg(AUDIO_NCP0, 0x102b, 0xffff);
        mAnalog->SetAnaReg(AUDIO_NCP1, 0x0600, 0xffff);
        //mAnalog->SetAnaReg(AUDIO_NCP1,0x0000,0xffff);
        ...
        mAnalog->SetAnaReg(AUDIO_CON9, 0x0001, 0x0001);
        //~Temp solution
        mAfe->Afe_DL_Start(AFE_MODE_FTM_I2S);
        //Temp solution
        mAfe->SetAfeReg(AFE_DAC_CON0, 0x00000000, 0x00000008); //Disable VUL
        mAfe->SetAfeReg(AFE_DAC_CON1, 0x00000a00, 0x00000f00); //48kHz
        mAfe->SetAfeReg(AFE_DL_SRC2_1, 0x80000000, 0xf0000000); //48kHz
        mAfe->SetAfeReg(AFE_CONN1, 0x00200000, 0xffffffff); //I00_O03_S
        mAfe->SetAfeReg(AFE_CONN2, 0x00002000, 0xffffffff); //I01_O04_S
        //~Temp solution
        mAfe->Afe_Set_Stream_Gain(0xffff); // hardcore AFE gain
        mAfe->Afe_DL_Unmute(AFE_MODE_DAC);
        Audio_Set_Speaker_On(Channel_Stereo);
    ...
    else
    {
        ...
        Audio_Set_Speaker_Off(Channel_Stereo);
        mAfe->Afe_DL_Stop(AFE_MODE_FTM_I2S);
        Meta_Close_Analog();
        ::ioctl(mFd, AUDDRV_RESET_BT_FM_GPIO, 0); // Reset GPIO pin mux
        ALOGD("-Audio_FM_I2S_Play false");
    }
    //Release Analog clock after access analog hw
    mAnalog->AnalogAFE_Release_ANA_CLK();
...
{
    ALOGD("Audio_MATV_I2S_Play is not supported!!! ");
    return false;
}
...
int AudioFtm::Audio_FMTX_Play(bool Enable, unsigned int  Freq)
{
    ...
    return WavGen_SW_SineWave(Enable, Freq, 0); // 0: FM-Tx, 1: HDMI
}
...
int AudioFtm::ATV_AudPlay_On()
{
    ...
    mAfe->Afe_DL_Start(AFE_MODE_FTM_I2S);
    //Temp solution
    mAfe->SetAfeReg(AFE_DAC_CON1, 0x00000800, 0x00000f00); //32kHz
    mAfe->SetAfeReg(AFE_DL_SRC2_1, 0x60000000, 0xf0000000); //32kHz
    //~Temp solution
    mAfe->Afe_Set_Stream_Gain(0xffff); // hardcore AFE gain
    mAfe->Afe_DL_Unmute(AFE_MODE_DAC);
    Audio_Set_Speaker_On(Channel_Stereo);
...
int AudioFtm::Audio_HDMI_Play(bool Enable, unsigned int Freq)
{
    ...
    if (Enable)
    {
        mAnalog->SetAnaReg(AUDIO_CON1, 0x000C, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON2, 0x000C, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON3, 0x01B0, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON9, 0x0052, 0xfffe); //AFIFO enable
        usleep(1);
        mAnalog->SetAnaReg(AUDIO_CON9, 0x0001, 0x0001);
        mAnalog->SetAnaReg(AUDIO_CON14, 0x00c0, 0xffff);
    }
    else
    {
        mAnalog->SetAnaReg(AUDIO_CON1, 0, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON2, 0, 0xffff);
        mAnalog->SetAnaReg(AUDIO_CON3, 0, 0xffff);
    }
    //Release Analog clock after access analog hw
    mAnalog->AnalogAFE_Release_ANA_CLK();

    return WavGen_SW_SineWave(Enable, Freq, 1); // 0: FM-Tx, 1: HDMI
...
int AudioFtm::Audio_READ_SPK_OC_STA(void)
{
    ...
    if (mFd >= 0)
    {
        ...
        ALOGD("Audio_READ_SPK_OC_STA get status(%d) \n", s4RegValue);
    }
    else
    {
        ALOGD("Audio_READ_SPK_OC_STA unable get mFd \n");
        s4RegValue = -1;
    }
    return s4RegValue;
}
