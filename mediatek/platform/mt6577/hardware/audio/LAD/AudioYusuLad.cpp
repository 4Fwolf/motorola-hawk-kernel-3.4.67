...
LAD::LAD(AudioYusuHardware *hw)
{
   ...
   //initialize 	//#ALPS00365955  ship add
...
*/
    ...
    ALOGD("LAD_SetInputSource (0x%x)",HW_Mode);
   ...
   ALOGD(AUDMSG"LAD_SetInputSource (0x%x), HW_Mode new(%x) old(%x)",A2M_SetInputSource,HW_Mode,m_InputSrc);
...
{
    ALOGD("LAD_ForceSetInputSource (0x%x)",HW_Mode);
#if MTK_DIGITAL_MIC_SUPPORT
   if(HW_Mode == LADIN_Microphone1)
   {
      HW_Mode = LADIN_SingleDigitalMic;
...
/*
   if (m_InputSrc == (int)HW_Mode)
       return true;
*/
   ALOGD(AUDMSG"LAD_ForceSetInputSource (0x%x), HW_Mode new(%x) old(%x)",A2M_SetInputSource,HW_Mode,m_InputSrc);
   m_InputSrc = (int)HW_Mode;
   return pCCCI->SendMessageInQueue(  CCCI_MSG1(A2M_SetInputSource, HW_Mode));
}
...
bool LAD::LAD_SetAcousticLoopbackDelay(uint32 delay)
{
    if(delay > 100){
       ALOGD("LAD_SetAcousticLoopbackDelay: We can't put a delay over 100 (2 sec)");
       return true;
    }

    LOG_LAD(AUDMSG"LAD_SetAcousticLoopbackDelay, A2M_SetAcousticLoopbackDelay delay:%d", delay);
    return pCCCI->SendMessageInQueue(CCCI_MSG1(A2M_SetAcousticLoopbackDelay, delay));
}
