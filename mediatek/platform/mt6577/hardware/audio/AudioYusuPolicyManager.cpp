...
status_t AudioYusuPolicyManager::setDeviceConnectionState(audio_devices_t device,
...
AudioSystem::device_connection_state AudioYusuPolicyManager::getDeviceConnectionState(audio_devices_t device,
                                                  const char *device_address)
{
                                    ...
                                    AudioSystem::audio_in_acoustics acoustics)
{
    ...
    for(size_t count =0; count <mInputs.size();count++ ){
        ...
        {
            ALOGD("inputDesc->mInputSource [%d] Break",inputDesc->mInputSource);
            break;
        }

        AudioInputDescriptor *inputTemp = mInputs.valueAt(count);

        if(inputTemp->mInputSource == AUDIO_SOURCE_HOTWORD)
        {
            ALOGD("inputTemp->mInputSource [%d] Continue",inputTemp->mInputSource);
            continue;
        }
...
audio_io_handle_t AudioYusuPolicyManager::selectOutputForEffects(
...
audio_io_handle_t AudioYusuPolicyManager::getOutputForEffect(const effect_descriptor_t *desc)
{
...
status_t AudioYusuPolicyManager::registerEffect(const effect_descriptor_t *desc,
...
bool AudioYusuPolicyManager::isNonOffloadableEffectEnabled()
{
...
bool AudioYusuPolicyManager::isStreamActiveRemotely(int stream, uint32_t inPastMs) const
{
    nsecs_t sysTime = systemTime();
    for (size_t i = 0; i < mOutputs.size(); i++) {
        const AudioOutputDescriptor *outputDesc = mOutputs.valueAt(i);
...
bool AudioYusuPolicyManager::isSourceActive(audio_source_t source) const
{
...
// of the system.
bool AudioYusuPolicyManager::isOffloadSupported(const audio_offload_info_t& offloadInfo)
{
    ...
    mSpeakerDrcEnabled(false)
{
    ...
    mAvailableOutputDevices &= ~(AUDIO_DEVICE_OUT_EARPIECE);
#endif
}
...
#ifdef MTK_AUDIO
    }else if (outputDesc->isStrategyActive(STRATEGY_PROPRIETARY_ANLG)) {
        device = getDeviceForStrategy(STRATEGY_PROPRIETARY_ANLG, fromCache);
    }else if (outputDesc->isStrategyActive(STRATEGY_PROPRIETARY_DGTL)) {
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_PROPRIETARY_ANLG:{
        if (!isInCall() && mForceUse[AudioSystem::FOR_PROPRIETARY]==AudioSystem::FORCE_SPEAKER) {
            device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_SPEAKER;
        }
        if (device == AUDIO_DEVICE_NONE) {
            device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE;
...
#ifdef MTK_AUDIO    //ALPS01439684 . When short-Antenna enable (no headset), fm won't mute.
            if((strategy == STRATEGY_SONIFICATION) && isStreamActive(AudioSystem::FM) && !(mAvailableOutputDevices&(AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_WIRED_HEADSET)))
                device = device|AUDIO_DEVICE_OUT_WIRED_HEADPHONE;
#endif
            if (device == AUDIO_DEVICE_NONE) {
    ...
    case STRATEGY_PROPRIETARY_DGTL: {
        if ( strategy == STRATEGY_PROPRIETARY_DGTL && !isInCall() && 
...
void AudioYusuPolicyManager::updateDevicesAndOutputs()
{
                                                       ...
                                                       uint32_t delayMs)
{
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++) {
        ...
        bool bFM2MusicStarTempMute = (outputDesc->isStrategyActive((routing_strategy)STRATEGY_PROPRIETARY_ANLG) && outputDesc->mRefCount[AudioSystem::MUSIC]&&(i==STRATEGY_MEDIA));
#endif
        if (doMute || tempMute
#ifdef MTK_AUDIO
...
#ifdef MTK_AUDIO
                if((STRATEGY_PROPRIETARY_ANLG == i) && (mute == false)){ // fix issue: headphone pluggin, fm play, enter setting, play ringtone, stop ringtone, fm sound will leak from speaker
...
#ifdef MTK_AUDIO
                        if((STRATEGY_PROPRIETARY_ANLG == i)||(STRATEGY_MEDIA == i && bFM2MusicStarTempMute))//[ALPS00414767] Add STRATEGY_MEDIA case
                        {
...
bool AudioYusuPolicyManager::isVirtualInputDevice(audio_devices_t device)
{
...
audio_io_handle_t AudioYusuPolicyManager::getActiveInput(bool ignoreVirtualInputs)
{
    for (size_t i = 0; i < mInputs.size(); i++) {
...
const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sSpeakerSonificationVolumeCurveDrc[AudioYusuPolicyManager::VOLCNT] = {
...
const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sDefaultSystemVolumeCurveDrc[AudioYusuPolicyManager::VOLCNT] = {
...
const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sDefaultVoiceVolumeCurve[AudioYusuPolicyManager::VOLCNT] = {
    {0, -42.0f}, {33, -28.0f}, {66, -14.0f}, {100, 0.0f}
};

const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sSpeakerVoiceVolumeCurve[AudioYusuPolicyManager::VOLCNT] = {
                                            ...
                                            audio_devices_t device)
{
    ...
    volume = computeCustomVolume(stream,volInt,device);
    ...
    if(stream == AudioSystem::FM)
    {
        ...
        //need to adjust fm chip volume if fm is line in path, 
        //the value will be used to compute index like index = volume * 100;
        volume = index /100.0f;
#else
        //need to adjust digital gain if fm is digital input because mt6620 can't control chip volume in case of digital input
        float volInt = (volume_Mapping_Step * (index - streamDesc.mIndexMin)) / (streamDesc.mIndexMax - streamDesc.mIndexMin);
        volume = linearToLog(volInt);
#endif
    }
...
#endif
#ifdef FM_ANALOG_IN_SUPPORT
            if((stream==AudioSystem::FM)&&(output!=mPrimaryOutput||device==AUDIO_DEVICE_NONE)){
...
audio_devices_t AudioYusuPolicyManager::AudioOutputDescriptor::device() const
{
    if (isDuplicated()) {
...
bool AudioYusuPolicyManager::AudioOutputDescriptor::isActive(uint32_t inPastMs) const
{
    return isStrategyActive(NUM_STRATEGIES, inPastMs);
}

bool AudioYusuPolicyManager::AudioOutputDescriptor::isStrategyActive(routing_strategy strategy,
                                                                       uint32_t inPastMs,
...
bool AudioYusuPolicyManager::AudioOutputDescriptor::isStreamActive(AudioSystem::stream_type stream,
                                                                       uint32_t inPastMs,
                                                                       nsecs_t sysTime) const
{
...
bool AudioYusuPolicyManager::stringToBool(const char *value)
{
...
#ifdef MTK_AUDIO
#ifdef DISABLE_EARPIECE
    profile->mSupportedDevices &= ~(AUDIO_DEVICE_OUT_EARPIECE);
#endif
#endif
    ALOGW_IF(profile->mSupportedDevices == AUDIO_DEVICE_NONE,
...
#ifdef MTK_AUDIO
#ifdef MTK_AUDIO_GAIN_TABLE
void AudioYusuPolicyManager::updateAnalogVolume(audio_io_handle_t output, audio_devices_t device,int delayMs,bool force)
{
    if(output == mPrimaryOutput)
    {
        if (device == 0) {
        ...
        uint64_t anaGain  = mAudioUCM->volIndexToAnalogVol(device);
        if( anaGain != 0xFFFFFFFFFFFFFFFF && (mAnalogGain != anaGain || force))
        {
            ALOGD("updateAnalogVolume  analogGain 0x%llx", anaGain);
            char str[34];
            if (snprintf(str, 34, "%llu", anaGain) <= 0) {
               ALOGE("updateAnalogVolume Fail to assign volume");
            }
            String8 str8 = String8(str);
            mAnalogGain = anaGain;
            AudioParameter outputCmd = AudioParameter();
            //outputCmd.addInt(String8(keySetHwAnalog),(int)&mAnalogGain);
            outputCmd.add(String8(keySetHwAnalog),str8);
...
float AudioYusuPolicyManager::computeCustomVolume(int stream, float &volInt,audio_devices_t device)
{
	...
	case AudioSystem::NOTIFICATION:
        ...
        case AudioSystem::MUSIC:
            ...
            break;
        case AudioSystem::FM:
            if(OutputDevice == AUDIO_DEVICE_OUT_SPEAKER || mFmForceSpeakerState ){
...
float AudioYusuPolicyManager::computeCustomVoiceVolume(int stream, int index, audio_io_handle_t output, audio_devices_t device)
{
    float volume = 1.0;
    AudioOutputDescriptor *outputDesc = mOutputs.valueFor(output);
    StreamDescriptor &streamDesc = mStreams[stream];
