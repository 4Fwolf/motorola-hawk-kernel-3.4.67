...
static void usbdl_jump_da(void)
{
    ...
    COMPILE_ASSERT(TC1_FAC_IMEI_LEN <= DA_MAX_IMEI_LEN);
    COMPILE_ASSERT(TC1_FAC_PID_LEN <= DA_MAX_PID_LEN);
    COMPILE_ASSERT(TC1_FAC_SV_LEN <= DA_MAX_SWV_LEN);

    //for write test value into storage device
    if ( TC1_FAC_NAME(FacReadImei(TRUE, &da_arg->imei[0]))) 
	{
        print("%s IMEI: %s\n", MOD, da_arg->imei);
        da_arg->imei_len = TC1_FAC_IMEI_LEN;
    }
    if (TC1_FAC_NAME(FacReadPid(&da_arg->pid[0]))) {
        print("%s PID : %s\n", MOD, da_arg->pid);
        da_arg->pid_len = TC1_FAC_PID_LEN;
    }
    TC1_FAC_NAME(FacGetSoftwareversion(TRUE, &da_arg->sw_ver[0]));
    print("%s SWV : %s\n", MOD, da_arg->sw_ver);
    da_arg->sw_ver_len = TC1_FAC_SV_LEN;
#endif
...
//<20121009-14863-Eric Lin, [Hawk4.0][secu] To remove some miscellaneous code from PL.
        switch (cmd) {
        case CMD_GET_BL_VER:
