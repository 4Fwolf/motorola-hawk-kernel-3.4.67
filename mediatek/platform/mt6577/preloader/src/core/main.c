...
{
    /* enable emergency recovery download */
    platform_download(1, CFG_EMERGENCY_DL_MAX_TIMEOUT_MS);
...
void bldr_jump(u32 addr, u32 arg1, u32 arg2)
{
    ...
    /* disable emergency recovery download */
    platform_download(0, 0);
...
void main(void)
{
    ...
    if (platform_is_three_g()) {
        addr = CFG_DSP_3G_ROM_MEMADDR;
        name = PART_DSP_3G_ROM;
    } else {
        addr = CFG_DSP_2G_ROM_MEMADDR;
        name = PART_DSP_2G_ROM;
    }
