...
//#endif //#ifdef REMOVE_FOR_POWERON_FAIL
}
#endif
...
//#define Repeatable  1		   
					   /* Repeatable pseudorandom sequence */
...
#else
#define PoolSize    ALG_SYS_MEM_SIZE //3000*1024	      /* Test buffer pool size */
#endif
...
#define CUSTOM_HEAP_SIZE 	(500*1024)/sizeof(unsigned int)
//static char bget_buf_pool[PoolSize];
//extern unsigned int alg_sys_mem[];

#if 0
unsigned int alg_sys_mem[CUSTOM_HEAP_SIZE];
#else
__attribute__((section(".mem.region"))) unsigned int alg_sys_mem[CUSTOM_HEAP_SIZE];
//unsigned int alg_sys_mem[CUSTOM_HEAP_SIZE];
#endif

unsigned char create_bget_buf_pool_b=0;
static char *bchain = NULL;	      /* Our private buffer chain */
static char *bp = NULL; 	      /* Our initial buffer pool */
//extern const long algHeapMemorySize;
const long algHeapMemorySize=CUSTOM_HEAP_SIZE;

void create_bget_buf_pool(void)
{
	if(create_bget_buf_pool_b) 
		return ;

    //bp = malloc(PoolSize);
	bp = (char *)alg_sys_mem;
	memset(bp,0,CUSTOM_HEAP_SIZE);
    bpool((void *) bp, (bufsize) algHeapMemorySize);
