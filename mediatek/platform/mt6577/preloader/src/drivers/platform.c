...
//<20121106-16319-Eric Lin, [secu] About read secu version functions.
#include "sec_rom_info.h"
//>20121106-16319-Eric Lin
/*============================================================================*/
...
ulong g_boot_time; 
//<2012/10/4-14682-joshualee, add the escape condition in preloader after download-reboot immidiately by MDT API (WDT enable)
extern unsigned short reboot_from;
//>2012/10/4-14682-joshualee
//<20121106-16319-Eric Lin, [secu] About read secu version functions.
extern AND_ROMINFO_T                    g_ROM_INFO;
//>20121106-16319-Eric Lin

//<<EricHsieh,Modify the boot condition
uint16 boot_reason=0;
#define BR_PWK (1<<0)
#define BR_USB (1<<1)
#define BR_RTC (1<<2)
#define BR_WDT	(1<<3)
#define BR_WDT_PK (1<<4)
#define BR_TOOL_PK (1<<5)
#define BR_UNKNOWN (1<<6)
#define BR_WDT_PK_HW_Reset (1<<7)
//>>EricHsieh,Modify the boot condition
...
//<2012/11/29-17943-kevincheng,Remove extra logs to increase preloader space    
...
//<20121106-16366-Eric Lin, To remove the DBG message from PL.
void platform_vusb_on(void)
{
...
//<20121106-16319-Eric Lin, [secu] About read secu version functions.
       bootarg->pl_sec_ver = g_ROM_INFO.m_rom_info_ver;
//>20121106-16319-Eric Lin

//<2012/11/29-17943-kevincheng,Remove extra logs to increase preloader space
    //print("%s boot time: %dms\n", MOD, bootarg->boot_time);
//>2012/11/29-17943-kevincheng
#endif
}
...
{
    /* enter download mode */
//<2012/11/29-17943-kevincheng,Remove extra logs for increase preloader space
...
{  
    ...
    /* should nerver be reached */
		...
		U8 cmd=0x25;
		int dat =0;
		ret = pmic6329_i2c_read(0xc2, &cmd, 1, &dat, 1);
		dat &= 0xF0;
		if (dat==0xA0)
		{
			//print("%s HW pull the system reboot pin to reboot, mtk71259 verify!\n", MOD);
			boot_reason|=BR_WDT_PK_HW_Reset;
			//return BR_WDT_BY_PASS_PWK;
		}
#endif    
...
//<2012/12/11-18586-kevincheng,Enable FCD feature
  //      print("%s COM handshake timeout force disable: Key\n", MOD);
//>2012/12/11-18586-kevincheng
        return TRUE;
    }
#endif
...
//<2012/10/4-14682-joshualee, add the escape condition in preloader after download-reboot immidiately by MDT API (WDT enable)
    //print("%s COM handshake detect...  [g_rgu_status] ths time : 0x%x \n", MOD, g_rgu_status    );
    //print("%s COM handshake detect...  [reboot_from] ths time : 0x%x \n", MOD, reboot_from    );

    // reboot from the preloader because of MDT API 
    if ( (g_rgu_status == 0x1) && (reboot_from == 0x1) ) {
        //print("%s COM handshake escapes because of reboot command from MDT \n", MOD);
        return FALSE;
    }       
//>2012/10/4-14682-joshualee
...
//<2012/10/4-14682-joshualee, add the escape condition in preloader after download-reboot immidiately by MDT API (WDT enable)
#if 1
        SETREG32(MTK_WDT_MODE, MTK_WDT_MODE_IN_DIS | MTK_WDT_MODE_KEY );
        //print("%s COM handshake detect... Current WDT_IN_DIS bit: 0x%x \n", MOD, (INREG32(MTK_WDT_MODE) & MTK_WDT_MODE_IN_DIS)  );
...
{
    u32 ret, tmp;
//<2012/10/18-15347-alberthsiao,Add MTK HW reset function for Hawk40
//<2012/10/19-15411-alberthsiao,Disable HW reset for Hawk35
#if defined(ARIMA_PROJECT_HAWK40)
//>2012/10/19-15411-alberthsiao
    U8 cmd=0x25;
    U8 dat = 0x00;
#endif
//>2012/10/18-15347-alberthsiao
...
//>2012/10/19-15411-alberthsiao
    {
    	  ...
    	  dat |= 0xA0;
    	  ret = pmic6329_i2c_write(0xc2, &cmd, 1, &dat, 1);
    	  ret = pmic6329_i2c_read(0xc2, &cmd, 1, &dat, 1);
    	  //print("PMIC read bank1 0x25 (%d)\n",dat);
        rtc_bbpu_power_on();
    }
    else
    {
    	  ...
    	  {
        	rtc_bbpu_power_on();
    	  	//print("PMIC keep bbup power-on read bank1 0x25 (%d)\n",dat);
    	  }
    	  else
    	  {
    	  	pl_power_off();
...
#if defined(CFG_WIFI_CLOCK_SET)
   //predefinne should be set to use below function for only specific customer project.
   //mt6577_gpio_init();
    mt_set_gpio_mode(GPIO201, 3);   //set to mode3: CLKM0
    mt_set_gpio_dir(GPIO201, 1);       //set to output    
    mt_set_clock_output(0, CLK_SRC_F32K, 1);	

    mt_set_gpio_mode(GPIO192, 3);	//set to mode3: CLKM4
    mt_set_gpio_dir(GPIO192, 1);	   //set to output	  
    mt_set_clock_output(4, CLK_SRC_F32K, 1);	
#endif
