 ...
 *  USB TTY DEBUG
 **************************************************************************/
#define  MT_USB_TTY_DBG_LOG   0

#if MT_USB_TTY_DBG_LOG
...
#define USB_INFO_ADDR 0xC1019000
#define PDN_CTR 0xC0001020
#endif
...
usbdl_init (void)
{
    ...
    __raw_writel(((u32)__raw_readl(PDN_CTR) & 0x7FFF), PDN_CTR);
    ...
    key = (u32)__raw_readl(USB_INFO_ADDR + 0x144);
    key = (key << 32) | ((u32)__raw_readl(USB_INFO_ADDR + 0x140));

     if (key != 0)
     {
	get_serial(key, 6577, serial_num);
        string->bLength = sizeof (iserial);
     }
     else
     {
	memcpy(serial_num, "0123456789ABCDEF", 16);
    	string->bLength = sizeof (iserial) - 6; //Reduce string size for 'MT-'.
     }
