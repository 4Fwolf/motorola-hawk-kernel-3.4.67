...
#if 1
unsigned short reboot_from = RE_BOOT_FROM_UNKNOW; 
#else //MTK's original
static unsigned short reboot_from = RE_BOOT_FROM_UNKNOW; 
#endif
//>2012/10/4-14682-joshualee
static unsigned short rgu_mode = 0;
...
static void mtk_wdt_set_time_out_value(UINT16 value)
{
    /*
...
void mtk_wdt_init(void)
{
    ...
    print("RGU LENGTH:   %x\n", DRV_Reg16(MTK_WDT_LENGTH));
    print("RGU STA:      %x\n", DRV_Reg16(MTK_WDT_STATUS));
    print("RGU INTERVAL: %x\n", DRV_Reg16(MTK_WDT_INTERVAL));
    print("RGU SWSYSRST: %x\n", DRV_Reg16(MTK_WDT_SWSYSRST));
    print("==== Dump RGU Reg End ====\n");
    
    rgu_mode = DRV_Reg16(MTK_WDT_MODE);

    wdt_sta = mtk_wdt_check_status(); // This function will store the reset reason: Time out/ SW trigger, add MTK_WDT_MODE_AUTO_RESTART for check flashtool using HW reboot
		if ((wdt_sta == MTK_WDT_STATUS_HWWDT_RST)&&(rgu_mode&MTK_WDT_MODE_AUTO_RESTART)) { /* For E1 bug, that SW reset value is 0xC000, we using "==" to check */
    	/* Time out reboot always by pass power key */
    	/* Time out reboot always by pass power key */
    ...
    mtk_wdt_mode_config(TRUE, FALSE, FALSE, FALSE, FALSE); // Wirte Mode register will clear status register
