...
//<2012/09/13 yaotsulin-13914, Patch Hawk35 NFC and mt6628 driver
//<2012/08/24 yaotsulin-13289, add NFC pn544 driver
...
//>2012/08/24 yaotsulin-13289
//>2012/09/13 yaotsulin-13914

//<2014/05/08-Yuting Shih. Add for NXP NFC and mt6628 driver.
...
//<2014/05/06-samhuang, Fixed the GPIO123/126/127 mode set back from LK power off charging mode.
static void SET_WCDMA_PA(void);
//>2014/05/06-samhuang
...
/* MTK I2C                                                            */
/*=======================================================================*/
//<2012/09/13-Yuting Shih-yaotsulin-13914. Add/modified for NXP NFC PN544 and mt6628 driver
static struct pn544_i2c_platform_data   pn544_pdata =
{
    .irq_gpio   = GPIO73,   /* NFC_IRQ  */
    .ven_gpio   = GPIO201,  /* NFC_EN */
    .firm_gpio  = GPIO73,   /* NFC_FIRM, */
};

#if defined( HAWK40_EP0 ) || defined( HAWK40_EP1 ) || defined( HAWK40_EP2 )
static struct i2c_board_info __initdata i2c_devs0[]={
	{
		I2C_BOARD_INFO( I2C_ID_NAME, 0x28 ),
		.irq = GPIO17,
		.platform_data = &pn544_pdata,
	},
};
#elif defined( HAWK35_EP0 ) || defined( HAWK35_DTV_EP1 )
...
#endif
//>2012/09/13-Yuting Shih-yaotsulin-13914.
...
/*=======================================================================*/
//<EricHsieh-2013/4/23, Fixed the GPIO123/126/127 mode set back from LK power off charging mode.
static void SET_WCDMA_PA(void){

	//printk("==>Mode of [GPIO123,126,127]= [%d,%d,%d]\n",mt_get_gpio_mode(GPIO123),mt_get_gpio_mode(GPIO126),mt_get_gpio_mode(GPIO127));
	if(mt_get_gpio_mode(GPIO123)!=1){
		//printk("Set GPIO123 to Mode one\n");
		mt_set_gpio_mode(GPIO123, GPIO_MODE_01);
	}
	if(mt_get_gpio_mode(GPIO126)!=1){
		//printk("Set GPIO126 to Mode one\n");
		mt_set_gpio_mode(GPIO126, GPIO_MODE_01);
	}
	if(mt_get_gpio_mode(GPIO127)!=1){
		//printk("Set GPIO127 to Mode one\n");
...
//<2014/05/06-samhuang, porting Hawk's driver
#if defined( HAWK40_EP0 ) || defined( HAWK40_EP1 ) || defined( HAWK40_EP2 )
	i2c_register_board_info( I2C_BUS_ID, i2c_devs0, ARRAY_SIZE( i2c_devs0 ));
#elif defined( HAWK35_EP0 ) || defined( HAWK35_DTV_EP1 )
	i2c_register_board_info( I2C_BUS_ID, i2c_devs1, ARRAY_SIZE( i2c_devs1 ));
#else
