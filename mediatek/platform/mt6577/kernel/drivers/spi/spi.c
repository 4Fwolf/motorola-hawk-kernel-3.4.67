...
/*EVB the clock frequency is 130MHz, may be is reason that ldvt env.
  ...
  */
#define SPI_CLOCK_PERIED 130000  //kHz

#define SPI_REC_MSG_MAX 1000
#define SPI_REC_NUM 100
#define SPI_REC_STR_LEN 1024
...
DECLARE_WORK(mt_spi_msgdone_workqueue,(void *)mt_spi_msgdone_handler);
...
transfer_dma_unmapping(struct mt_spi_t *ms,struct spi_transfer *xfer )
{
	...
	//if((xfer->tx_dma != INVALID_DMA_ADDRESS) && (xfer->tx_dma != NULL)){ 
	if((xfer->tx_dma != INVALID_DMA_ADDRESS) && (xfer->tx_dma != (dma_addr_t)NULL)){ 
		dma_unmap_single(dev, xfer->tx_dma, xfer->len, DMA_TO_DEVICE);
		SPI_INFO(dev,"tx_dma:0x%x,unmapping\n",xfer->tx_dma);
		xfer->tx_dma = INVALID_DMA_ADDRESS;
	}
	//if((xfer->rx_dma != INVALID_DMA_ADDRESS) && (xfer->rx_dma != NULL)){
	if((xfer->rx_dma != INVALID_DMA_ADDRESS) && (xfer->rx_dma != (dma_addr_t)NULL)){
...
static int mt_spi_setup ( struct spi_device *spidev )
{
	...
	if ( !chip_config ) {
		...
		SPI_DBG("device %s: set default at chip's runtime state\n", 
			dev_name ( &spidev->dev ) );
//<2012/11/14-16962-Cokychen, [8317][DTV] Change SPI setting for DTV EP1
		#if defined(ARIMA_PROJECT_HAWK35_DTV)
			chip_config->setuptime = 10; // 3;
			chip_config->holdtime = 10; // 3;
			chip_config->high_time = 10; //20;  // pair  (20, 20) --3.25MHz in MT6575
			chip_config->low_time = 10; //20;
			...
			chip_config->com_mod = DMA_TRANSFER; //FIFO_TRANSFER;
			chip_config->pause = 0;
