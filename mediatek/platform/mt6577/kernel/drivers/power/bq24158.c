...
kal_uint32 bq24158_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	bq24158_read_byte(RegNum, &bq24158_reg);
	BQ24158DB("[bq24158_config_interface] Reg[%x]=0x%x\n", RegNum, bq24158_reg);
	
	bq24158_reg &= ~(MASK << SHIFT);
	bq24158_reg |= (val << SHIFT);

	bq24158_write_byte(RegNum, bq24158_reg);
	BQ24158DB("[bq24158_config_interface] write Reg[%x]=0x%x\n", RegNum, bq24158_reg);

	// Check
	bq24158_read_byte(RegNum, &bq24158_reg);
	BQ24158DB("[bq24158_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24158_reg);
...
void bq24158_dump_register(void)
{
    ...
    for (i=0;i<bq24158_REG_NUM;i++)
	{
        ...
        BQ24158DB("[bq24158_dump_register] Reg[0x%X]=0x%X\n", i, bq24158_reg[i]);        
    }
}
...
{	
    ...
    BQ24158DB("[bq24158_hw_init] (0x06,0x77)\n");
	bq24158_config_interface_reg(0x06,0x78); // set ISAFE and HW CV point (4.34) //MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
#else
    BQ24158DB("[bq24158_hw_init] (0x06,0x70) \n");
...
{             
    ...
    BQ24158DB("[bq24158_driver_probe] \n");
...
{
	BQ24158DB("[show_bq24158_access] 0x%x\n", g_reg_value_bq24158);
...
static ssize_t store_bq24158_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	BQ24158DB("[store_bq24158_access] \n");
	
	if(buf != NULL && size != 0)
	{
		BQ24158DB("[store_bq24158_access] buf is %s and size is %d \n",buf,size);
		reg_address = simple_strtoul(buf,&pvalue,16);
		
		if(size > 3)
		{		
			reg_value = simple_strtoul((pvalue+1),NULL,16);		
			BQ24158DB("[store_bq24158_access] write bq24158 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
		...
		{	
			...
			BQ24158DB("[store_bq24158_access] read bq24158 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq24158);
			BQ24158DB("[store_bq24158_access] Please use \"cat bq24158_access\" to get value\r\n");
		}		
	}	
	return size;
}
...
{	
	...
	BQ24158DB("******** bq24158_user_space_probe!! ********\n" );
...
{	
	...
	BQ24158DB("[bq24158_init] init start\n");

	i2c_register_board_info(BQ24158_BUSNUM, &i2c_bq24158, 1);

	if(i2c_add_driver(&bq24158_driver)!=0)
	{
		BQ24158DB("[bq24158_init] failed to register bq24158 i2c driver.\n");
	}
	else
	{
		BQ24158DB("[bq24158_init] Success to register bq24158 i2c driver.\n");
	}

	// bq24158 user space access interface
	ret = platform_device_register(&bq24158_user_space_device);
    if (ret) {
		BQ24158DB("****[bq24158_init] Unable to device register(%d)\n", ret);
		return ret;
    }	
    ret = platform_driver_register(&bq24158_user_space_driver);
    if (ret) {
		BQ24158DB("****[bq24158_init] Unable to register driver (%d)\n", ret);
		return ret;
    }
