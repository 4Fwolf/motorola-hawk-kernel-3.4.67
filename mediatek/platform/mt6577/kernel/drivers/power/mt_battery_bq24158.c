...
//<2012/10/26-15776-jessicatseng, [Hawk 4.0] To fix sometimes get the wrong charger/USB type
#if defined(RECHECK_CHARGER_TYPE)    	
extern CHARGER_TYPE hw_charger_type_detection(void);
static int g_recheck_charger_type = 0;
#endif
//>2012/10/26-15776-jessicatseng
///////////////////////////////////////////////////////////////////////////////////////////
...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount)
{
	...
	{
        //<2012/10/23-tedwu, enable delay if bypassing 100nF on pin BATON for ESD issue was mounted.
        // TODO: check hardware revision then enable it.
        #if 0
        //#if defined(HAWK35_FT0) || defined(HAWK35_FT1) ...
        msleep(20);
        //gpt_busy_wait_us(20000);
        //#endif
        #endif
        //>2012/10/23-tedwu
...
//// Global Variable
///////////////////////////////////////////////////////////////////////////////////////////
//<2012/12/14-18780-jessicatseng, [Hawk 4.0][BU2SC00139453] MS battery level change error (E.G. from 15% change to52%)
#if defined(ARIMA_PROJECT_HAWK40)
CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
#else
static CHARGER_TYPE CHR_Type_num = CHARGER_UNKNOWN;
#endif
//>2012/12/14-18780-jessicatseng
...
//<<EricHsieh,2013/2/21. For second steop charging process.
int g_HW_Charging_Step2 = 0;
//>>EricHsieh,2013/2/21. For second steop charging process.
...
//<2012/10/16-15225-jessicatseng, Add "energy full" for battery's property
    POWER_SUPPLY_PROP_ENERGY_FULL
//>2012/10/16-15225-jessicatseng
};
...
//<2012/11/12-16768-kevincheng,factory cable detection
#if defined(MOTO_CABLE_DETECTION)
        case POWER_SUPPLY_PROP_SPEC_CABLE:
             val->intval = data->IS_FACTORY_CABLE;
            break;
#endif
//>2012/11/12-16768-kevincheng
	default:
...
//<2012/11/13-tedwu, change to use BATTERY_PACK_HW4X.
...
//<2012/11/28-17836-kevincheng,Disable charging when FC in
#if defined(MOTO_CABLE_DETECTION)
    if(( upmu_is_chr_det() == KAL_TRUE ) && (g_factory_cable_id == 0))
#else
	if( upmu_is_chr_det() == KAL_TRUE )
#endif
//>2012/11/28-17836-kevincheng
	{		 
	...
	//<<MaoyiChou,2013/1/20, Add debug for charging issue picture
	if (Enable_BATDRV_LOG >= 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "Mt_battery_bq24158.c::mt6329_ac_update(BMT_status.charger_type=%d, ac_data->AC_ONLINE=%d, ac_psy->type=0x%x)\n", BMT_status.charger_type, ac_data->AC_ONLINE, ac_psy->type);
	}
	//>>MaoyiChou,2013/1/20, Add debug for charging issue picture
...
#if defined(MOTO_CABLE_DETECTION)
	if (Enable_BATDRV_LOG >= 1) {
    	printk(">>>> [mt_battery_bq24158] usb update , g_factory_cable_id : %d\n",g_factory_cable_id);
	}
    if(g_factory_cable_id)
    {
       usb_data->IS_FACTORY_CABLE = 1;
    }
    else
    {
       usb_data->IS_FACTORY_CABLE = 0;
    }
#endif
//>2012/11/12-16768-kevincheng
	//<<MaoyiChou,2013/1/20, Add debug for charging issue picture
	if (Enable_BATDRV_LOG >= 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "Mt_battery_bq24158.c::mt6329_usb_update(BMT_status.charger_type=%d, usb_data->USB_ONLINE=%d, usb_psy->type=0x%x)\n", BMT_status.charger_type, usb_data->USB_ONLINE, usb_psy->type);
	}
	//>>MaoyiChou,2013/1/20, Add debug for charging issue picture
...
//<2012/10/12-15016-jessicatseng, [Hawk 4.0]Fix battery's voltage is more than 4.11V, SOC will show 100%
            ...
            if((bq24158_status == 0x2) && (BMT_status.bat_full == KAL_TRUE))
//>2012/10/12-15016-jessicatseng
            {
            	/*Use no gas gauge*/
            ...
            else 
            {
                ...
                else
                {
					...
					{						
						if(bat_volt_check_point >= 100 )
						{					
							bat_volt_check_point=100; //MaoyiChou,2013/3/29, Fix battery stay 99%
							//BMT_status.SOC=99;
							gSyncPercentage=1;
    ...
    {
//<2012/11/23-17539-jessicatseng, Change battery's status to unknown when getting invalid battery ID
#if defined(CHK_BATTERY_ID__MOTO_ID)
	if((upmu_is_chr_det() == KAL_TRUE) && (g_Battery_Fail == KAL_TRUE) && (Is_Invalid_Battery() == KAL_FALSE))
	{
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_UNKNOWN;
	} 
	else	
...
int g_Get_I_Charging(void)
{
	...
	{
//<2012/12/04-18137-jessicatseng, [Hawk 4.0] ISense and BatSense voltage are got by average method for charging current
#if defined(ARIMA_PROJECT_HAWK40)
    	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*1000 / 82;
//<2013/01/02-tedwu, [Hawk3.5][Defect:BU2SC00142076] Pop screen show "your battery is over current protection".
    	...
    	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*10/1;
//>2013/01/02-tedwu
#else		
    	ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*10/R_CURRENT_SENSE;
#endif    	
//>2012/12/04-18137-jessicatseng
	}
	else
	{
    	ICharging = 0;
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if(gFG_can_reset_flag == 1)
	{
        ...
        {
            if (Enable_BATDRV_LOG >= 1) {
            	printk("[BATTERY] double check Battery charging full voltage !!\n");                
            }
            BMT_status.bat_full = KAL_FALSE;    
            //g_bat_full_user_view = KAL_FALSE;  //mtk71259 20120703  20120707
	...
	fg_qmax_update_for_aging();  //mtk71259 20120704
	...
	//<<EricHsieh,2013/2/21. For second steop charging process.
	bat_volt_check_point = BMT_status.SOC;
	if(g_HW_Charging_Step2==1)
		g_HW_Charging_Step2=2; //step 2 process done.
	//>>EricHsieh,2013/2/21. For second steop charging process.
...
PMU_STATUS BAT_BatteryPartialModeAction(void)
{
	...
	else
	{
		...
		if((BMT_status.bat_vol < PARTIAL_MAX_VOLTAGE)&&(BMT_status.bat_partial_full == KAL_FALSE))
		{
			/*  Enable charger */
			pchr_turn_on_charging_bq24158();
		}
...
//<2012/11/13-16848-jessicatseng, [Hawk 3.5] Don't enlarge the range of charging current checked
#if defined(ARIMA_PROJECT_HAWK40)
		//if(BMT_status.ICharging > 1000)
		if( (BMT_status.ICharging > 2000) &&
...
#elif defined(ARIMA_PROJECT_HAWK35)
		//if(BMT_status.ICharging > 1000)
		if( (BMT_status.ICharging > 2000) &&  // Set 2000 by refering "Rapid Charge Requirement"?.
			(BMT_status.total_charging_time > 300)
			)
		//if(BMT_status.ICharging > 200) //test
//>2013/01/02-tedwu
...
//<2012/11/23-17541-jessicatseng, Modify battery's OVP to 4.425V
		if(BMT_status.bat_vol > 4425/*4350*/)
		//if(BMT_status.bat_vol > 3800) //test
//>2012/11/23-17541-jessicatseng
		{
...
//<2012/10/18-15340-jessicatseng, Show warning dialog when getting "invalid battery" and "partial charging" in power on mode
		...
		if(upmu_is_chr_det() == KAL_TRUE)
		{		
			if(Is_Invalid_Battery() == KAL_FALSE)
			{
				g_BatteryNotifyCode |= 0x0020;
				if (Enable_BATDRV_LOG == 1) {
				xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Invalid Battery\n");
				}
			}
		}	
...
#if defined(MOTO_SOP_0296_PARTIAL_CHARGE)
		if((upmu_is_chr_det() == KAL_TRUE) && 
			(BMT_status.temperature >= PARTIAL_CHARGE_MIN_TEMPERATURE) && 
			(BMT_status.temperature < PARTIAL_CHARGE_MAX_TEMPERATURE))
		{
//<2013/01/28-AlbertWu,remove high temperature message event.
			//g_BatteryNotifyCode |= 0x0040;
//>2013/01/28-AlbertWu
			if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] bat_temp(%d)\n", BMT_status.temperature);
			}
		}
		
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] BATTERY_NOTIFY_CASE_0007 (%x)\n", 
				g_BatteryNotifyCode);
		}	
#endif
...
//<2012/11/28-17836-kevincheng,Disable charging when FC in
#if defined(MOTO_CABLE_DETECTION)
	if(g_factory_cable_id == 0)
		{
#endif
bq24158_config_interface_reg(0x04,0x80);
//<2012/11/22-17489-jessicatseng, [Hawk 4.0] Charger IC is enabled / disabled by GPIO54 for EP2
...
void bq24158_set_ac_current(void)
{
    ...
    bq24158_config_interface_reg(0x04,0x62); // 782mA  //MaoyiChou,2013/1/30, [Hawk35]Charging full on off mode and then plug out charger to power on, fixed battery capacity not full .
    bq24158_config_interface_reg(0x05,0x04);
    #endif
    //>2012/09/16-tedwu
...
//<2012/09/28-14469-jessicatseng, [Hawk 4.0] Modify charging current for JB
    ...
    bq24158_config_interface_reg(0x02,0xAE); //MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
    bq24158_config_interface_reg(0x04,0x73); //cut off current 166mA,//MaoyiChou,2013/3/29, Fix battery stay 99%
    bq24158_config_interface_reg(0x05,0x04);
    #endif
...
//<<EricHsieh,2013/2/21. For second steop charging process.
void bq24158_set_ac_current_Step2(void)
{
	if (Enable_BATDRV_LOG == 1) {
		printk("[BATTERY:bq24158] bq24158_set_ac_charging_current_2 \r\n");	
	}	
...
void select_charging_curret_bq24158(void)
{
	...
	else if (g_ftm_battery_flag) 
	{
		...
		{			
//<2012/11/13-16844-jessicatseng, [Hawk 4.0] Fix USB charging current is wrong when USB cable plugged in the second time
#if defined(ARIMA_PROJECT_HAWK40)
			bq24158_config_interface_reg(0x01,0x48);
			bq24158_config_interface_reg(0x04,0x23); //cut off current 166mA,//MaoyiChou,2013/3/29, Fix battery stay 99%
			bq24158_config_interface_reg(0x05,0x04);
#else
			bq24158_config_interface_reg(0x01,0x78);
#endif			
//>2012/11/13-16844-jessicatseng
...
//<<EricHsieh,2013/2/21. For second steop charging process.
void select_charging_curret_bq24158_Step2(void)
{
    if( g_temp_CC_value == Cust_CC_0MA)
	{
		pchr_turn_off_charging_bq24158();
...
#if defined(ARIMA_PROJECT_HAWK40)
			bq24158_config_interface_reg(0x01,0x48);
			bq24158_config_interface_reg(0x04,0x21); //MCH_CUT_OFF_CUR_34mA
			bq24158_config_interface_reg(0x05,0x04);
#else
			bq24158_config_interface_reg(0x01,0x78);
#endif			
...
void ChargerHwInit_bq24158(void)
{
	...
	if(temp_init_flag==0)
	{
        ...
        bq24158_config_interface_reg(0x01,0x88); // 800mA current limit, 3.4V fast charge
        bq24158_config_interface_reg(0x02,0xAE);//MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
        bq24158_config_interface_reg(0x04,0x62/*0x64*/); // 782mA  //MaoyiChou,2013/1/30, [Hawk35]Charging full on off mode and then plug out charger to power on, fixed battery capacity not full .
        bq24158_config_interface_reg(0x05,0x04);
//<2012/09/28-14469-jessicatseng, [Hawk 4.0] Modify charging current for JB
...
//<<EricHsieh,2013/2/21. For second steop charging process.
void ChargerHwInit_bq24158_Step2(void)
{
	if (Enable_BATDRV_LOG == 1) {
		printk("[mt6329 BAT_probe] ChargerHwInit_2\n" );
	}
	...
	if(temp_init_flag==0)
	{
        ...
        bq24158_config_interface_reg(0x01,0x88); // 800mA current limit, 3.4V fast charge
        bq24158_config_interface_reg(0x02,0xAE);//MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
        bq24158_config_interface_reg(0x04,0x61/*0x64*/); // 782mA  //MaoyiChou,2013/1/30, [Hawk35]Charging full on off mode and then plug out charger to power on, fixed battery capacity not full .
        bq24158_config_interface_reg(0x05,0x04);
//<2012/09/28-14469-jessicatseng, [Hawk 4.0] Modify charging current for JB
        #elif defined(ARIMA_PROJECT_HAWK40)
	    bq24158_config_interface_reg(0x06,0x78); //set ISAFE  //MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
	    ...
	    bq24158_config_interface_reg(0x01,0x88);
	    bq24158_config_interface_reg(0x02,0xAE);//MaoyiChou,2013/2/1, Changed target voltage from 4.34v to 4.36v
	    bq24158_config_interface_reg(0x04,0x71); //set IOCHARGE  //MCH_CUT_OFF_CUR_34mA
	    bq24158_config_interface_reg(0x05,0x04);
//>2012/09/28-14469-jessicatseng        
        #else
	    bq24158_config_interface_reg(0x06,0x70);
...
//>2012/08/09-kylechang
	...
	else
	{
		...
		{
//<2012/12/04-18194-jessicatseng, [Hawk 4.0] Don't charge the battery until fuel gauge initialize done
			#if defined(ARIMA_PROJECT_HAWK40)
			pchr_turn_off_charging_bq24158();
			#endif
//>2012/12/04-18194-jessicatseng
...
//<<EricHsieh,2013/2/21. For second steop charging process.
void pchr_turn_on_charging_bq24158_step2(void)
{
#if 0
    mt_set_gpio_mode(gpio_number,gpio_on_mode);  
    mt_set_gpio_dir(gpio_number,gpio_on_dir);
...
//>2012/08/09-kylechang
	...
	{
		ChargerHwInit_bq24158_Step2();
	
		if (Enable_BATDRV_LOG == 1) {
			printk("[BATTERY:bq24158] pchr_turn_on_charging_2 !\r\n");
		}

		select_charging_curret_bq24158_Step2();
...
//<2012/11/19-tedwu, [Hawk 3.5] Provide the data of charging current for ATS
//<2012/10/24-15637-jessicatseng, [Hawk 4.0] Provide the data of charging current for ATS 
#if defined(ARIMA_PROJECT_HAWK40)
//<2012/12/04-18137-jessicatseng, [Hawk 4.0] ISense and BatSense voltage are got by average method for charging current
...
//>2012/11/19-tedwu
   ...
   //-----------------------------------------------------------------------------
    ...
    if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:ADC:bq24158] VCHR:%d BAT_SENSE:%ld I_SENSE:%ld TBAT:%d BMT_status.ICharging:%ld (%d)\n", 
            BMT_status.charger_vol, BMT_status.ADC_BAT_SENSE, BMT_status.ADC_I_SENSE, BMT_status.temperature, BMT_status.ICharging, TBAT_OVER_CRITICAL_LOW);
    }
...
//<2012/10/12-15016-jessicatseng, [Hawk 4.0]Fix battery's voltage is more than 4.11V, SOC will show 100%
		...
		BMT_status.SOC = fgauge_read_capacity_by_v();
...
//>2012/10/12-15016-jessicatseng
	    ...
	    if (!batteryBufferFirst)
	    {
	        ...
	        for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {
				...
				batteryTempBuffer[i] = BMT_status.temperature;  //add here
	        }
	...
	if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:AVG:bq24158] BatTemp:%d Vbat:%ld VBatSen:%ld SOC:%ld ChrDet:%d Vchrin:%d Icharging:%ld ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG >= 1) {		
		printk("[BATTERY:FG:bq24158] %d,%ld,%ld,%ld,%d,%d,%ld,%d,%d,%d,%d\r\n", 
       	BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
    ...
    if(BAT_status != PMU_STATUS_OK)
    {
		//<<MaoyiChou,2013/1/20, Add debug for charging issue picture
		if (Enable_BATDRV_LOG >= 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Protection Check=> BAT_status=%d \n", BAT_status);
		}
		//>>MaoyiChou,2013/1/20, Add debug for charging issue picture
        return PMU_STATUS_FAIL;                  
    }
...
//<2012/10/05-14765-jessicatseng, [Hawk 4.0] Enable to check MOTO battery ID in power on mode
	...
	{	
		//<<MaoyiChou,2013/1/20, Add debug for charging issue picture
		if (Enable_BATDRV_LOG >= 1) {
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] MOTO battery check fail!! \n");
		}
		//>>MaoyiChou,2013/1/20, Add debug for charging issue picture
...
//<2012/10/09-14860-jessicatseng, [Hawk 4.0] Integrate SOP-0296 partial charging
	...
	if((BMT_status.temperature >= PARTIAL_CHARGE_MIN_TEMPERATURE) &&
		 (BMT_status.temperature < PARTIAL_CHARGE_MAX_TEMPERATURE))
	{
		printk("[BATTERY] Enter partial charging BMT_status.temperature = %d\n", BMT_status.temperature);
...
//>2012/10/19-15385-jessicatseng	
    ...
    {   
		//<<EricHsieh,2013/2/21. For second steop charging process.
		if(g_HW_Charging_Step2==1)
			pchr_turn_on_charging_bq24158_step2();	//mtk71259   20120707
        else
			pchr_turn_on_charging_bq24158();
		//>>EricHsieh,2013/2/21. For second steop charging process.
...
//<2012/10/12-15016-jessicatseng, [Hawk 4.0]Fix battery's voltage is more than 4.11V, SOC will show 100%
		...
		if((BMT_status.bat_full) && (g_HW_Charging_Done == 1) && (!g_Battery_Fail))	
        {
			bq24158_status = bq24158_get_chip_status();
			...
			//<<EricHsieh,2013/2/21. For second steop charging process.
			if (BMT_status.SOC <= 99 || BMT_status.bat_vol <= 4340 || g_HW_Charging_Step2 == 0)
			{
            	if (Enable_BATDRV_LOG >= 1) {
            		printk("[BATTERY] Capacity = %d%, bat_vol=%d,g_HW_Charging_Step2=%d  Battery Re-charging !!\n",BMT_status.SOC,BMT_status.bat_vol,g_HW_Charging_Step2);                
            	}
							
				pchr_turn_off_charging_bq24158();
				bq24158_config_interface_reg(0x04,0x81/*0x64*/); //reset charger ic
				//pchr_turn_on_charging_bq24158();
				pchr_turn_on_charging_bq24158_step2();
            	BMT_status.bat_full = KAL_FALSE;    
            	g_bat_full_user_view = KAL_TRUE;  //mtk71259  20120630
				//BMT_status.bat_charging_state = CHR_PRE;
...
//<2012/10/23-15566-jessicatseng, Fix battery's capacity shows 100% if phone boots with charger and battery's voltage is over 4.11V
int boot_check_once=0;
//>2012/10/23-15566-jessicatseng
...
void BAT_thread_bq24158(void)
{
   ...
   // bq24158_config_interface_reg(0x01,0x30);
    bq24158_config_interface_reg(0x02,0xb);  //add here 
	bq24158_config_interface_reg(0x02,0xa);
	#endif 
    ...
    if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //<2013/02/05-tedwu, for Motorola SOP 0503:Discharge in high temperature
...
//<2012/11/12-16768-kevincheng,factory cable detection
#if defined(MOTO_CABLE_DETECTION)
if((g_factory_cable_id == 1)&&(upmu_is_chr_det() == KAL_FALSE))
  {
   printk(">>>> [mt_battery_bq24158] MOTO factory cable plug out then power down\n");
   mt_power_off();
  }
#endif
//>2012/11/12-16768-kevincheng  
...
//<2012/10/26-15776-jessicatseng, [Hawk 4.0] To fix sometimes get the wrong charger/USB type
#if defined(RECHECK_CHARGER_TYPE)    	
		else if((BMT_status.charger_type == NONSTANDARD_CHARGER) && (g_recheck_charger_type == 0))
		{
			printk("[BATTERY_TOP] recheck charger type start\n");
			g_recheck_charger_type = 1;
...
//>2012/10/26-15776-jessicatseng
	...
	//<<MaoyiChou,2013/1/20, Add debug for charging issue picture
	if (Enable_BATDRV_LOG >= 1) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Check Battery Status=> BAT_status=%d, BMT_status.bat_charging_state=0x%x \n", BAT_status, BMT_status.bat_charging_state);
	}
	//>>MaoyiChou,2013/1/20, Add debug for charging issue picture
...
//>2012/10/09-14860-jessicatseng
		...
		if( bq24158_status == 0x2 )
		{
			...
			//BMT_status.bat_full = KAL_TRUE; //mtk71259  bat_full may not be true
		   // BMT_status.total_charging_time = 0;
			//BMT_status.PRE_charging_time = 0;
			//BMT_status.CC_charging_time = 0;
			//BMT_status.TOPOFF_charging_time = 0;
		...
		//<<EricHsieh,2013/2/21. For second steop charging process.
		if(g_HW_Charging_Step2==0)
			pchr_turn_on_charging_bq24158();
		else if(g_HW_Charging_Step2==1)
			pchr_turn_on_charging_bq24158_step2();
		//>>EricHsieh,2013/2/21. For second steop charging process.
				
		if (Enable_BATDRV_LOG >= 1) {
    		printk("[BATTERY:bq24158] Total charging timer=%ld, bq24158_status=%d \n", 
...
void pmu_init_for_bq24158(void)
{
	printk("[pmu_init_for_bq24158] \n");
}

//<2012/11/12-16768-kevincheng,factory cable detection
#if defined(MOTO_CABLE_DETECTION)
void usb_is_factory_cable_init(void)
{
    if(g_factory_cable_id)
       mt6329_usb_main.IS_FACTORY_CABLE = 1;
    else
       mt6329_usb_main.IS_FACTORY_CABLE = 0;

    printk(">>>> [mt_battery_bq24158] cable init , g_factory_cable_id : %d\n",g_factory_cable_id);
}
#endif
//>2012/11/12-16768-kevincheng  
