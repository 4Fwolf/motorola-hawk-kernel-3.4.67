...
//<2012/10/05-14776-jessicatseng, [Hawk 4.0] The control signal of "red" LED is inverse
...
#if defined(ARIMA_PROJECT_HAWK40)
	if((pmic_type != MT65XX_LED_PMIC_NLED_ISINK1 && pmic_type != MT65XX_LED_PMIC_NLED_ISINK2 && pmic_type != MT65XX_LED_PMIC_NLED_ISINK3) || led->nled_mode != NLED_BLINK) {
		return -1;
	}
#elif defined(HAWK35_DTV_EP1)
	if((pmic_type != MT65XX_LED_PMIC_NLED_ISINK4 && pmic_type!= MT65XX_LED_PMIC_NLED_ISINK5 && pmic_type!= MT65XX_LED_PMIC_NLED_ISINK1) || led->nled_mode != NLED_BLINK) {
		return -1;
	}
#else		
...
//<2012/09/28-14478-jessicatseng, [Hawk 4.0] Porting RGB LED for JB	
			...
			case MT65XX_LED_PMIC_NLED_ISINK1:
				upmu_isinks_ch1_mode(PMIC_PWM_1);
				upmu_isinks_ch1_step(0);
			...
			case MT65XX_LED_PMIC_NLED_ISINK2:
				upmu_isinks_ch2_mode(PMIC_PWM_2);
				upmu_isinks_ch2_step(0);
			...
			case MT65XX_LED_PMIC_NLED_ISINK3:	
				upmu_isinks_ch3_mode(PMIC_PWM_2);
				upmu_isinks_ch3_step(0);
			...
			case MT65XX_LED_PMIC_NLED_ISINK1:
				...
				upmu_top2_bst_drv_ck_pdn(0x0);
				hwBacklightISINKTurnOn(1);
				hwBacklightFlashlightTuning(0x4, 0x0);//0xb;
...
//<2014/04/05-samhuang, porting hawk's driver
#if defined(HAWK35_EP0) || defined(HAWK35_DTV_EP0) || defined(HAWK35_DTV_EP1)
	if(pwm_num == PWM1)
	{
		mt_set_gpio_mode(GPIO_PWM_1_PIN,GPIO_PWM_1_PIN_M_PWM);
		if((level > 0) && (level < 256))
		{
			...
			if(level == 255)
			{
				...
				pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA1 = 0xFFFFFFFF;///0x01FFFFFF;		
	...
	if(pwm_num == PWM1)
	{
		//<2012/10/17-15286-jessicatseng, [Hawk 4.0] Set maximum brightness of LCM's backlight for PWM duty 100
		#if !(defined(HAWK40_PRE_EP0) || defined(HAWK40_PRE_EP1))
		mt_set_gpio_mode(GPIO_PWM_1_PIN,GPIO_PWM_1_PIN_M_PWM);
		#endif	
		if((level > 0) && (level < 256))
		{
			...
			else
			{
				...
				{
					pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA0 =  0xFFFFFFFF ;
					pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA1 = (1 << (level - 32)) - 1;
				}		
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
    ...
    //<2012/09/07 Sherman Wei,Add for Notification
    else if (pmic_type == MT65XX_LED_PMIC_FLASH)
    {
        if (level)
        {
            hwBacklightFlashlightTuning(0x1F, 0x0);//0xb;
            hwBacklightFlashlightTurnOn();
        }
        else
        {
...
//<2012/09/28-14478-jessicatseng, [Hawk 4.0] Porting RGB LED for JB	
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		if (level) 
		{
			hwBacklightISINKTuning(1, 3, 0x0, 0);
			upmu_top2_bst_drv_ck_pdn(0x0);
			hwBacklightISINKTurnOn(1);
		}
		else 
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK2)
	{
		if (level) 
		{
			hwBacklightISINKTuning(2, 3, 0x0, 0);
			upmu_top2_bst_drv_ck_pdn(0x0);
			hwBacklightISINKTurnOn(2);
		}
		else 
...
//<2012/11/19-17190-jessicatseng, [Hawk 4.0] Add ISINK3 function
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
	{
		if (level) 
		{
			hwBacklightISINKTuning(3, 3, 0x0, 0);
			upmu_top2_bst_drv_ck_pdn(0x0);
			hwBacklightISINKTurnOn(3);
		}
		else 
...
//>2012/11/19-17190-jessicatseng
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		if (level) 
		{
			...
			upmu_top2_bst_drv_ck_pdn(0x0);
			hwBacklightISINKTurnOn(1);
//<2012/11/06 ShermanWei, NFC hawk35HW temp solution for PMIC-Flash control
			hwBacklightFlashlightTuning(0x4, 0x0);//0xb;
			hwBacklightFlashlightTurnOn();
//>2012/11/06 ShermanWei
		}
		else 
		{
			hwBacklightISINKTurnOff(1);
...
//<2012/09/28-14478-jessicatseng, [Hawk 4.0] Porting RGB LED for JB	
#if defined(ARIMA_PROJECT_HAWK40)
			else if((led_data->cust.mode == MT65XX_LED_MODE_PMIC) && (led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK1
				...
				|| led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK5 || led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK1))
#else				
			else if((led_data->cust.mode == MT65XX_LED_MODE_PMIC) && (led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK4
...
//>2012/09/28-14478-jessicatseng					
	...
	//<2012/10/04 Sherman Wei,Add for bug that missed call->plug USB->unplug USB, LED will not flash/light
	#if defined (HAWK35_EP0) || (ARIMA_PROJECT_HAWK35) || (ARIMA_PROJECT_ARGONMINI_6577) || (ARIMA_PROJECT_ARGONMINI_6575)
	else
	{
	///	LEDS_DEBUG("[LED]--mt65xx_blink_set\n");
