...
#endif
char test_kdump[]={6,5,7,7,'k','d','u','m','p','t','e','s','t'};
//==============
...
static unsigned int simp_mmc_power_up(struct simp_mmc_host *host,bool on)
{
    switch(host->mtk_host->id){
        case 0:
        		//simp_msdc_ldo_power(on, MT65XX_POWER_LDO_VMC, VOL_1800); 
            break;
        case 1:
				    simp_msdc_ldo_power(on, MT65XX_POWER_LDO_VGP, VOL_3300);
#ifdef HW_LAYOUT_EVB
#ifndef MTK_EMMC_SUPPORT
				    simp_msdc_ldo_power(on, MT65XX_POWER_LDO_VMC, VOL_3300);
#endif
#endif   
				    simp_msdc_ldo_power(on, MT65XX_POWER_LDO_VMCH, VOL_3300); 
            break;
...
#else
   enable_clock(PERI_MSDC0_PDN + host->mtk_host->id, "SD");
#endif 
    return SIMP_SUCCESS;
}
...
/* support PANIC dump info to emmc card
...
static int simp_mmc_init(bool boot)
{
    ...
    if(boot){
            ...
            pmmc_extend_host->mtk_host->id       = 1; 
