...
extern const int using_class_ab_amp;	//from libaudiosetting.so, related to USING_CLASS_AB_AMP
...
status_t mATVAudioPlayer::createOutputTrack() {
    ...
    if (matv_use_analog_input == 1)
    {
      ...
      if (mAudioSink->open(MATV_AUDIO_SAMPLING_RATE, MATV_AUDIO_CHANNEL_NUM, AUDIO_CHANNEL_OUT_STEREO, AUDIO_FORMAT_PCM_16_BIT, 3) != NO_ERROR) {
    ...
    {
      int samplerate = I2SSampleRate(mI2Sdriver);
      SXLOGD("[%d]Create AudioTrack object: rate=%d, channels=%d\n",mI2Sid, samplerate , MATV_AUDIO_CHANNEL_NUM);
      if (mAudioSink->open(samplerate , MATV_AUDIO_CHANNEL_NUM, AUDIO_CHANNEL_OUT_STEREO, AUDIO_FORMAT_PCM_16_BIT, 3) != NO_ERROR) {
        SXLOGE("mAudioSink open failed");
        return ERROR_OPEN_FAILED;
       }
    }
    ...
    while (1) {
...

        {
            ...
            if (firstOutput &&(numread>0)) {               
               ...
               ///SXLOGD("before first read %d, %d", numread,  bufSize);
               firstOutput = false;
            }
