...
static bool nand_check_bch_error(u8 * pDataBuf, u32 u4SecIndex, u32 u4PageAddr)
{
    ...
    if (0 != (u4ErrorNumDebug & 0xFFFF))
    {
        for (i = 0; i <= u4SecIndex; ++i)
        {
            ...
            if (0xF == u4ErrNum)
            {
                MSG(ERR, " UnCorrectable at PageAddr=%d, Sector=%d\n", u4PageAddr, i);
                bRet = false;
            } else
            {
                MSG(ERR, " Correct %d at PageAddr=%d, Sector=%d\n", u4ErrNum, u4PageAddr, i);
            }
        }
    }
#else
...
int nand_init_device(struct nand_chip *nand)
{
	...
	if(en_interrupt){
		...
		mt_irq_set_sens(MT6575_NFI_IRQ_ID, MT65xx_EDGE_SENSITIVE);
		mt_irq_set_polarity(MT6575_NFI_IRQ_ID, MT65xx_POLARITY_LOW);
		mt_irq_unmask(MT6575_NFI_IRQ_ID);
	}
