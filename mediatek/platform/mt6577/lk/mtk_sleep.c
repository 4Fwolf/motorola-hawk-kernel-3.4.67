...
{
    mt_irq_set_sens(MT6575_SLEEP_IRQ_ID, MT65xx_LEVEL_SENSITIVE);
    mt_irq_set_polarity(MT6575_SLEEP_IRQ_ID, MT65xx_POLARITY_LOW);
...
static void sc_go_to_sleep(u32 timeout, kal_bool en_deep_idle)
{
    ...
    mt_irq_mask_all(&mask); // mask all interrupts

    gpt_irq_ack();

    if (en_deep_idle)
    {
        mt_irq_unmask(MT6575_SLEEP_IRQ_ID);
    }
    else
    {
    ...
    if (en_deep_idle)
    {
        mt_irq_ack(MT6575_SLEEP_IRQ_ID);
    }

    mt_irq_mask_restore(&mask); // restore all interrupts
...
{
    unsigned int topmisc, pdn0, upllcon0;
    ...
    /* disable UPLL when entering WFI mode in sleep */
    upllcon0 = DRV_Reg16(UPLL_CON0);
    DRV_WriteReg16(UPLL_CON0, upllcon0 | (1 << 0));
    ...
    /* restore UPLL_CON0 power down */
    DRV_WriteReg16(UPLL_CON0, upllcon0);
