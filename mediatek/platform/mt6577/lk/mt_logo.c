...
void mt_disp_show_boot_logo(void)
{
    ...
    else
    {
        ///show_logo(0);
        init_fb_screen();
        fill_animation_logo(BOOT_LOGO_INDEX, mt_get_fb_addr(), mt_get_fb_addr() + 2 * phical_screen.fb_size, logo_addr, phical_screen);
...
void mt_disp_show_battery_capacity(UINT32 capacity)
{
    ...
    {     
        ...
        fill_animation_battery_by_ver(capacity, mt_get_fb_addr(), mt_get_fb_addr() + 2 * phical_screen.fb_size, logo_addr, phical_screen, show_animationm_ver);            
...
void mt_disp_show_charger_ov_logo(void)
{
    ...
    {
        init_fb_screen();
        fill_animation_logo(CHARGER_OV_INDEX, mt_get_fb_addr(), mt_get_fb_addr() + 2 * phical_screen.fb_size, logo_addr, phical_screen);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
...
void mt_disp_show_low_battery(void)
{
    ...
    else
    {
        init_fb_screen();
        //show_logo(2);
        fill_animation_logo(LOW_BATTERY_INDEX, mt_get_fb_addr(), mt_get_fb_addr() + 2 * phical_screen.fb_size, logo_addr, phical_screen);
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
