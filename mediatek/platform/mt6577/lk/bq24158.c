...
U32 bq24158_i2c_write (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    ...
    {
        BQ24158DB("[bq24158_i2c_write] exceed I2C FIFO length!! \n");
        return 0;
    }
...
kal_uint32 bq24158_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	BQ24158DB("[bq24158_read_interface] Reg[%x]=0x%x\n", RegNum, data);
	
	data &= (MASK << SHIFT);
	*val = (data >> SHIFT);
	
	BQ24158DB("[bq24158_read_interface] val=0x%x\n", *val);
...
void bq24158_read_register(int i)
{
    ...
    bq24158_i2c_read(chip_slave_address, &cmd, cmd_len, &data, data_len);
    bq24158_reg[i] = data;
    BQ24158DB("[bq24158_read_register] Reg[0x%X]=0x%X\n", i, bq24158_reg[i]);
}
...
//<2012/10/31-tedwu, Fix issue of removing then reinserting battery pack, will in "charge done" state on first few times.
