...
extern void	arch_sync_cache_range(unsigned int start, unsigned int len);
static BOOL _IsEngineBusy(void)
{
    LCD_REG_STATUS status;

	arch_sync_cache_range(*(unsigned int*)(&LCD_REG->STATUS), 32);
...
LCD_STATUS LCD_SelectWriteIF(LCD_IF_ID id)
{
    ...
    LCD_OUTREG16(&LCD_REG->WROI_CMD_ADDR, AS_UINT32(&cmd_addr));
    LCD_OUTREG16(&LCD_REG->WROI_DATA_ADDR, AS_UINT32(&dat_addr));
