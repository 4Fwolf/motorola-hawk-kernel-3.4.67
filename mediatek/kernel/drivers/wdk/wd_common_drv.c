...
//<2012/10/5-joshualee, the switching node of force download mode for MT6577
ssize_t force_dl_switch_attr_show(struct kobject *kobj, struct attribute *attr, char *buffer);
ssize_t force_dl_switch_attr_store(struct kobject *kobj, struct attribute *attr, const char *buffer, size_t size);
ssize_t force_dl_bit_show(struct kobject *kobj, char *page);
ssize_t force_dl_bit_store(struct kobject *kobj, const char *page, size_t size);

struct sysfs_ops force_dl_switch_sysfs_ops = {
    .show  = force_dl_switch_attr_show,
    .store = force_dl_switch_attr_store,
};
struct force_dl_switch_sys_entry {
    struct attribute attr;
    ssize_t (*show)(struct kobject *kobj, char *page);
    ssize_t (*store)(struct kobject *kobj, const char *page, size_t size);
};
static struct force_dl_switch_sys_entry f_dl_entry = {
    ...
    //{ .name = "f_dl_mode", .mode = 0666 },
    { .name = "f_dl_mode", .mode = 0664 },
    //>2012/11/15-16993-xiaoleiding
    force_dl_bit_show,
    force_dl_bit_store,
};
struct attribute *force_dl_attributes[] = {
    &f_dl_entry.attr,
    NULL,
};
struct kobj_type force_dl_switch_ktype = {
    .sysfs_ops = &force_dl_switch_sysfs_ops,
    .default_attrs = force_dl_attributes,

};
static struct force_dl_sysobj {
    struct kobject kobj;
}f_dl_sysobj;

int arima_f_dl_sysfs(void) 
{
    struct force_dl_sysobj *obj = &f_dl_sysobj;
    memset( &obj->kobj, 0x00, sizeof(obj->kobj));

    obj->kobj.parent = kernel_kobj;
    if ( kobject_init_and_add( &obj->kobj, &force_dl_switch_ktype  , NULL, "f_dl_bit") ) {
        kobject_put(&obj->kobj);
        return -ENOMEM;
    }
    kobject_uevent(&obj->kobj , KOBJ_ADD);
...
ssize_t force_dl_switch_attr_show(struct kobject *kobj, struct attribute *attr, char *buffer)
{
    struct force_dl_switch_sys_entry *entry = container_of(attr, struct force_dl_switch_sys_entry, attr );
    return entry->show(kobj, buffer);
}
ssize_t force_dl_switch_attr_store(struct kobject *kobj, struct attribute *attr, const char *buffer, size_t size)
{
...
ssize_t  force_dl_bit_show(struct kobject *kobj, char *buffer)
{
    int remain = PAGE_SIZE;
    int len;
    char *ptr = buffer;
...
ssize_t  force_dl_bit_store(struct kobject *kobj , const char *buffer, size_t size    )
{
     int control_value;
     int res = sscanf(buffer, "%x", &control_value );
     
     if ( res != 1 )  {
         printk("%s: expect 1 numbers\n", __FUNCTION__);
     }
     else {
         // reset the force download bit ( on/off ) based on the input
         if( control_value >=1 ) {
             // trigger the waiting status in preloader     
             (*(volatile unsigned int*)0xF0000000) = ((*(volatile unsigned int*)0xF0000000)& 0xFFFFFFDF)|0x00002200;
         }
         else {   // <=0
             // clear the waiting status in preloader
             (*(volatile unsigned int*)0xF0000000) = ((*(volatile unsigned int*)0xF0000000)| 0x00000020)|0x00002200;
         }       
     }
     return size;
}
