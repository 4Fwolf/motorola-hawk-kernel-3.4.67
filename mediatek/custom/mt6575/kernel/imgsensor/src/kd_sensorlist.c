...
#ifdef ARIMA_PROJECT_HAWK35_DTV
...
#ifdef ARIMA_PROJECT_HAWK35_DTV
static SENSOR_FUNCTION_STRUCT *g_pSensorFunc_backup = NULL;
...
int kdSetDriver(unsigned int* pDrvIndex)
{
	...
	printk("++++ Arima Kyle +++++kdSetDriver START\n");
    ...
    if (drvIdx < MAX_NUM_OF_SUPPORT_SENSOR)
    {
        ...
        {
            printk("ERROR:NULL g_pSensorFunc\n");
            return -EIO;
        }


		#ifdef ARIMA_PROJECT_HAWK35_DTV
		g_pSensorFunc_backup = g_pSensorFunc;
		printk("ynn: get g_bIsAtvStart value%d\n",g_bIsAtvStart);
		if(g_bIsAtvStart)
		{
			NewportTVSensorInit(&g_pSensorFunc);
			PK_DBG("ynn: NewportTVSensorInit\n");
			printk("ynn: NewportTVSensorInit ++++ Arima Kyle +++++\n");
		}
		#endif
	...
	printk("++++ Arima Kyle +++++kdSetDriver END\n");
...
inline static int adopt_CAMERA_HW_Close(void)
{
   ...
   atomic_set(&g_CamHWOpening, 0);
		...
		{
			 PK_DBG("quit*****************\n");
			g_pSensorFunc = g_pSensorFunc_backup;
		}
	#endif
    // reset the delay frame flag	
    spin_lock(&kdsensor_drv_lock);
