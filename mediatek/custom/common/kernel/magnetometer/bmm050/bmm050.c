...
*/
#define BMM050_BLOCK_DAEMON_ON_SUSPEND
//#undef	BMM050_BLOCK_DAEMON_ON_SUSPEND
/*
* Enable gyroscope feature with BMM050
*/
#define BMM050_M4G	
#undef BMM050_M4G
/*
* Enable rotation vecter feature with BMM050
*/
#define BMM050_VRV	
#undef BMM050_VRV	
...
//<2012/09/18, SunnyHu, porting m-sensor selftest
/* Register for normal selftest */
#define BMM050_SELF_TEST_X__POS		0
#define BMM050_SELF_TEST_X__LEN		1
#define BMM050_SELF_TEST_X__MSK		0x01
#define BMM050_SELF_TEST_X__REG		BMM050_DATAX_LSB
#define BMM050_SELF_TEST_Y__POS		0
#define BMM050_SELF_TEST_Y__LEN		1
#define BMM050_SELF_TEST_Y__MSK		0x01
#define BMM050_SELF_TEST_Y__REG		BMM050_DATAY_LSB
#define BMM050_SELF_TEST_Z__POS		0
#define BMM050_SELF_TEST_Z__LEN		1
#define BMM050_SELF_TEST_Z__MSK		0x01
#define BMM050_SELF_TEST_Z__REG		BMM050_DATAZ_LSB
//>2012/09/18, SunnyHu, porting m-sensor selftest
...
BMM050_RETURN_FUNCTION_TYPE bmm050api_get_selftest(
		unsigned char *selftest);
//<2012/09/18, SunnyHu, porting m-sensor selftest
static BMM050_RETURN_FUNCTION_TYPE bmm050api_do_selftest(
		unsigned char *selftest);
//>2012/09/18, SunnyHu, porting m-sensor selftest
...
BMM050_RETURN_FUNCTION_TYPE bmm050api_init(struct bmm050api *bmm050)
{
	...
	/* wait two millisecond for BMM to settle */
...
//<2012/09/18, SunnyHu, porting m-sensor selftest
#define SELFTEST_ERR_NOT_FINISH	((char)-11)
#define SELFTEST_ERR_XYZ			((char)-12)
static BMM050_RETURN_FUNCTION_TYPE bmm050api_do_selftest(unsigned char *selftest)
{
	BMM050_RETURN_FUNCTION_TYPE comres = 0;
	unsigned char v_data1_u8r;
	unsigned char v_result_u8r[3] = {0};
	int i = 0;
	...
	*selftest = 0;
	...
	if (p_bmm050 == BMM050_NULL) {
		printk(KERN_ERR "~~~~~<%s> E_BMM050_NULL_PTR\n", __func__);
		comres = E_BMM050_NULL_PTR;
	} else {
		...
		comres = p_bmm050->BMM050_BUS_READ_FUNC(
				...
				&v_data1_u8r, 1);
		v_data1_u8r = BMM050_SET_BITSLICE(
				v_data1_u8r, BMM050_CNTL_S_TEST, 1);
		comres |= p_bmm050->BMM050_BUS_WRITE_FUNC(
				p_bmm050->dev_addr, BMM050_CNTL_S_TEST__REG,
				&v_data1_u8r, 1);
		...
		for(i=0; i<5; i++)
		{
			...
			// check if selftest complete
			comres = p_bmm050->BMM050_BUS_READ_FUNC(
					p_bmm050->dev_addr, BMM050_CNTL_S_TEST__REG,
					&v_data1_u8r, 1);
			v_data1_u8r = BMM050_GET_BITSLICE(v_data1_u8r, BMM050_CNTL_S_TEST);
			if(v_data1_u8r == 0) {
				printk(KERN_ERR "~~~~~<%s> selftest complete\n", __func__);
				break;
			}
			else
				printk(KERN_ERR "~~~~~<%s> selftest not complete %d\n", __func__, i+1);
		}
		if(i>=5)
		{
			printk(KERN_ERR "~~~~~<%s> SELFTEST_ERR_NOT_FINISH\n", __func__);
			return SELFTEST_ERR_NOT_FINISH;
		}

		// get selftest result of x,y,z axis
		comres = p_bmm050->BMM050_BUS_READ_FUNC(
				p_bmm050->dev_addr, BMM050_SELF_TEST_X__REG,
				&(v_result_u8r[0]), 1);
		comres = p_bmm050->BMM050_BUS_READ_FUNC(
				p_bmm050->dev_addr, BMM050_SELF_TEST_Y__REG,
				&(v_result_u8r[1]), 1);
		comres = p_bmm050->BMM050_BUS_READ_FUNC(
				p_bmm050->dev_addr, BMM050_SELF_TEST_Z__REG,
				&(v_result_u8r[2]), 1);
		printk(KERN_ERR "~~~~~<%s> v_result_u8r[] = 0x%02x, 0x%02x, 0x%02x\n", __func__, v_result_u8r[0], v_result_u8r[1], v_result_u8r[2]);
		
		v_result_u8r[0] = BMM050_GET_BITSLICE(v_result_u8r[0], BMM050_SELF_TEST_X);
		v_result_u8r[1] = BMM050_GET_BITSLICE(v_result_u8r[1], BMM050_SELF_TEST_Y);
		v_result_u8r[2] = BMM050_GET_BITSLICE(v_result_u8r[2], BMM050_SELF_TEST_Z);
		printk(KERN_ERR "~~~~~<%s> v_result_u8r[] & mask = 0x%02x, 0x%02x, 0x%02x\n", __func__, v_result_u8r[0], v_result_u8r[1], v_result_u8r[2]);

		for(i=0; i<3; i++)
		{
			if(v_result_u8r[i] == 0) {
				printk(KERN_ERR "~~~~~<%s> SELFTEST_ERR_XYZ v_result_u8r[%d] fail\n", __func__, i);
				*selftest = SELFTEST_ERR_XYZ;
			}
		}
	}
	return comres;
...
#if defined(BMM050_M4G) || defined(BMM050_VRV)
static int m4g_data[CALIBRATION_DATA_SIZE];
#endif //BMM050_M4G || BMM050_VRV
...
#endif //BMM050_VRV
#ifdef BMM050_BLOCK_DAEMON_ON_SUSPEND
static atomic_t driver_suspend_flag = ATOMIC_INIT(0);
#endif //BMM050_BLOCK_DAEMON_ON_SUSPEND
...
//<2012/09/18, SunnyHu, porting m-sensor selftest
		case MSENSOR_IOCTL_DO_SELFTEST:
			MSE_ERR( "%s MSENSOR_IOCTL_DO_SELFTEST", __FUNCTION__);
			ret = (int)bmm050api_do_selftest(&result);
			if(ret < 0) {
				MSE_ERR( "%s bmm050api_get_selftest fail = %d", __FUNCTION__, ret);
				return ret;
			}
			MSE_ERR( "%s bmm050api_do_selftest result = %d", __FUNCTION__, (int)result);
			return ((int)result);
...
static int bmm050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//<2012/10/25, SunnyHu, disable debug message
	//MSE_LOG("%s: OK\n", __func__);
	printk(KERN_INFO MSE_TAG "20121211 %s: OK\n", __func__);
	//>2012/10/25, SunnyHu, disable debug message
	return 0;
