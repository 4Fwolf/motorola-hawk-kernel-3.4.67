...
//<2014/05/06-samhuang, porting audio driver
...
{
#ifdef ARIMA_PROJECT_HAWK35
  #if defined( HAWK35_EP0 )
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ONE );
  #else
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ONE ); /* EP2 */
  #endif
#endif

#ifdef ARIMA_PROJECT_HAWK40
  ...
  #else
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ONE );  /* compatible EP1 */
    mt_set_gpio_out( GPIO110, GPIO_OUT_ONE ); /* FP */
  #endif
#endif
}
...
static void headset_switch_off(void)
{
#ifdef ARIMA_PROJECT_HAWK35
  #if defined( HAWK35_EP0 )
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ZERO );
  #else
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ZERO ); /* EP2 */
  #endif
#endif
  ...
  #else
    mt_set_gpio_out( GPIO_HEADSET_SW_EN_PIN, GPIO_OUT_ZERO );  /* compatible EP1 */
    mt_set_gpio_out( GPIO110, GPIO_OUT_ZERO ); /* FP */
  #endif
#endif
}
...
void Sound_Speaker_Turnon(int channel)
{
    ...
    if(gsk_on)
        return;
//<2014/06/20-Yuting Shih-[BU2SC00145520]. Merged with JB.
//HAWK-12135_START JIGNESH_PATEL :-FM voice output from speaker and earphone at the same time when set FM voice output from speaker only
...
//>2014/06/20-Yuting Shih-[BU2SC00145520].
...
{
//<2014/06/20-Yuting Shih. Merged with JB.
    headsetOn = true;
//>2014/06/20-Yuting Shih.
}
