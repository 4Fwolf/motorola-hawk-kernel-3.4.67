...
//<2012/09/27-14385-kevincheng,Hawk40 ALSPS Porting
#include <mach/mt_devs.h>
#include <mach/mt_typedefs.h>
#include <mach/mt_gpio.h>
#include <mach/mt_pm_ldo.h>
//>2012//09/27-14385-kevincheng
...
//<2012/10/23 ShermanWei
//<2012/11/02-16066-kevincheng.L snesor modify
//<2012/11/12-16731-kevincheng,Fix light sensor issue
#if defined(ARIMA_PROJECT_HAWK35) || defined(HAWK35_EP0)
    #define ALS_I_THRESHOLD          0x1a0          // incandescent threshold (1.65)
    #define ALS_F_THRESHOLD          0x270          // fluorescent threshold (2.4375)
    #define ALS_M_THRESHOLD          0x1c0           // incandescent and fluorescent mix threshold (1.75)
/*24 bits integer, 8 bits fraction*/
    #define ALS_AX2                  0x1b08  //27.03125		////0x1ce0  //28.875
    #define ALS_BX                   0x88a0  //136.625		////0x3e0   ///3.875
    #define ALS_C                    0x7a48  //122.28125	////0x4410  //68.067
    #define ALS_AX                   0x1494A
    #define ALS_B                    0x20028
	//<2013/1/14 -20242 Sky Liu, Modify P-sencer Calibration rule
    #define ALS_L                    0x190 //0x170//140
	//>2013/1/14 -20242- Sky Liu
#else
    #define ALS_I_THRESHOLD	     0x180//0x018F		// incandescent threshold (1.55859375)
    #define ALS_F_THRESHOLD	     0x380//0x0217		// fluorescent threshold (2.08984375)
    #define ALS_M_THRESHOLD	     0x1c0//0x0199 		// incandescent and fluorescent mix threshold (1.59765625)

    #define ALS_AX2		     0x2240//0x4C433
    #define ALS_BX		     0xaf80//0x11FE3
    #define ALS_C		     0xb180//0x4AFB3
    #define ALS_AX		     0x1494A
    #define ALS_B		     0x20028
    #define ALS_L                    0x1A0
#endif
...
//<2012/10/03 ShermanWei
//<2012/10/19-15123-kevincheng,[Hawk4.0]P sensor temp modify
//<2013/03/10-22804-Sky Liu,[Hawk3,5]P sensor yard rate low in factory, modify the parameter
#if defined(ARIMA_PROJECT_HAWK35) || defined(HAWK35_EP0)
#define P_SENSOR_LTHD			540 //800///1200
#define P_SENSOR_HTHD			600 //1000///1500
#else
#define P_SENSOR_LTHD			400//300
#define P_SENSOR_HTHD			500
#endif
//>2013/03/10-22804-Sky Liu
//>2012/10/19-15123-kevincheng
//>2012/10/03 ShermanWei

struct epl6881_i2c_addr {    /*define a series of i2c slave address*/
	u8  epl_addr;   /*epl6803 8bit i2c address*/
};
...
//static const char ElanEPLsensorName[]="ELAN_LS_SENSOR";
//static int initial_epl6801(struct elan_epl_data *epld);
...
#define EPL6881_I2C_ADDR_EPL 0   /*!< the index in obj->hw->i2c_addr: EPL address */
#define on 1
...
//<2012/10/19-15123-kevincheng,[Hawk4.0]P sensor temp modify
#if defined(ARIMA_PROJECT_HAWK35) || defined(HAWK35_EP0)
int threshold_base = 450;///<2013/01/22 ShermanWei ///600;
#else
int threshold_base = 350;
#endif
//>2012/10/19-15123-keivncheng
int Dyna_en = 0;
...
//<2013/01/09 ShermanWei ,Fix bug:LCM will off in phone call connected in case of CrossTalk is high
poverflag = false;
//>2013/01/09 ShermanWei
...
static int epl6881_get_addr(struct alsps_hw *hw, struct epl6881_i2c_addr *addr)
{
	...
	addr->epl_addr = (hw->i2c_addr[EPL6881_I2C_ADDR_EPL]);
...
static void epl6881_power(struct alsps_hw *hw, unsigned int on) 
{
	...
	if(hw->power_id != POWER_NONE_MACRO)
	{
		...
		else if(on)
		{
			...
			mt_set_gpio_out(GPIO48, 1);
			#elif defined( ARIMA_PROJECT_HAWK35 )
			//mt_set_gpio_out(GPIO184, 1);
			#endif
		}
		else
		{
			#if defined(ARIMA_PROJECT_HAWK40)			
			mt_set_gpio_out(GPIO48, 0);
			#elif defined( ARIMA_PROJECT_HAWK35 )
			//mt_set_gpio_out(GPIO184, 0);
			#endif
		}
	}
	power_on = on;
...
//<2012/11/07-15984-kevincheng,Improve P Sensor Dynamic Flow
                       calibaration_result=elan_calibaration_read();
...
//<2012/10/23 ShermanWei
//<2012/11/02-16066-kevincheng,L sensor modify
//<2012/11/12-16731-kevincheng,Fix light sensor issue
//<2012/11/12-16775-kevincheng,Light Sensor Equation
    struct elan_epl_data *epld = epl_data;
...
static void elan_epl_als_rawdata(void)
{
    ...
    //EPL_DEBUG_INTERFACE("[ELAN epl6881] %s done\n", __func__);
}
...
//<2012/10/03 ShermanWei, Add for show important Register Data
...
static DEVICE_ATTR(elan_ls_operationmode, S_IWUSR|S_IRUGO,elan_ls_operationmode_show,elan_ls_operationmode_store);
...
//<2013/01/03 ShermanWei,fix bug Power Down epl6881
ret = elan_epl6881_I2C_Write(client,REG_7, W_SINGLE_BYTE, 0x02, EPL_C_P_DOWN);
//>2013/01/03 ShermanWei,
    return ret;
}
...
void epl6881_calibration_polling(void)
{
     ...
     EPL_DEBUG_INTERFACE("[ELAN_epl6881]%d\n");
     elan_epl6881_psensor_enable(epld);
...
//>2012/09/10-13741-kevincheng
    ...
    printk(KERN_INFO"elan epl6801 sensor probe success.\n");
...
/*
static const struct i2c_device_id elan_epl6801_id[] =
{
    { ELAN_LS_6801, 0 },
    { }
};
...
static int __init elan_epl6881_init(void)
{
    ...
    i2c_register_board_info(0, &epl6881_i2c_alsps, 1);
