...
/*----------------------------------------------------------------------------*/
#define DEBUG 1
//#define DEBUG_MSG 1 // 2012/10/25, SunnyHu, disable debug message
/*----------------------------------------------------------------------------*/
...
static struct i2c_board_info __initdata bma250_i2c_info ={ I2C_BOARD_INFO(BMA250_DEV_NAME, BMA250_I2C_ADDR)};
...
//<2012/10/25, SunnyHu, disable debug message
#if DEBUG_MSG
...
/*----------------------------------------------------------------------------*/
//<2012/09/18, SunnyHu, porting g-sensor selftest
#define BMA250_BW_1kHZ		0x0f
#define V_CTRL_REG3_X_1		0x11
#define V_CTRL_REG3_X_2		0x15
#define V_CTRL_REG3_Y_1		0x12
#define V_CTRL_REG3_Y_2		0x16
#define V_CTRL_REG3_Z_1		0x13
#define V_CTRL_REG3_Z_2		0x17
#define V_CTRL_REG3_END		0x00
#define SELFTEST_MIN_X			-51
#define SELFTEST_MAX_X			51
#define SELFTEST_MIN_Y			-51
#define SELFTEST_MAX_Y			51
#define SELFTEST_MIN_Z			-25
#define SELFTEST_MAX_Z			25
#define SAMPLE_TIMES			15
#define SELFTEAT_ERR_I2C			-11
#define SELFTEST_ERR_READ_DATA	-12
#define SELFTEST_ERR_SPEC			-13
static int BMA250_DoSelfTest(struct i2c_client *client)
{
	struct bma250_i2c_data *obj = i2c_get_clientdata(client);
	s16 tmpbuf[BMA250_AXES_NUM];
	s16 xyz, xyz_new, xyz_diff[BMA250_AXES_NUM];
	int i, res = 0;

	//----------------------------------------------------------------//
	//write initial configuration for selftest                                                             //
	//----------------------------------------------------------------//
	// set g-range to 8g
	res = hwmsen_write_byte(client, BMA250_REG_DATA_FORMAT, BMA250_RANGE_8G);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// set bandwidth to 1kHz
	res = hwmsen_write_byte(client, BMA250_REG_BW_RATE, BMA250_BW_1kHZ);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	...
	//----------------------------------------------------------------//
	// set x-1 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_X_1);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	for(i=0; i<SAMPLE_TIMES; i++) {
		if(res = BMA250_ReadData(client, tmpbuf))
			goto selftest_err_read_data;
		xyz += tmpbuf[0];
	}
	xyz /= SAMPLE_TIMES;
	// set x-2 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_X_2);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	xyz_new /= SAMPLE_TIMES;
	// calculate difference
	xyz_diff[0] = xyz_new - xyz;
	GSE_ERR("X diff = %d, expext >= +/-%d\n", xyz_diff[0], SELFTEST_MAX_X);
	if( (xyz_diff[0] > SELFTEST_MIN_X) && (xyz_diff[0] < SELFTEST_MAX_X) )
		goto selftest_err_spec;

	//----------------------------------------------------------------//
	// selftest for Y axis                                                                                      //
	//----------------------------------------------------------------//
	// set y-1 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_Y_1);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	xyz /= SAMPLE_TIMES;
	// set y-2 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_Y_2);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	xyz_new /= SAMPLE_TIMES;
	// calculate difference
	xyz_diff[1] = xyz_new - xyz;
	GSE_ERR("Y diff = %d, expext >= +/-%d\n", xyz_diff[1], SELFTEST_MAX_Y);
	if( (xyz_diff[1] > SELFTEST_MIN_Y) && (xyz_diff[1] < SELFTEST_MAX_Y) )
		goto selftest_err_spec;
	...
	//----------------------------------------------------------------//
	// set z-1 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_Z_1);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	xyz /= SAMPLE_TIMES;
	// set z-2 trigger
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_Z_2);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;
	// wait for 10ms
	mdelay(10);
	...
	xyz_new /= SAMPLE_TIMES;
	// calculate difference
	xyz_diff[2] = xyz_new - xyz;
	GSE_ERR("Z diff = %d, expext >= +/-%d\n", xyz_diff[2], SELFTEST_MAX_Z);
	if( (xyz_diff[2] > SELFTEST_MIN_Z) && (xyz_diff[2] < SELFTEST_MAX_Z) )
		goto selftest_err_spec;

	//----------------------------------------------------------------//
	//disable selftest and return to normal state                                                    //
	//----------------------------------------------------------------//
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_END);
	if(res != BMA250_SUCCESS) 
		goto selftest_err_i2c;

	GSE_ERR("selftest complete and pass\n");
	return 0;


selftest_err_i2c:
	GSE_ERR("I2C write byte error: ret value=%d\n", res);
	return SELFTEAT_ERR_I2C;

selftest_err_read_data:
	GSE_ERR("I2C read data error: ret value=%d", res);
	return SELFTEST_ERR_READ_DATA;

selftest_err_spec:
	//----------------------------------------------------------------//
	//disable selftest and return to normal state                                                    //
	//----------------------------------------------------------------//
	GSE_ERR("selftest fail, not meet for spec\n");
	res = hwmsen_write_byte(client, BMA250_SELF_TEST, V_CTRL_REG3_END);
	return SELFTEST_ERR_SPEC;
}
//>2012/09/18, SunnyHu, porting g-sensor selftest
...
//<2012/09/18, SunnyHu, porting g-sensor selftest
		case GSENSOR_IOCTL_DO_SELFTEST:
			GSE_ERR("===== %s cmd GSENSOR_IOCTL_DO_SELFTEST 20120918\n", __func__);
			/* useless
			// resume if the original state is suspend
			if(atomic_read(&obj->suspend))
			{
				GSE_ERR("===== %s resume first\n", __func__);
				original_suspend = 1;
			...
			*/
			// do selftest
			GSE_ERR("===== %s do selftest\n", __func__);
			err = BMA250_DoSelfTest(client);
			/* useless
			// suspend if the original state is suspend
			if(original_suspend)
			{
				GSE_ERR("===== %s suspend to original state\n", __func__);
				atomic_set(&obj->suspend, 1);
				if(tmperr = BMA250_SetPowerMode(obj->client, false))
				{
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//<2012/10/25, SunnyHu, disable debug message
	//GSE_LOG("%s: OK\n", __func__);
	printk(KERN_INFO GSE_TAG "20121211 %s: OK\n", __func__);
	//>2012/10/25, SunnyHu, disable debug message
...
static int __init bma250_init(void)
{
	...
	GSE_FUN();
	i2c_register_board_info(hw->i2c_num, &bma250_i2c_info, 1);
