...
sub ReadExcelFile()
{
	...
	while($pt_name ne "END"){
		...
		if ($EMMC_SUPPORT eq "yes")
		{
				if($MTK_SHARED_SDCARD eq "samyes"){
...
# Fill MBR & EBR table -----------------------------------------------------
	for ($iter=0; $iter<@PARTITION_FIELD; $iter++) {   
		...
		if($PARTITION_FIELD[$iter] eq "FLEX"){
				$start_block[$PARTITION_IDX_FIELD[$iter]] = ($START_FIELD_Byte[$iter]-$mbr_start)/512;
				$size_block[$PARTITION_IDX_FIELD[$iter]] =  ($SIZE_FIELD_KB[$iter])*1024/512; 
		}
		if($PARTITION_FIELD[$iter] eq "USRDATA"){
...
# <2012/05/17-samhunag, add flex partation
	my $p7_start_block;
	my $p7_size_block;
# >2012/05/17-samhuang
...
my @BR = (
	...
	["$PRODUCT_OUT/EBR1", [[0x83,0x0,0x0],
						[0x05,0x0,0x0]]],
	["$PRODUCT_OUT/EBR2", [[0x83,0x0,0x0],[0x83,0x0,0x0]]]
);
# >2012/05/17-samhuang
...
#		}
		if($PARTITION_FIELD[$iter] eq "ANDROID"){
			...
			$p6_start_block = ($START_FIELD_Byte[$iter-1]-$mbr_start)/512;
			$p6_size_block =  $SIZE_FIELD_KB[$iter-1]*1024/512;
		}
		# >2012/05/17-samhunag
	}
	#MBR
	print "Arima-MBR start is $mbr_start\n";
...
# <2012/05/17-samhunag, add flex partation
	if ($AddPartition == 1) {
		$BR[2][1][1][1] = 0x20 + $p6_size_block;
		$BR[2][1][1][2] = $p7_size_block;
        }
# >2012/05/17-samhunag
# Generate MBR&EBR binary file -----------------------------------------------------
foreach my $sBR (@BR){
